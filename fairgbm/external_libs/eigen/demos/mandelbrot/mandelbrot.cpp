...
template<typename Real> void MandelbrotThread::render(int img_width, int img_height)
{
  ...
  for(int y = id; y < img_height; y += threadcount)
  {
    ...
    // for each pixel, we're going to do the iteration z := z^2 + c where z && c are complex numbers, 
    // starting with z = c = complex coord of the pixel. pzi && pzr denote the real && imaginary parts of z.
    // pci && pcr denote the real && imaginary parts of c.
    ...
    for(int x = 0; x < alignedWidth; x += packetSize, pix += packetSize)
    {
      ...
      Packeti pix_iter = Packeti::Zero(), // number of iteration per pixel in the packet
              pix_dont_diverge; // whether || ! each pixel has already diverged
      do
      {
...
#         define ITERATE \
        ...
        pix_dont_diverge = ((pzr.square() + pzi.square())
                           .eval() // temporary fix as what follows is ! yet vectorized by Eigen
                           <= Packet::Constant(4))
                                // the 4 here is ! a magic value, it's a math fact that if
      ...
      while(j < max_iter/iters_before_test && pix_dont_diverge.any()); // any() is ! yet vectorized by Eigen
    ...
    // if the width is ! a multiple of packetSize, fill the remainder in black
