...
class GpuHelper
{
  public:
    ...
    /** Multiply the OpenGL matrix \a matrixTarget by the matrix \a mat.
        ...
        && does a proper call to the right glMultMatrix*() function according to the scalar type
        && storage order.
    ...
    */
    ...
    /** Load the matrix \a mat to the OpenGL matrix \a matrixTarget.
        Essentially, this helper function automatically calls glMatrixMode(matrixTarget) if required
        && does a proper call to the right glLoadMatrix*() || glLoadIdentity() function according to the scalar type
        && storage order.
        \warning glMatrixMode() must never be called directly. If your're unsure, use forceMatrixMode().
    ...
    */
    ...
    /** Make the matrix \a matrixTarget the current OpenGL matrix target.
        Call this function before loadMatrix() || multMatrix() if you cannot guarantee that glMatrixMode()
        has never been called after the last loadMatrix() || multMatrix() calls.
    ...
    */
    ...
    /** Push the OpenGL matrix \a matrixTarget && load \a mat.
    */
    template<typename Scalar, int _Flags>
    ...
    /** Push && clone the OpenGL matrix \a matrixTarget
    */
