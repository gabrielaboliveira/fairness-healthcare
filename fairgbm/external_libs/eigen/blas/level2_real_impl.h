...
int EIGEN_BLAS_FUNC(syr)(const char *uplo, const int *n, const RealScalar *palpha, const RealScalar *px, const int *incx, RealScalar *pc, const int *ldc)
{
  ...
  // if the increment is ! 1, let's copy it to a temporary vector to enable vectorization
...
/**  DSBMV  performs the matrix-vector  operation
  ...
  *     y := alpha*A*x + beta*y,
  *
  *  where alpha && beta are scalars, x && y are n element vectors &&
...
/**  DSPR    performs the symmetric rank 1 operation
  ...
  *     A := alpha*x*x' + A,
  *
  *  where alpha is a real scalar, x is an n element vector && A is an
...
/**  DSPR2  performs the symmetric rank 2 operation
  ...
  *
  *  where alpha is a scalar, x && y are n element vectors && A is an
  *  n by n symmetric matrix, supplied in packed form.
  */
