...
// this function takes a line that may contain a name &&/|| email address,
// && returns just the name, while fixing the "bad cases".
...
map<string,int> contributors_map_from_churn_output(const char *filename)
{
  ...
  while(!getline(churn_out,line).eof())
  {
    ...
    // get the number (of changesets || of modified lines for each contributor)
    int number;
...
// for "van den Schbling" types of last names, that's ! a problem, that's actually what we want.
