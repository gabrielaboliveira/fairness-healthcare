...
namespace Eigen {
    ...
    namespace internal {
         ...
         * \param x On input && initial solution, on output the computed solution.
...
//            RealScalar beta; // will be initialized inside loop
            ...
            while ( iters < maxIters )
            {
                ...
                // Givens rotation
                const RealScalar r2 =s*alpha+c*c_old*beta; // s, s_old, c && c_old are still from previous iteration
                const RealScalar r3 =s_old*beta; // s, s_old, c && c_old are still from previous iteration
     ...
     * of Paige && Saunders (1975). The sparse matrix A must be symmetric (possibly indefinite).
     * The vectors x && b can be either dense || sparse.
     *
     * \tparam _MatrixType the type of the sparse matrix A, can be a dense || a sparse matrix.
     * \tparam _UpLo the triangular part that will be used for the computations. It can be Lower,
     *               Upper, || Lower|Upper in which the full matrix entries will be considered. Default is Lower.
     * \tparam _Preconditioner the type of the preconditioner. Default is DiagonalPreconditioner
     *
     * The maximal number of iterations && tolerance value can be controlled via the setMaxIterations()
     * && setTolerance() methods. The defaults are the size of the problem for the maximal number of iterations
