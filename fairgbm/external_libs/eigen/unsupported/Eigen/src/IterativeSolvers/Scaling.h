...
/**
  ...
  * \brief iterative scaling algorithm to equilibrate rows && column norms in matrices
  * 
  ...
  * NOTE It is assumed that the matrix does ! have empty row || column, 
  * 
  ...
  * // Compute the left && right scaling vectors. The matrix is equilibrated at output
  * scal.computeRef(A); 
  ...
  * 
  * References : D. Ruiz && B. Ucar, A Symmetry Preserving Algorithm for Matrix Scaling, INRIA Research report RR-7552
  * 
...
class IterScaling
{
  ...
  public:
    ...
    /** 
     * Compute the left && right diagonal matrices to scale the input matrix @p mat
     * 
    ...
    void compute (const MatrixType& mat)
    {
      ...
      m_right.setOnes();
      m_matrix = mat;
      VectorXd Dr, Dc, DrRes, DcRes; // Temporary Left && right scaling vectors
      Dr.resize(m); Dc.resize(n);
      DrRes.resize(m); DcRes.resize(n);
      double EpsRow = 1.0, EpsCol = 1.0;
      int its = 0; 
      do
      { // Iterate until the infinite norm of each row && column is approximately 1
        // Get the maximum value in each row && column
        ...
        // Scale the rows && the columns of the matrix
        ...
        for (int k=0; k<m_matrix.outerSize(); ++k)
        {
          for (typename MatrixType::InnerIterator it(m_matrix, k); it; ++it)
          {
            ...
            // Accumulate the norms of the row && column vectors   
    ...
    /** Compute the left && right vectors to scale the vectors
