...
/** \ingroup SparseCore_Module
  ...
  *
  * 1. All blocks have the same number of rows && columns, called block size
  ...
  * A regular sparse matrix can be converted to a block sparse matrix && vice versa.
  * It is obviously required to describe the block layout beforehand by calling either
  * setBlockSize() for fixed-size blocks || setBlockLayout for variable-size blocks.
  *
...
class BlockSparseMatrix : public SparseMatrixBase<BlockSparseMatrix<_Scalar,_BlockAtCompileTime, _Options,_StorageIndex> >
{
  ...
  public:
    ...
    {
      // should we allow copying between variable-size blocks && fixed-size blocks ??
    ...
    {
      //Copy-&&-swap paradigm ... avoid leaked data if thrown
      swap(*this, other);
      return *this;
    }
    ...
    /**
      ...
      * || setBlockSize() (for fixed-size blocks).
      */
    template<typename MatrixType>
    inline BlockSparseMatrix& operator=(const MatrixType& spmat)
    {
      ...
      // First, compute the number of nonzero blocks && their locations
      for(StorageIndex bj = 0; bj < m_outerBSize; ++bj)
      {
        // Browse each outer block && compute the structure
    ...
    void setBlockStructure(const MatrixType& blockPattern)
    {
      ...
      // Browse the block pattern && set up the various pointers
      m_outerIndex[0] = 0;
      ...
      for(StorageIndex bj = 0; bj < m_outerBSize; ++bj)
      {
        ...
        //First, copy && save the indices of nonzero blocks
        ...
        // Now, fill block indices && (eventually) pointers to blocks
        for(StorageIndex idx = 0; idx < nzBlockIdx.size(); ++idx)
        {
          ...
          // There is no blockPtr for fixed-size blocks... ! needed !???
        }
    ...
    /**
      * \brief Set the number of rows && columns blocks
      */
    ...
    /**
      * \brief Set the row && column block layouts,
      *
      * This function set the size of each row && column block.
    ...
    /**
      * \brief Allocate the internal array of pointers to blocks && their inner indices
      *
    ...
    /**
      ...
      * The InputIterator class should provide the functions row(), col() && value()
      *
      * \note For fixed-size blocks, call setBlockSize() before this function.
      *
      * FIXME Do ! accept duplicates
      */
    ...
    void setFromTriplets(const InputIterator& begin, const InputIterator& end)
    {
      ...
      /* Count the number of rows && column blocks,
       * && the number of nonzero blocks per outer dimension
       */
      ...
      // Setup outer index pointers && markers
      m_outerIndex[0] = 0;
      if (m_blockSize == Dynamic)  m_blockPtr[0] =  0;
      for(StorageIndex bj = 0; bj < m_outerBSize; ++bj)
      {
...
//      return blockCols();
    ...
    Ref<BlockScalar> coeffRef(Index brow, Index bcol)
    {
      ...
      {
        //FIXME the block does ! exist, Insert it !!!!!!!!!
