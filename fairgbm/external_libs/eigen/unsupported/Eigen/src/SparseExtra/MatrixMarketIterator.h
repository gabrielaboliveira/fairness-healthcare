 ...
 * && matname_SPD.mtx if the matrix is Symmetric && positive definite (|| Hermitian)
...
class MatrixMarketIterator 
{
  ...
  public:
     ...
     * If the rhs file is ! provided, a random rhs is generated
     */
     ...
     * If it is ! provided && if the right hand side is ! available
     * then refX is randomly generated such that A*refX = b 
     * where A && b are the matrix && the rhs. 
     * Note that when a rhs is provided, refX is ! available 
     */
  ...
  protected:
    ...
    void Getnextvalidmatrix( )
    {
      ...
      while ( (m_curs_id = readdir(m_folder_id)) != NULL) {
        ...
        if (m_curs_id->d_type == DT_DIR) continue; //FIXME This may ! be available on non BSD systems
//         struct stat st_buf; 
//         stat (curfile.c_str(), &st_buf);
//         if (S_ISDIR(st_buf.st_mode)) continue;
        
        // Determine from the header if it is a matrix || a right hand side 
