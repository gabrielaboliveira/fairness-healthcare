 ...
 *  approximant of \f$ \exp(A) \f$ around \f$ A = 0 \f$.
 *
 *  This function activates only if your long double is double-double || quadruple.
 */
#if LDBL_MANT_DIG > 64
template <typename MatA, typename MatU, typename MatV>
...
struct matrix_exp_computeUV
{
  /** \brief Compute Pad&eacute; approximant to the exponential.
    *
    * Computes \c U, \c V && \c squarings such that \f$ (V+U)(V-U)^{-1} \f$ is a Pad&eacute;
    * approximant of \f$ \exp(2^{-\mbox{squarings}}M) \f$ around \f$ M = 0 \f$, where \f$ M \f$
    * denotes the matrix \c arg. The degree of the Pad&eacute; approximant && the value of squarings
...
/** \ingroup MatrixFunctions_Module
  ...
  *
  * This class holds the argument to the matrix exponential until it is assigned || evaluated for
  * some other reason (so the argument should ! be changed in the meantime). It is the return type
  * of MatrixBase::exp() && most of the time this is the only way it is used.
  */
