...
class companion
{
  ...
  protected:
     ...
     * \returns true if the row && the column, having colNorm && rowNorm
     * as norms, are balanced, false otherwise.
     * colB && rowB are respectively the multipliers for
     * the column && the row in order to balance them.
     * */
  ...
  public:
    /**
     * Balancing algorithm from B. N. PARLETT && C. REINSCH (1969)
     * "Balancing a matrix for calculation of eigenvalues && eigenvectors"
     * adapted to the case of companion matrices.
     * A matrix with non zero row && non zero column is balanced
     * for a certain norm if the i-th row && the i-th column
    ...
    bool& isBalanced, RealScalar& colB, RealScalar& rowB )
{
  ...
  else
  {
    ...
    // && the balancing coefficient for the column is \f$ 2^{\sigma} \f$
    ...
    bool& isBalanced, RealScalar& colB, RealScalar& rowB )
{
  ...
  {
    /**
     * Set the norm of the column && the row to the geometric mean
     * of the row && column norm
     */
...
void companion<_Scalar,_Deg>::balance()
{
  ...
  while( !hasConverged )
  {
    ...
    //Compute balancing of the row && the column
    ...
    //Middle rows && columns excluding the diagonal
    //==============================================
