...
struct kissfft_impl
{
  ...
  // perform two FFTs of src even && src odd
  ...
  inline
    ...
    {
      // TODO look for PlanKey(nfft, ! inverse) && conjugate the twiddles
  ...
  inline
    ...
    {
      using std::acos;
      std::vector<Complex> & twidref = m_realTwiddles[ncfft2];// creates new if ! there
