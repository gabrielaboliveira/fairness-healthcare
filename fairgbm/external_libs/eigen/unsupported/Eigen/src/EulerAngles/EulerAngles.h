...
namespace Eigen
{
  /** \class EulerAngles
    ...
    * Therefore, you can convert from Eigen rotation && to them
    *  (including rotation matrices, which is ! called "rotations" by Eigen design).
    *
    * Euler angles usually used for:
    *  - convenient human representation of rotation, especially in interactive GUI.
    *  - gimbal systems && robotics
    *  - efficient encoding(i.e. 3 floats only) of rotation for network protocols.
    *
    * However, Euler angles are slow comparing to quaternion || matrices,
    ...
    *  to the human friendly representation, && vise-versa.
    *
    * All the user need to do is a safe simple C++ type conversion,
    *  && this class take care for the math.
    * Additionally, some axes related computation is done in compile time.
    *
    * #### Euler angles ranges in conversions ####
    * Rotations representation as EulerAngles are ! single (unlike matrices),
    *  && even have infinite EulerAngles representations.<BR>
    * For example, add || subtract 2*PI from either angle of EulerAngles
    *  && you'll get the same rotation.
    * This is the general reason for infinite representation,
    *  but it's ! the only general reason for ! having a single representation.
    *
    ...
    * - otherwise, alpha && gamma angles will be in the range [-PI, PI].<BR>
    *   As for Beta angle:
    ...
    *
    * Convenient typedefs for EulerAngles exist for float && double scalar,
  ...
  class EulerAngles : public RotationBase<EulerAngles<_Scalar, _System>, 3>
  {
    ...
    public:
      ...
      EulerAngles() {}
      /** Constructs && initialize an EulerAngles (\p alpha, \p beta, \p gamma). */
      EulerAngles(const Scalar& alpha, const Scalar& beta, const Scalar& gamma) :
        m_angles(alpha, beta, gamma) {}
      
      // TODO: Test this constructor
      /** Constructs && initialize an EulerAngles from the array data {alpha, beta, gamma} */
      explicit EulerAngles(const Scalar* data) : m_angles(data) {}
      
      /** Constructs && initializes an EulerAngles from either:
        ...
        *  Alpha && gamma angles will be in the range [-PI, PI].<BR>
        *  As for Beta angle:
        *   - If the system is Tait-Bryan, the beta angle will be in the range [-PI/2, PI/2].
        *   - otherwise:
        *     - If the beta axis is positive, the beta angle will be in the range [0, PI]
      ...
      /** Constructs && initialize Euler angles from a rotation \p rot.
        *
      ...
      */
      ...
      /*EulerAngles(const QuaternionType& q)
      {
        ...
        //  we can compute only the needed matrix cells && then convert to euler angles. (see ZYX example below)
      ...
      */
      ...
      // TODO: Assign && construct from another EulerAngles (with different system)
