...
//both will be taken, && finally results will be merged
...
//the core series expansion 
    ...
    Packet4f temp=pmul(s4,z1);//! really a madd but a mul by z so that the next term can be a madd
    z1=pmadd(temp,x1,x1);
    z1=padd(z1,z1);
...
//the core series expansion 
    ...
    Packet4f temp=pmul(s4,z2);//! really a madd but a mul by z so that the next term can be a madd
    z2=pmadd(temp,x2,x2);
    }
