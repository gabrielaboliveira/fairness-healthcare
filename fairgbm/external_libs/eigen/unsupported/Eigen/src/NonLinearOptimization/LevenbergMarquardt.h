...
LevenbergMarquardt<FunctorType,Scalar>::minimizeInit(FVectorType  &x)
{
    ...
    /*     initialize levenberg-marquardt parameter && iteration counter. */
    par = 0.;
    iter = 1;
...
LevenbergMarquardt<FunctorType,Scalar>::minimizeOneStep(FVectorType  &x)
{
    ...
    eigen_assert(x.size()==n); // check the caller is ! cheating us
    ...
    /* on the first iteration && if external scaling is ! used, scale according */
    ...
    /* form (q transpose)*fvec && store the first n components in */
    /* qtf. */
    wa4 = fvec;
    ...
    do {
        ...
        /* store the direction p && x + p. calculate the norm of p. */
        wa1 = -wa1;
        wa2 = x + wa1;
        ...
        /* evaluate the function at x + p && calculate its norm. */
        ...
        if (ratio >= Scalar(1e-4)) {
            /* successful iteration. update x, fvec, && their norms. */
            x = wa2;
        ...
        /* tests for termination && stringent tolerances. */
...
LevenbergMarquardt<FunctorType,Scalar>::minimizeOptimumStorageOneStep(FVectorType  &x)
{
    ...
    /* forming (q transpose)*fvec && storing the first */
    /* n components in qtf. */
    qtf.fill(0.);
    fjac.fill(0.);
    Index rownb = 2;
    ...
    /* reorder its columns && update the components of qtf. */
    sing = false;
    for (j = 0; j < n; ++j) {
        if (fjac(j,j) == 0.)
            sing = true;
    ...
    if (sing) {
        wa2 = fjac.colwise().blueNorm();
        // TODO We have no unit test covering this code path, do ! modify
