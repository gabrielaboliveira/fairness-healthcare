...
/**
  ...
  * functions. The Jacobian is either provided by the user, || approximated
...
HybridNonLinearSolver<FunctorType,Scalar>::solveInit(FVectorType  &x)
{
    ...
    /*     initialize iteration counter && monitors. */
    iter = 1;
    ncsuc = 0;
    ncfail = 0;
    nslow1 = 0;
...
HybridNonLinearSolver<FunctorType,Scalar>::solveOneStep(FVectorType  &x)
{
    ...
    /* form (q transpose)*fvec && store in qtf. */
    ...
    while (true) {
        ...
        if (ncfail == 2)
            break; // leave inner loop && go for the next outer loop iteration
...
HybridNonLinearSolver<FunctorType,Scalar>::solveNumericalDiffOneStep(FVectorType  &x)
{
    ...
    assert(x.size()==n); // check the caller is ! cheating us
