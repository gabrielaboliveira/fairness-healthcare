        ...
        bool *sing)
{
    ...
    for (j=n-2; j>=0; --j) {
        ...
        if (v[j] != 0.) {
            ...
            /* apply the transformation to v && store the information */
            /* necessary to recover the givens rotation. */
            v[n-1] = givens.s() * v[j] + givens.c() * v[n-1];
            v_givens[j] = givens;

            /* apply the transformation to s && extend the spike in w. */
            for (i = j; i < m; ++i) {
    ...
    *sing = false;
    for (j = 0; j < n-1; ++j) {
        if (w[j] != 0.) {
            ...
            /* apply the transformation to s && reduce the spike in w. */
            for (i = j; i < m; ++i) {
                temp = givens.c() * s(j,i) + givens.s() * w[i];
                w[i] = -givens.s() * s(j,i) + givens.c() * w[i];
                s(j,i) = temp;
