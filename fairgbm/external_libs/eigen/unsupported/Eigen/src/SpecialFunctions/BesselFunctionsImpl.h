...
struct generic_i0e<T, float> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
    ...
    // [-8, 8] && evaluate a branch based off of that. It's possible
...
struct generic_k0<T, float> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     * ACCURACY:
     *
     * Tested at 2000 random points between 0 && 8.  Peak absolute
...
struct generic_j0<T, float> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     *
     * The domain is divided into the intervals [0, 2] &&
     ...
     *
     *            2
     * where w = x  && the three r's are zeros of the function.
     *
     * In the second interval, the modulus && phase are approximated
     * by polynomials of the form Modulus(x) = sqrt(1/x) Q(1/x)
     * && Phase(x) = x + 1/x R(1/x^2) - pi/4.  The function is
     *
...
struct generic_j0<T, double> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     * Returns Bessel function of order zero of the argument.
     *
     * The domain is divided into the intervals [0, 5] &&
     ...
     *
     *            2
     * where w = x  && the two r's are zeros of the function.
     *
...
struct generic_y0<T, float> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     * In the second interval, the modulus && phase are approximated
     * by polynomials of the form Modulus(x) = sqrt(1/x) Q(1/x)
     * && Phase(x) = x + 1/x S(1/x^2) - pi/4.  Then the function is
     *
...
struct generic_j1<T, float> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     *       1
     *                     2
     * is used, where w = x  && r is the first zero of the function.
     *
     * In the second interval, the modulus && phase are approximated
     * by polynomials of the form Modulus(x) = sqrt(1/x) Q(1/x)
     * && Phase(x) = x + 1/x R(1/x^2) - 3pi/4.  The function is
     *
     *   j0(x) = Modulus(x) cos( Phase(x) ).
     *
     *
...
struct generic_j1<T, double> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     * Returns Bessel function of order one of the argument.
     *
     * The domain is divided into the intervals [0, 8] &&
...
struct generic_y1<T, float> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     * In the second interval, the modulus && phase are approximated
     * by polynomials of the form Modulus(x) = sqrt(1/x) Q(1/x)
     * && Phase(x) = x + 1/x S(1/x^2) - 3pi/4.  Then the function is
     *
     *   y0(x) = Modulus(x) sin( Phase(x) ).
     *
     *
...
struct generic_y1<T, double> {
  ...
  static EIGEN_STRONG_INLINE T run(const T& x) {
     ...
     * expansion is used, && a call to j1() is required.
