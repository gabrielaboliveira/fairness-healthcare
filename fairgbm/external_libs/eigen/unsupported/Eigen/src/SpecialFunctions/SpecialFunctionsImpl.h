...
/** \internal \returns the error function of \a a (coeff-wise)
    ...
    This implementation works on both scalars && Ts.
*/
template <typename T>
 ...
 * && the other for y up to exp(-2).  For larger arguments,
 * w = y - 0.5, &&  x/sqrt(2pi) = w + w**3 R(w**2)/S(w**2)).
 *
 *
 * ACCURACY:
 *
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T generic_ndtri_lt_exp_neg_two(
  ...
  /* Approximation for interval z = sqrt(-2 log a ) between 2 && 8
   * i.e., a between exp(-2) = .135 && exp(-32) = 1.27e-14.
   */
  const ScalarType p1[] = {
  ...
  /* Approximation for interval z = sqrt(-2 log a ) between 8 && 64
   * i.e., a between exp(-32) = 1.27e-14 && exp(-2048) = 3.67e-890.
   */
  const ScalarType p2[] = {
...
struct cephes_helper {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE Scalar machep() { assert(false && "machep ! supported for this type"); return 0.0; }
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE Scalar big() { assert(false && "big ! supported for this type"); return 0.0; }
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE Scalar biginv() { assert(false && "biginv ! supported for this type"); return 0.0; }
};
...
struct igammac_cf_impl {
  /* Computes igamc(a, x) || derivative (depending on the mode)
...
struct igamma_series_impl {
  /* Computes igam(a, x) || its derivative (depending on the mode)
...
struct igammac_impl {
  ...
  static Scalar run(Scalar a, Scalar x) {
     ...
     * The integral is evaluated by either a power series ||
...
struct gamma_sample_der_alpha_impl
   ...
   * However, this formula is numerically unstable && expensive, so instead
   ...
   * in the same code, increasing the accuracy && speed due to cancellation
   * of some terms.
   *
...
struct zeta_impl {
    ...
    static Scalar run(Scalar x, Scalar q) {
         ...
         *                 k=0
         *
         * where x > 1 && q is ! a negative integer || zero.
         ...
         * REFERENCE:
         *
         * Gradshteyn, I. S., && I. M. Ryzhik, Tables of Integrals,
         * Series, && Products, p. 1073; Academic Press, 1980.
         *
         */
         ...
         * If q<0 && x is an integer, there is a relation to
...
struct betainc_impl {
  ...
  static EIGEN_STRONG_INLINE Scalar run(Scalar, Scalar, Scalar) {
     ...
     * implementation a && b are restricted to positive values.
     ...
     * ACCURACY (float):
     *
     * Tested at random points (a,b,x) with a && b in the indicated
     * interval && x between 0 && 1.
     *
     * arithmetic   domain     # trials      peak         rms
     * Relative error:
     ...
     *
     * Larger errors may occur for extreme ratios of a && b.
     *
     * ACCURACY (double):
     * arithmetic   domain     # trials      peak         rms
...
struct betainc_helper<double> {
  ...
  static EIGEN_STRONG_INLINE double incbps(double a, double b, double x) {
    ...
    u = a * numext::log(x);
    // TODO: gamma() is ! directly implemented in Eigen.
    /*
...
struct betainc_impl<double> {
  ...
  static double run(double aa, double bb, double xx) {
    ...
    /* Reverse a && b if x is greater than the mean. */
    if (xx > (aa / (aa + bb))) {
      reversed_a_b = true;
      a = bb;
      b = aa;
