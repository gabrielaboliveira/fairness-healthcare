...
struct functor_traits<scalar_bessel_i0_op<Scalar> > {
  enum {
    ...
    // The cost is N multiplications && 2N additions. We also add
...
struct functor_traits<scalar_bessel_i0e_op<Scalar> > {
  enum {
    // On average, a Chebyshev polynomial of order N=20 is computed.
    // The cost is N multiplications && 2N additions.
...
struct functor_traits<scalar_bessel_j0_op<Scalar> > {
  enum {
    ...
    // The cost is N multiplications && N additions each, along with a
    // sine, cosine && rsqrt cost.
...
struct functor_traits<scalar_bessel_k0_op<Scalar> > {
  enum {
    ...
    // The cost is N multiplications && 2N additions. In addition we compute
    // i0, a log, exp && prsqrt && sin && cos.
