...
EigenContractionKernelInternal(const LhsMapper lhs, const RhsMapper rhs,
  ...
  // declare && initialize 64 registers for output 8x8 block
  ...
  // on writes && very few 2-way conflicts on reads. There is an 8x8 grid of these blocks.

  // On the RHS we just add 8 padding elements to the end of each block. This gives no bank
  // conflicts on writes && also none on reads.
...
#define writeRegToShmem(_)                      \
  ...
  // declare && initialize result array
...
#undef initResultRow
  ...
  for (Index base_k = 0; base_k < k_size; base_k += 64) {
    ...
    // of LHS && one row of RHS into registers (takes 16 registers).
...
#define rrow(j) _rrow##j
    ...
    // Now x corresponds to k, y to m, && z to n
...
#undef computePass
  ...
  // we've now iterated over all of the large (ie width 64) k blocks &&
