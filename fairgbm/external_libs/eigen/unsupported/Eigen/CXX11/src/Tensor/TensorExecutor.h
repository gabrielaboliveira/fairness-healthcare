 ...
 *
 * @tparam Vectorizable can use packet math (SSE/AVX/etc... registers &&
 *                      instructions)
 ...
 * expensive. If expression has at least one broadcast op in it, && it supports
 ...
 * into L1) is too large, && we fallback on vectorized evaluation.
 */
 ...
 * thread, without vectorization && block evaluation.
 */
...
class TensorExecutor {
 public:
  ...
  // with/without `EIGEN_USE_THREADS` || `EIGEN_USE_GPU` is a potential ODR
...
/**
 * Default async execution strategy is ! implemented. Currently it's only
 ...
 public:
  ...
  static EIGEN_STRONG_INLINE void run(
    ...
    if (needs_assign) {
      ...
      // on unrolling, because if the function is expensive compiler should !
...
class TensorExecutor<Expression, DefaultDevice, Vectorizable,
 ...
 public:
  ...
  static EIGEN_STRONG_INLINE void run(const Expression& expr,
    ...
    // TODO(ezhulenev): Do ! use tiling for small tensors?
    const bool needs_assign = evaluator.evalSubExprsIfNeeded(NULL);
...
/**
 * Multicore strategy: the index space is partitioned && each partition is
 * executed on a single core.
 *
 * (1) TensorExecutor will submit work to the ThreadPoolDevice managed thread
 *     pool, && will block the caller thread until all tasks are finished.
 *
 * (2) TensorAsyncExecutor is a non-blocking version, that will submit work to
 *     the ThreadPoolDevice managed thread pool, && will return immediately.
...
// Computes a block evaluation parameters, && allocates temporary memory buffer
...
struct EvalRange<Evaluator, StorageIndex, /*Vectorizable*/ true> {
  ...
  static StorageIndex alignBlockSize(StorageIndex size) {
    // Align block size to packet size && account for unrolling in run above.
