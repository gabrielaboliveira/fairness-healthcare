...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, ThreadPoolDevice> :
  ...
  // inputs are RowMajor, we will "cheat" by swapping the LHS && RHS:
  // If we want to compute A * B = C, where A is LHS && B is RHS, the code
  // will pretend B is LHS && A is RHS.
  ...
  void evalProductImpl(Scalar* buffer, DoneCallback done) const {
    // This function computes a lot of heuristics in multiple steps, && it
    // also has multiple exit points. To keep it sane, readable && all in one
    ...
    //     to the device thread pool, && block on a barrier until it is
    //     completed.
    //
    // (2) In async mode we allocate Context on the heap, && after all tasks
    //     are finished, we call provided the done callback, && delete a
    //     context from the heap.
    //
    // (*) EvalParallelContext & EvalShardedByInnerDimContext owns all the state
    // && temporary buffers, requried for executing the tensor contraction.
    ...
    // - parallel packing || first lhs then rhs
    ...
    // values based on these approximations && then refine the approximations.
    ...
    // different input configurations, thread counts && instruction sets.
    // So feel free to question any of them.

    // Compute whether we want to shard by row || by column.
    ...
    // _parallelizable_ computations, && computations are ! parallelizable
    // across k dimension.
    ...
    // model is ! tuned. Remove this when the cost model is tuned.
    ...
    // Now that we know number of threads, recalculate sharding && blocking.
    ...
    // If there is enough concurrency in the sharding dimension, we choose !
    // to paralellize by the other dimension, && execute all kernels in sync
    // mode. This reduces parallelism from the nm x nn down to nn
    // (shard_by_col==true) || nm (shard_by_col==false).
    const Index sharding_dim_tasks = shard_by_col ? nn : nm;
    const int num_worker_threads = this->m_device.numThreadsInPool();

    // With small number of threads we want to make sure that we do ! reduce
    ...
    // Last by ! least, decide whether we want to issue both lhs && rhs
    // packing in parallel; || issue lhs packing first, && then issue rhs
    // packing when lhs packing completes (for !shard_by_col lhs && rhs are
...
#undef CONTEXT_ARGS
  ...
  // Asynchronous evaluation notification that does ! block in Wait().
  template <typename DoneCallback, typename Context>
  class EvalParallelNotification {
   public:
  ...
  // accessible by block packing && kernel tasks.
  ...
  class EvalParallelContext {
   public:
    ...
    EvalParallelContext(const Self* self, int num_threads, Scalar* buffer,
        ...
        : created_by_thread_id_(std::this_thread::get_id()),
          ...
          // by threads that are ! managed by the pool.
          ...
          // on the shard_by_col value && we parallelize by sharding dim ONLY.
      ...
      for (Index x = 0; x < P; x++) {
        ...
        // nm_ + nn_ notifications, because they will ! receive notifications
    ...
    void run() {
      ...
      //
      // If parallel evaluation is executed in async mode, this is a no-op, &&
      ...
      // the same thread, && will delete this context.
      //
      ...
      // worker threads && the system will deadlock.
      done_.Wait();
    }
   ...
   private:
    ...
    // This notification is specialized on the type of DoneCallback && can be
    ...
    // related to k-th slice, we need to issue m lhs packing tasks && n rhs
    // packing tasks.
    //
    ...
    // bottleneck we allow kernels from k-th && k+1-th slices to run in
    // parallel. Note that (m, n, k) && (m, n, k+1) kernels write to the same
    // output block, so they must ! run in parallel.
    //
    // This gives us the following dependency graph.
    // On each k slice we have m x n kernel tasks, m lhs paking tasks && n rhs
    // packing tasks.
    ...
    // will have it's own "thead local" (! a c++ thread local storage) memory
    // for packed_lhs || packed_rhs (shard_by_col = false of true). This memory
    ...
    // already computed (99% of the time), && we can pack data into the thread
    // local storage, && guarantee that all the kernels will be executed
    // immediately in the same thread. This significantly increases L1 cache hit
    // ratio && reduces pressure on the memory bus.
    //
    ...
    // && packed_rhs_ to allow kernels to be executed later on a thread
    ...
    // unique threads in a system is below || equal to the number of threads in
    // a thread pool. We will fallback on dynamic memory allocation after that.

    // ThreadLocalBlocks is a container for Lhs || Rhs thread local buffers. Its
    ...
    // initialization, && will reuse pre-allocated buffers if possible, || will
    ...
    class ThreadLocalBlocksInitialize {
     ...
     private:
      ...
      // Also explicit specializations are ! allowed at class scope in C++03,
    ...
    // longer schedule K+1 && following slices in thread local mode, because
    // there is no more guarantee that previous kernels were executed
    // sequentially in the same thread (size is nn_ || nm_).
    ...
    // In following two methods (pack_lhs && pack_rhs), if we know for sure
    // that we'll be able to immediately call a kernel with packed data, && do
    // ! submit it to the thread pool, we can use thread local memory for
    // packed data.
    //
    ...
    void pack_rhs(Index n, Index k) {
      ...
      for (Index n1 = n * gn_; n1 < nend; n1++) {
        if (!TensorContractionKernel::HasBeta && k == 0) {
          ...
          // ! support `beta`. Otherwise we will pass beta 0.0 to the first
          ...
          // worker thread, which can lead to underutilization && deadlocks.
    ...
    void enqueue_packing_helper(Index start, Index end, Index k, bool rhs) {
      if (end - start == 1) {
        ...
        // (1) pack_lhs && pack_rhs call signal_switch before completing
  ...
  struct EvalShardedByInnerDimContext {
   ...
   private:
    ...
    // the packet size && subtle breakage occurs if this is violated.
    ...
    // aggregation in parallel && reduce the size of the sequential step.
    //
    ...
    void evalAsync(Index start_block_idx, Index end_block_idx) {
      ...
      if (v == 1) {
        ...
        // it might deallocate Self* pointer captured by context, && we'll
    ...
    // tensor contraction mappers && computing loop bounds in gemm_pack_lhs
  ...
  // Decide whether we want to shard m x n contraction by columns || by rows.
  static bool shardByCol(Index m, Index n, Index num_threads) {
    // Note: we are comparing both n && m against Traits::nr, it is !
    // a mistake. We are trying to figure out how both n && m will fit into
    ...
    if (m / num_threads >= Traits::nr &&
        // && ! enough data for vectorization over columns
        (n / num_threads < Traits::nr ||
         // ... || barely enough data for vectorization over columns,
         // but it is ! evenly dividable across threads
         (n / num_threads < 4 * Traits::nr &&
          (n % (num_threads * Traits::nr)) != 0 &&
          // ... && it is evenly dividable across threads for rows
          ((m % (num_threads * Traits::nr)) == 0 ||
           // .. || it is ! evenly dividable for both dimensions but
           // there is much more data over rows so that corner effects are
           // mitigated.
           (m / n >= 6)))))
      return false;
    // Wait, || if matrices are just substantially prolonged over the other
    // dimension.
  ...
  // checkGrain checks whether grain (gm, gn) is suitable && is better than
  // (oldgm, oldgn).
  int checkGrain(Index m, Index n, Index bm, Index bn, Index bk, Index gm,
    ...
    // If it is too large, then we reject it && all larger tasks.
    ...
    // kernels && 4 threads. Grains of 2, 3 && 4 all yield good task sizes.
  ...
  TensorOpCost contractionCost(Index m, Index n, Index bm, Index bn, Index bk,
    ...
    if (prepacked) {
      // Packing && kernels are executed in different tasks. When we calculate
    ...
    // Lhs packing memory cost does ! contribute considerably to overall
    // execution time because lhs is prefetched early && accessed sequentially.
    if (shard_by_col)
  ...
  static bool shardByInnerDim(Index m, Index n, Index k, int num_threads,
    ...
    if (n == 1 ||                // If mat*vec ||...
        num_threads_by_k < 2 ||  // running single threaded ||...
        num_threads_by_k <
            num_threads ||  // sharding by k gives less parallelism ||...
      ...
      shard_by_k = false;
                   ...
                   Traits::nr ||  // both other dimensions are tiny ||...
               // k per thread is ! small &&...
               (k / num_threads_by_k > 8 * Traits::nr &&
                // one of the outer dimensions is tiny || sharding by k offers
                // more parallelism.
  ...
  double computeBandwidth(bool shard_by_col, Index bm, Index bn,
                          Index bk) const {
    // Peak VFMA bandwidth is 0.5. However if we have ! enough data for
    // vectorization bandwidth drops. The 4.0 && 2.0 bandwidth is determined
    // experimentally.
