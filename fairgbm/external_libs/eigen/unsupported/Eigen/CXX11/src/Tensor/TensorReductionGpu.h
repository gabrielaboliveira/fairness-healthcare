...
__device__ EIGEN_ALWAYS_INLINE void atomicReduce(T* output, T accum, R& reducer) {
  ...
  if (sizeof(T) == 4)
  {
    ...
    gpu_assert(0 && "Wordsize ! supported");
  }
...
// reduction should be associative since reduction is ! atomic in wide vector but atomic in half2 operations
template <template <typename T> class R>
...
#pragma unroll
  for (int offset = warpSize/2; offset > 0; offset /= 2) {
    ...
    // && list the float && int versions of __shfl_down as the candidate functions. 
...
struct FullReductionLauncher {
  static void run(const Self&, Op&, const GpuDevice&, OutputType*, typename Self::Index) {
    gpu_assert(false && "Should only be called on doubles, floats && half floats");
  }
};
...
struct FullReductionLauncher<Self, Op, Eigen::half, false> {
  static void run(const Self&, Op&, const GpuDevice&, half*, typename Self::Index) {
    gpu_assert(false && "Should ! be called since there is no packet accessor");
  }
};
...
struct FullReductionLauncher<Self, Op, Eigen::half, true> {
  static void run(const Self& self, Op& reducer, const GpuDevice& device, half* output, typename Self::Index num_coeffs) {
    ...
    if (num_blocks > 1) {
      // We initialize the output && the scrathpad outside the reduction kernel when we can't be sure that there
...
struct FullReducer<Self, Op, GpuDevice, Vectorizable> {
  ...
  static void run(const Self& self, Op& reducer, const GpuDevice& device, OutputType* output) {
    gpu_assert(HasOptimizedImplementation && "Should only be called on doubles, floats || half floats");
...
struct InnerReductionLauncher {
  static EIGEN_DEVICE_FUNC bool run(const Self&, Op&, const GpuDevice&, OutputType*, typename Self::Index, typename Self::Index) {
    gpu_assert(false && "Should only be called to reduce doubles, floats && half floats on a gpu device");
    return true;
  }
};
...
struct OuterReducer<Self, Op, GpuDevice> {
  ...
  static
    ...
    //
    // don't know why this happens (&& why is it a runtime error instead of a compile time error)
    //
    // this will be fixed by HIP PR#457
    EIGEN_DEVICE_FUNC
    #endif
    bool run(const Self&, Op&, const Device&, OutputType*, typename Self::Index, typename Self::Index) {
    gpu_assert(false && "Should only be called to reduce doubles || floats on a gpu device");
    return true;
  }
