 ...
 * \brief read, a template function used for loading the data from global
 memory. This function is used to guarantee coalesced && vectorized load whenever possible
 *
 * \tparam PacketLoad: determines if the each element of this tensor block should be loaded in a packet mode
 *
 * \param is_coalesced_layout: determines whether || ! the Tensor data in a memory can be access coalesced &&
 vectorized when possible. Coalesced memory access is a key factor in Kernel performance. When a tensor is 2d && the
 contracting dimension is 1, it is always possible to accessed tensor data coalesced && vectorized. This is the case
 when RHS(right hand side) Tensor is transposed || when LHS(left hand side) Tensor is ! transposed.
 *
...
/*!
 * \brief read, special overload of read function, when the read access is ! vectorized
 *
 * \tparam PacketLoad: determines if the each element of this tensor block should be loaded in a packet mode
 *
 * \param is_coalesced_layout: determines whether || ! the Tensor data in a memory can be access coalesced &&
 ...
 * coalesced && vectorized store whenever possible.
 *
 * \tparam StorageIndex: determines the Index type
 *
 ...
 * is used to guarantee coalesced && vectorized store whenever possible.
 *
 * \tparam data_source: an enum value representing if the location of the data in a memory hierarchy.
 *
 * \tparam PacketType:  determines the type of packet
 ...
 * by each workgroup.
 *
 * \tparam is_transposed: iff true, determines whether || ! the block of the Tensor is transposed
 *
 ...
 *
 * \param is_internal : this will determined if the thread within the work-group computes an internal block of tensor ||
 * the edge blocks. When it is internal, there is no need to check the boundaries && all the if stantement can be
 * resolve by compiler.
 */
 ...
 *
 * \tparam Vectorizable: determines whether || ! the vectorization is enabled for the Eigen expression.
 *
 ...
 *
 * \param scratch: local memory containing tiles of LHS && RHS tensors for each work-group
 *
...
class TensorContractionKernel {
 public:
  ...
  /**
   * \brief MemHolder this is a place holder struct for creating memory hierarchy in SYCL. Inside SYCL kernel it is !
   * allowed to have dynamic memory allocation. While the local memory is created outside of the kernel && passed to
   * the kernel as an accessor, the private memory can only allowed to be allocated statically. Since we are abstracting
   * the TiledMemory for both local && private memory, the MemHolder structs is used as a helper to abstract out
   ...
   *
   * \param lhs_scratch_extract : determines the LHS tile memory. It is either private || local memory based on the
   * selected contraction_type.
   *
   * \param rhs_scratch_extract : determines the RHS tile memory. It is either private || local memory based on the
   * selected contraction_type.
   *
   * \param lhs_extract_index: determins the position of each thread on a local memory for lhs input. When private
   * memory is used this is set to zero as this is ! applicable in case of private memory.
   *
  ...
  // The compute block computes the contraction operation private block for each thread && store the resutl in the
  // privateRes memory of Each computation the compute block function is independent of local && no local concepts as
  ...
  // memory. The store function is independent of local && no local concepts s that it can be abstract out in the base
  // class.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void store(OutPtr *out_ptr, PacketReturnType *privateRes,
    ...
    // when local memory is ! used M && N are both accessed in a coalesced way. However, when local memory is
    ...
    for (StorageIndex wLPTN = 0; wLPTN < Properties::WorkLoadPerThreadN / PrivateNStride; wLPTN++) {
      ...
      // memory && extracting from local to global is the same as no transposed version. However, when local memory is
      // ! used && RHS is transposed we packetize the load for RHS.
      EIGEN_UNROLL_LOOP
      for (StorageIndex nId = 0; nId < PrivateNStride; nId++) {
        ...
        for (StorageIndex wLPTM = 0; wLPTM < Properties::WorkLoadPerThreadM / PacketSize; wLPTM++) {
          ...
          if (check_boundary<is_internal_block>(chk_bound(globalRow, nId))) {
            // Store the final results in C. The C matrix has always M as a first StorageIndex && N as a second
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    ...
    for (StorageIndex cId = 0; cId < Properties::TileSizeDimK / InputBlockProperties::c_stride; cId++) {
      ...
      for (StorageIndex ncId = 0; ncId < WorkLoadPerThreadNC / InputBlockProperties::nc_stride; ncId++) {
        ...
        // even if M is ! accessed in a coalesced mode, we have to load packet_size number of m per thread.
 ...
 * (https://www.khronos.org/registry/SYCL/specs/sycl-1.2.1.pdf) for accessor definition)
 *
 * \tparam VectorMapper: determines the tensor contraction mapper for the vector input (can be lhs || rhs)
 *
 * \tparam TensorMapper: determines the tensor contraction mapper for the tensor input (can be lhs || rhs)
 *
 * \tparam StorageIndex: determines the StorageIndex Type
 *
 * \tparam Properties: determines the Contraction Panel properties
 ...
 * \tparam Vectorizable: determines whether || ! the vectorization is enabled for the Eigen expression.
 *
 * \tparam is_lhs_vec: determines whether lhs is a vector || rhs is a vector
 *
...
struct GeneralVectorTensor {
  ...
  // Since the access layout for a vector can always be coalesced, when LHS is a vector, we pass false && false to make
  // sure that the !^ is true When RHS is a vector, we pass true && true to make sure that the !^ is true.
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>,
  ...
  static_assert(std::is_same<OutputKernelType, const NoOpOutputKernel>::value,
                "SYCL tensor contraction does ! support output kernels.");
  ...
  void evalTyped(typename Base::EvaluatorPointerType buffer) const {
...
#endif
    {
      ...
      // This is based on empirical calculation for AMD r9-nano && Fiji
  ...
  EIGEN_ALWAYS_INLINE void launchSC(EvaluatorPointerType buffer, const LhsMapper &lhs, const RhsMapper &rhs,
    ...
    // Here we force the code ! to be more than 2-step reduction: Our empirical research shows that if each thread
