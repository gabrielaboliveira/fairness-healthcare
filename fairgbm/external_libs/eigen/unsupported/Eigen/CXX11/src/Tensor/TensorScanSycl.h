 ...
 * buffer is given as an input && all the threads within a work-group scan &&
 * reduces the boundaries between the blocks (generated from the previous
 * kernel). && write the data on the temporary buffer. If the second kernel is
 * required, the third && final kerenl (ScanAdjustmentKernelFunctor) will
 * adjust the final result into the output buffer.
 * The original algorithm for the parallel prefix sum can be found here:
 *
 * Sengupta, Shubhabrata, Mark Harris, && Michael Garland. "Efficient parallel
...
struct ScanLauncher<Self, Reducer, Eigen::SyclDevice, vectorize> {
  ...
  void operator()(Self &self, EvaluatorPointerType data) {
    ...
    // this is the scan op (can be sum || ...)
