...
struct TensorEvaluator<const TensorBroadcastingOp<Broadcast, ArgType>, Device>
{
  ...
  // We do block based broadcasting using a trick with 2x tensor rank && 0
 ...
 typedef internal::TensorBlockDescriptor<NumDims, Index> TensorBlockDesc;
        ...
        m_device(device), m_broadcast(op.broadcast()), m_impl(op.expression(), device)
  {
    ...
    // && store the result in a scalar. Instead one should reshape the scalar into a a N-D
    // tensor with N >= 1 of 1 element first && then broadcast.
    ...
    // Handle special format like NCHW, its input shape is '[1, N..., 1]' &&
  ...
  // TODO: attempt to speed this up. The integer divisions && modulo are slow
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketReturnType packet(Index index) const
  {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      if (isCopy) {
        ...
        // See PR 437: on NVIDIA P100 && K20m we observed a x3-4 speed up by enforcing
  ...
  // Ignore the LoadMode && always use unaligned loads since we can't guarantee
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packetColMajor(Index index) const
  {
    ...
    // Todo: this could be extended to the second dimension if we're !
    // broadcasting alongside the first dimension, && so on.
  ...
  block(TensorBlockDesc& desc, TensorBlockScratch& scratch,
    ...
    // We will fill output block by broadcasting along the bcast dim, &&
 ...
 private:
  ...
  // before `bcast_dim` (only dimension `2` in this example) && computing
  ...
  // single block along the broadcasting dimension. Sizes && strides along the
  ...
  struct BlockBroadcastingParams {
    ...
    // Block sizes && strides for the input block where all dimensions before
    // `bcast_dim` are equal to `1`.
    Dimensions input_block_sizes;
    Dimensions input_block_strides;

    // Block sizes && strides for blocks with extra dimensions && strides `0`.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index BroadcastBlockAlongBcastDim(
    ...
    if (params.bcast_dim_size == 1) {
      ...
      //
      // Or, when b && c do ! exist, we just need to process the whole block
      // together.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index BroadcastBlock(
    ...
    // ---------------------------------------------------------------------- //
    // Materialize input block into a temporary memory buffer only if it's !
    ...
    if (input_block.data() != NULL) {
      ...
      // Maybe reuse previously allocated buffer, || allocate a new one with a
      // scratch allocator.
