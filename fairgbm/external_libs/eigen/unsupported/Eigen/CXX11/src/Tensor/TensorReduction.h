...
struct FullReducer<Self, Op, ThreadPoolDevice, Vectorizable> {
  ...
  // launch one reducer per thread && accumulate the result.
 ...
 * with the accumulator && the other for reducing two accumulators.
 * Such a reducer can be useful for instance when the accumulator is a boolean || a bitset that checks for
...
struct TensorReductionEvaluatorBase<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>
{
      ...
      : m_impl(op.expression(), device), m_reducer(op.reducer()), m_result(NULL), m_device(device)
  {
    ...
    // Build the bitmap indicating if an input dimension is reduced || !.
    for (int i = 0; i < NumInputDims; ++i) {
      m_reduced[i] = false;
    }
  ...
  bool evalSubExprsIfNeededCommon(EvaluatorPointerType data) {
    ...
    // Attempt to use an optimized reduction.
      ...
      // must break into two subexpression && use the SYCL generic Reducer on the device.
      if(RunningOnSycl) {
         const Index num_values_to_reduce = internal::array_prod(m_reducedDims);
         const Index num_coeffs_to_preserve = internal::array_prod(m_dimensions);
         if (!data) {
 ...
 // SYCL need the Generic reducer for the case the recution algorithm is neither inner, outer, && full reducer
 template <typename, typename, typename> friend struct internal::GenericReducer;
  ...
  // Bitmap indicating if an input dimension is reduced || !.
...
: public TensorReductionEvaluatorBase<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Eigen::SyclDevice> {
  ...
  // The coeff function in the base the recursive method which is ! an standard layout && cannot be used in the SYCL kernel
  ...
  // The packet function in the base the recursive method which is ! an standard layout && cannot be used in the SYCL kernel
