...
{
  // Type promotion to handle the case where the types of the lhs && the rhs are different.
...
// Helper class to allocate && deallocate temporary memory for packed buffers.
...
// WARNING: In this code we assume that Lhs && Rhs tensor expressions are in
...
//   TensorContractionInputMapper, || some specialization of it based on the
...
struct TensorContractionKernel {
  ...
  // Pack blocks of Lhs && Rhs into contiguous blocks in memory.
  ...
  EIGEN_DEVICE_FUNC EIGEN_DONT_INLINE void invoke(
    ...
    // Default GEBP kernel does ! support beta.
 ...
 private:
  // These are dimensions of the original Tensors, && selected block sizes. The
...
struct TensorContractionEvaluatorBase
{
  ...
  TensorContractionEvaluatorBase(const XprType& op, const Device& device)
    ...
    // Check for duplicate axes && make sure the first index in eval_op_indices
    ...
    // dimensions of the left && then the right tensor. Additionally, we also
    // compute the strides corresponding to the left non-contracting
    // dimensions && right non-contracting dimensions.
    ...
    // in the matrix as they are in the tensor. This is ! the case for
...
#define TENSOR_CONTRACTION_ASYNC_DISPATCH(METHOD, DONE, ALIGNMENT, ARGS, FN) \
  ...
  EIGEN_DEVICE_FUNC void evalGemmPartial(Scalar* buffer, Index k_start, Index k_end, int num_threads) const {
    ...
    // define data mappers for Lhs && Rhs
    typedef typename internal::remove_const<typename EvalLeftArgType::Scalar>::type LhsScalar;
    typedef typename internal::remove_const<typename EvalRightArgType::Scalar>::type RhsScalar;
    ...
    // If a contraction kernel does ! support beta, explicitly initialize
