...
struct ScanLauncher {
  void operator()(Self& self, typename Self::CoeffReturnType* data) {
    ...
    // We fix the index along the scan axis to 0 && perform a
    // scan per remaining entry. The iteration is split into two nested
    // loops to avoid an integer division by keeping track of each idx1 &&
    // idx2.
...
// threads. Currently set to twice the cache line size on Intel && ARM
// processors.
...
// parallel, but it would be better to use a parallel scan algorithm &&
