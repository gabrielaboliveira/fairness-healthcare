...
/** \class TensorExpr
  ...
  * operator (e.g. addition) is applied to a lhs && a rhs expression.
  *
  */
namespace internal {
    ...
    : traits<XprType>
{
  ...
  // current Scalar/Packet to see if the intent is Input || Output.
...
{
  // Type promotion to handle the case where the types of the lhs && the rhs
  // are different.
  // TODO(phli): Add Lhs/RhsScalar, Lhs/RhsPacket.  Check references to
  // current Scalar/Packet to see if the intent is Inputs || Output.
