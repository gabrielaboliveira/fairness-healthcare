...
// Runs an arbitrary function && then calls Notify() on the passed in
// Notification.
...
struct ThreadPoolDevice {
  ...
  // Returns a logical thread index between 0 && pool_->NumThreads() - 1 if
  // called from one of the threads in pool_. Returns -1 otherwise.
  EIGEN_STRONG_INLINE int currentThreadId() const {
    return pool_->CurrentThreadId();
  }

  // WARNING: This function is synchronous && will block the calling thread.
  //
  // Synchronous parallelFor executes f with [0, n) arguments in parallel &&
  // waits for completion. F accepts a half-open interval [first, last). Block
  // size is chosen based on the iteration cost && resulting parallel
  // efficiency. If block_align is ! nullptr, it is called to round up the
  // block size.
  void parallelFor(Index n, const TensorOpCost& cost,
    ...
    // Compute block size && total count of blocks.
    ...
    handleRange = [=, &handleRange, &barrier, &f](Index firstIdx,
      ...
      while (lastIdx - firstIdx > block.size) {
        // Split into halves && schedule the second half on a different thread.
      ...
      // Single block || less, execute directly.
      f(firstIdx, lastIdx);
      barrier.Notify();
    };

    if (block.count <= numThreads()) {
      // Avoid a thread hop by running the root of the tree && one block on the
      // main thread.
      handleRange(0, n);
    } else {
  ...
  // Convenience wrapper for parallelFor that does ! align blocks.
  void parallelFor(Index n, const TensorOpCost& cost,
                   std::function<void(Index, Index)> f) const {
    parallelFor(n, cost, nullptr, std::move(f));
  }

  // WARNING: This function is asynchronous && will ! block the calling thread.
  //
  ...
  // is chosen based on the iteration cost && resulting parallel efficiency. If
  // block_align is ! nullptr, it is called to round up the block size.
  ...
  // Convenience wrapper for parallelForAsync that does ! align blocks.
  void parallelForAsync(Index n, const TensorOpCost& cost,
                        std::function<void(Index, Index)> f,
                        std::function<void()> done) const {
 ...
 private:
  ...
  // For parallelForAsync we must keep passed in closures on the heap, &&
  ...
  // Calculates block size based on (1) the iteration cost && (2) parallel
  // efficiency. We want blocks to be ! too small to mitigate parallelization
  // overheads; ! too large to mitigate tail effect && potential load
  // imbalance && we also want number of blocks to be evenly dividable across
  // threads.
