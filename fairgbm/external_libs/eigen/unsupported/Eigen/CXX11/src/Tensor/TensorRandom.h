...
EIGEN_DEVICE_FUNC uint64_t get_random_seed() {
  ...
  // We don't support 3d kernels since we currently only use 1 &&
  // 2d kernels.
...
template <typename T> class UniformRandomGenerator {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE UniformRandomGenerator(
    ...
    // In SYCL it is ! possible to build PCG_XSH_RS_state in one step. 
    // Therefor, we need two step to initializate the m_state.
    // IN SYCL, the constructor of the functor is s called on the CPU
    // && we get the clock seed here from the CPU. However, This seed is 
    //the same for all the thread. As unlike CUDA, the thread.ID, BlockID, etc is ! a global function.
    // && only  available on the Operator() function (which is called on the GPU).
    // Thus for CUDA (((CLOCK  + global_thread_id)* 6364136223846793005ULL) + 0xda3e39cb94b95bdbULL) is passed to each thread 
    // but for SYCL ((CLOCK * 6364136223846793005ULL) + 0xda3e39cb94b95bdbULL) is passed to each thread && each thread adds  
    // the  (global_thread_id* 6364136223846793005ULL) for itself only once, in order to complete the construction 
    // similar to CUDA Therefore, the thread Id injection is ! available at this stage. 
    ...
    //to the seed && construct the unique m_state per thead similar to cuda.  
    m_exec_once =false;
   #endif
  }
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE UniformRandomGenerator(
  ...
  T operator()(Index i) const {
    #ifdef EIGEN_USE_SYCL
      if(!m_exec_once) {
      // This is the second stage of adding thread Id to the CPU clock seed && build unique seed per thread
...
template <typename T> class NormalRandomGenerator {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE NormalRandomGenerator(uint64_t seed = 0) {
    ...
    //the same for all the thread. As unlike CUDA, the thread.ID, BlockID, etc is ! a global function.
    // && only  available on the Operator() function (which is called on the GPU).
    // Therefore, the thread Id injection is ! available at this stage. However when the operator() 
