...
struct TensorEvaluator<const TensorAssignOp<LeftArgType, RightArgType>, Device>
{
  ...
  {
    // The dimensions of the lhs && the rhs tensors should be equal to prevent
    // overflows && ensure the result is fully initialized.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(EvaluatorPointerType) {
    ...
    // evaluation isn't supported && the caller still needs to manually assign the values generated
  ...
  costPerCoeff(bool vectorized) const {
    // We assume that evalPacket || evalScalar is called to perform the
    // assignment && account for the cost of the write here, but reduce left
