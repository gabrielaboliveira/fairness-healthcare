...
/** \class Tensor
  ...
  * \tparam Scalar_  Numeric type, e.g. float, double, int || `std::complex<float>`.
  *                 User defined scalar types are supported as well (see \ref user_defined_scalars "here").
  * \tparam NumIndices_ Number of indices (i.e. rank of the tensor)
  *
  * The remaining template parameters are optional -- in most cases you don't have to worry about them.
  * \tparam Options_  A combination of either \b #RowMajor || \b #ColMajor, && of either
  *                 \b #AutoAlign || \b #DontAlign.
  *                 The former controls \ref TopicStorageOrders "storage order", && defaults to column-major. The latter controls alignment, which is required
  *                 for vectorization. It defaults to aligning tensors. Note that tensors currently do ! support any operations that profit from vectorization.
  ...
  * taking blocks || using tensors in expressions is easily possible, including an interface with the vector/matrix code
  * by providing .asMatrix() && .asVector() (|| similar) methods for rank 2 && 1 tensors. However, currently, the %Tensor
  * class does ! provide any of these features && is only available as a stand-alone class that just allows for
...
class Tensor : public TensorBase<Tensor<Scalar_, NumIndices_, Options_, IndexType_> >
{
  ...
  public:
    ...
    // work, because that uses base().coeffRef() - && we don't yet
