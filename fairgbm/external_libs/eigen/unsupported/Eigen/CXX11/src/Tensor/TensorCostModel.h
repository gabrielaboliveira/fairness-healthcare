...
// estimated number of operand bytes loads, bytes stored, && compute cycles.
class TensorOpCost {
 public:
  // TODO(rmlarsen): Fix the scalar op costs in Eigen proper. Even a simple
  // model based on minimal reciprocal throughput numbers from Intel ||
  ...
  // sequential || are completely masked by computations.
  EIGEN_DEVICE_FUNC void dropMemoryCost() {
    bytes_loaded_ = 0;
    bytes_stored_ = 0;
  }

  // TODO(rmlarsen): Define min in terms of total cost, ! elementwise.
  ...
  // TODO(rmlarsen): Define max in terms of total cost, ! elementwise.
...
class TensorCostModel {
 ...
 // Costs in device cycles.
  ...
  // evaluating an expression with the given output size && cost per
  // coefficient.
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE double totalCost(
    ...
    // We don't know whether data is in L1, L2 || L3. But we are most interested
    // in single-threaded computational time around 100us-10ms (smaller time
    // is too small for parallelization, larger time is ! interesting
    ...
    // only into L3 presumably will take more than 10ms to load && process.
