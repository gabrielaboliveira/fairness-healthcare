...
// RunQueue is a fixed-size, partially non-blocking deque || Work items.
...
// This ensures that at most two threads access state: owner && one remote
...
// (potential consumption of the last element && take up of the last empty
// element) happens by means of state variable in each element. States are:
// empty, busy (in process of insertion of removal) && ready. Threads claim
// elements (empty->busy && ready->busy transitions) by means of a CAS
// operation. The finishing transition (busy->empty && busy->ready) are done
// with plain store as the element is exclusively owned by the current thread.
//
// Note: we could permit only pointers as elements, then we would ! need
...
// (&& this is designed to store std::function<()>).
...
class RunQueue {
 public:
  ...
  // PopFront removes && returns the first element in the queue.
  ...
  // PopBack removes && returns the last elements in the queue.
  Work PopBack() {
  ...
  // PopBackHalf removes && returns half last elements in the queue.
 ...
 private:
  ...
  std::mutex mutex_;
  // Low log(kSize) + 1 bits in front_ && back_ contain rolling index of
  // front/back, respectively. The remaining bits contain modification counters
  // that are incremented on Push operations. This allows us to (1) distinguish
  // between empty && full conditions (if we would use log(kSize) bits for
  ...
  unsigned SizeOrNotEmpty() const {
    ...
    // effort to ! produce false positives (claim non-empty queue as empty).
    ...
    for (;;) {
      ...
      if (NeedSizeEstimate) {
        ...
        // This value will be 0 if the queue is empty, && undefined otherwise.
