...
// algorithms. Think of condition variable, but wait predicate does ! need to
...
//   ec.Notify(true);
//
// Notify is cheap if there are no waiting threads. Prewait/CommitWait are !
// cheap, but they are executed only if the preceding predicate check has
// failed.
//
// Algorithm outline:
// There are two main variables: predicate (managed by user) && state_.
// Operation closely resembles Dekker mutual algorithm:
// https://en.wikipedia.org/wiki/Dekker%27s_algorithm
// Waiting thread sets state_ then checks predicate, Notifying thread sets
// predicate then checks state_. Due to seq_cst fences in between these
// operations it is guaranteed than either waiter will see predicate change
// && won't block, || notifying thread will see state_ change && will unblock
// the waiter, || both. But it can't happen that both threads don't see each
...
class EventCount {
 public:
  ...
  // && then call either CancelWait || CommitWait.
  void Prewait() {
  ...
  void CommitWait(Waiter* w) {
    ...
    for (;;) {
      ...
      if ((state & kSignalMask) != 0) {
        // Consume the signal && return immidiately.
        newstate = state - kWaiterInc - kSignalInc;
      } else {
        // Remove this thread from pre-wait counter && add to the waiter stack.
  ...
  void CancelWait() {
    ...
    for (;;) {
      CheckState(state, true);
      uint64_t newstate = state - kWaiterInc;
      // We don't know if the thread was also notified || !,
      // so we should ! consume a signal unconditionaly.
      // Only if number of waiters is equal to number of signals,
      // we know that the thread was notified && we must take away the signal.
  ...
  // Notify wakes one || all waiting threads.
  ...
  void Notify(bool notifyAll) {
    ...
    for (;;) {
      ...
      uint64_t newstate;
      if (notifyAll) {
        // Empty wait stack && set signal to number of pre-wait threads.
        newstate =
        ...
        // Pop a waiter from list && unpark it.
 ...
 private:
  ...
  //   (stored in Waiter node && incremented on push).
  ...
  static void CheckState(uint64_t state, bool waiter = false) {
    static_assert(kEpochBits >= 20, "! enough bits to prevent ABA problem");
    const uint64_t waiters = (state & kWaiterMask) >> kWaiterShift;
    const uint64_t signals = (state & kSignalMask) >> kSignalShift;
