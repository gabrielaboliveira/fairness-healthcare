...
struct tensor_static_symgroup_equality
{
  ...
  private:
    /* this should be zero if they are identical, || else the tensor
     * will be forced to be pure real, pure imaginary || even pure zero
     */
    constexpr static int flags_cmp_ = ffa ^ ffb;

    /* either they are ! equal, then we don't care whether the flags
     * match, || they are equal, && then we have to check
     */
