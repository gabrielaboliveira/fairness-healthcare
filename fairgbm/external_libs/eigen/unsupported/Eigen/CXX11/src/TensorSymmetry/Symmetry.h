...
/** \class DynamicSGroup
  ...
  *
  * The %DynamicSGroup class represents a symmetry group that need ! be known at
  ...
  * || that the compiler could generate the group but that it wouldn't make sense to
...
/** \class StaticSGroup
  ...
  *
  * This class represents a symmetry group that is known && resolved completely
  * at compile time. Ideally, no run-time penalty is incurred compared to the
  * manual unrolling of the symmetry.
  *
  * <b><i>CAUTION:</i></b>
  *
  * Do ! use this class directly for large symmetry groups. The compiler
  * may run into a limit, || segfault || in the very least will take a very,
  ...
  * group becomes too large. (In that case, unrolling may ! even be
  * beneficial.)
  */
template<typename... Gen>
class StaticSGroup;
...
/** \class SGroup
  ...
  * known at compile time. It may || may ! be resolved at compile time,
...
private:
  ...
  // don't use std::max, since it's ! constexpr until C++14...
...
/** \internal
  ...
  * \ingroup TensorSymmetry_Module
  *
  * \brief Pre-select whether to use a static || dynamic symmetry group
  *
  * When a symmetry group could in principle be determined at compile time,
  * this template implements the logic whether to actually do that || whether
  ...
  *     compared to ! using symmetry at all. In practice, this might !
  *     be the case.</dd>
  ...
  * <dt><b>Up to && including 4 generators:</b></dt>
  ...
  *       with 16 GiB RAM (all generators non-redundant && the subgroups don't
  *       factorize):
  *
  ...
  * the memory && CPU requirements become unreasonable. Thus we only instantiate
  * the template group theory logic if the number of generators supplied is 4 ||
