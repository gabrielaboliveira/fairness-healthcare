...
static void test_full_reductions_sum_sycl(
  ...
  // Check that the CPU && GPU reductions return the same result.
...
static void test_full_reductions_max_with_offset_sycl(
  ...
  // As we don't include this in the reduction the result should ! be 2.
...
static void test_full_reductions_mean_with_odd_offset_sycl(
  ...
  // number of local threads in a workgroup is even, but is ! a power of two.
...
static void test_last_dim_reductions_max_with_offset_sycl(
  ...
  const IndexType offset = 64;
  // Introduce an offset in both the input && the output.
  TensorMap<data_tensor> in_offset(in.data() + offset, tensor_offset_range);
  TensorMap<reduced_tensor> red_offset(redux.data() + 1, reduced_range);
  red_offset = in_offset.maximum(red_axis);

  // Check that the first value hasn't been changed && that the reduced values
  // are ! equal to the previously set maximum in the input outside the range.
...
// Tensor contains nan &&/|| inf in one reduction
