...
void verify_euler(const EulerAngles<Scalar, EulerSystem>& e)
{
  ...
  // One of those cases is when the Euler angles are on the pole, && because it's singular case,
  ...
  // Test if ea && eabis are the same
  // Need to check both singular && non-singular cases
  // There are two singular cases.
  // 1. When I==K && sin(ea(1)) == 0
  // 2. When I!=K && cos(ea(1)) == 0

  // TODO: Make this test work well, && use range saturation function.
  /*// If I==K, && ea[1]==0, then there no unique solution.
  // The remark apply in the case where I!=K, && |ea[1]| is close to +-pi/2.
