...
void static test_no_out_of_bounds(const Device &sycl_device, IndexType m_size,
  ...
  // Allocate buffers twice as big to check for invalid read && write
...
// threshold for float && cause the test to fail. While it the GMV multiple
// kernel will be created && each one run the overflow of accumutation breaks
// among the kernels.
