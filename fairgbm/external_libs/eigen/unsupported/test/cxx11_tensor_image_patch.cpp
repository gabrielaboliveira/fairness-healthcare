          ...
          for (int d = 0; d < 2; ++d) {
            for (int b = 0; b < 7; ++b) {
              ...
              VERIFY_IS_EQUAL(entire_image_patch_row_major(b, patchId, c, r, d),
                              expected_row_major);
              // Check that ColMajor && RowMajor agree.
...
void test_patch_padding_valid()
{
  ...
  int input_cols = 3;
  int input_batches = 1;
  int ksize = 2;  // Corresponds to the Rows && Cols for tensor.extract_image_patches<>.
...
void test_patch_padding_valid_same_value()
{
  ...
  int input_cols = 5;
  int input_batches = 2;
  int ksize = 3;  // Corresponds to the Rows && Cols for tensor.extract_image_patches<>.
  int stride = 2;  // Only same stride is supported.
  // ColMajor
  Tensor<float, 4> tensor(input_depth, input_rows, input_cols, input_batches);
...
void test_patch_padding_same_negative_padding_clip_to_zero() {
  ...
  int input_cols = 1;
  int input_batches = 1;
  int ksize = 1;  // Corresponds to the Rows && Cols for
  ...
  // row padding will be computed as -2 originally && then be clipped to 0.
