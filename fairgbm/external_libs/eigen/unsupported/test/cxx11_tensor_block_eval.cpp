...
// -------------------------------------------------------------------------- //
// Utility functions to generate random tensors, blocks, && evaluate them.
...
// Block offsets && extents allows to construct a TensorSlicingOp corresponding
...
static void VerifyBlockEvaluator(Expression expr, GenBlockParams gen_block) {
  ...
  // Choose a random offsets, sizes && TensorBlockDescriptor.
  ...
  if (tensor_block.kind() == internal::TensorBlockKind::kMaterializedInOutput) {
    ...
    // We explicitly disable vectorization && tiling, to run a simple coefficient
    // wise assignment loop, because it's very simple && should be correct.
  ...
  // Tensor block && tensor slice must be the same.
...
static void test_eval_tensor_broadcast() {
  ...
  // Check that desc.destination() memory is ! shared between two broadcast
  // materializations.
  VerifyBlockEvaluator<T, NumDims, Layout>(
...
static void test_eval_tensor_slice() {
  ...
  // Make sure that slice start + size do ! overflow tensor dims.
  for (int i = 0; i < NumDims; ++i) {
...
static void VerifyBlockAssignment(Tensor<T, NumDims, Layout>& tensor,
  ...
  // We use tensor evaluator as a target for block && slice assignments.
  auto eval = TensorEvaluator<decltype(expr), Device>(expr, d);

  // Generate a random block, || choose a block that fits in full expression.
