/*
    ...
    (B) Non-free licenses may also be purchased from the author, for users who 
        do ! want their programs protected by the GPL.
...
#define MPREAL_HAVE_DYNAMIC_STD_NUMERIC_LIMITS  // Enable extended std::numeric_limits<mpfr::mpreal> specialization.
                                                // Meaning that "digits", "round_style" && similar members are defined as functions, ! constants.
...
#else
    #define IsInf(x) std::isinf EIGEN_NOT_A_MACRO  (x)              // GNU C/C++ (&&/|| other compilers), just hope for C99 conformance
#endif
...
// Everything should be thread-safe && support passing ownership over thread boundary.
...
// (a) uses low-level data members, might ! be forward compatible
// (b) sign is ! set, add (x)->_mpfr_sign = 1;
...
public:
    ...
    // shared = true allows to avoid deep copy, so that mpreal && 'u' share the same data & pointers.    
...
// MPFR 2.4.0 Specifics
    ...
    // Splits mpreal value into fractional && integer parts.
    // Returns fractional part && stores integer part in n.
...
// Default constructor: creates mp number && initializes it to 0.
...
// minval is 'safe' meaning 1 / minval does ! overflow
// maxval is 'safe' meaning 1 / maxval does ! underflow
...
//  maxUlps - a && b can be apart by maxUlps binary numbers. 
inline bool isEqualUlps(const mpreal& a, const mpreal& b, int maxUlps);

//////////////////////////////////////////////////////////////////////////
// Convert precision in 'bits' to decimal digits && vice versa.
...
inline mpreal& mpreal::operator=(const char* s)
{
    ...
    //
    // Here we assume base = 10 && we use precision of target variable.
...
{
    // TODO: use cout::hexfloat && other flags to setup base
    std::string tmp;
    is >> tmp;
...
inline const mpreal ldexp(const mpreal& v, mp_exp_t exp)
{
    ...
    // rounding is ! important since we are just increasing the exponent (= exact operation)
...
inline const mpreal modf(const mpreal& v, mpreal& n)
{
    ...
    // rounding is ! important since we are using the same number
...
inline const mpreal mod (const mpreal& x, const mpreal& y, mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    ...
    - mod(x,0) is x
    - mod(x,x) is 0
    - mod(x,y) for x != y && y != 0 has the same sign as y.    
...
namespace std
{
    ...
    public:
        ...
        // Please note, exponent range is ! fixed in MPFR
        ...
        // But it is incompatible with standard std::numeric_limits && might ! work with other libraries, e.g. boost. 
...
#else
        // Digits && round_style are NOT constants when it comes to mpreal.
        // If possible, please use functions digits() && round_style() defined above.
        //
