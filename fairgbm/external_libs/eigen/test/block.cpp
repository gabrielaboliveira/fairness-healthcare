...
// Check at compile-time that T1==T2, && at runtime-time that a==b
...
template<typename MatrixType> void block(const MatrixType& m)
{
  ...
  //check operator(), both constant && non-constant, on row() && col()
  m1 = m1_copy;
  ...
  //check operator(), both constant && non-constant, on block()
  ...
  if (rows>=5 && cols>=8)
  {
    ...
    // test operator() on fixed block() both as constant && non-constant
    m1.template block<BlockRows,BlockCols>(1,1)(0, 3) = m1.template block<2,5>(1,1)(1,2);
    // check that fixed block() && block() agree
