...
//     int c = internal::random<int>(0,depth-1);
    ...
    {
      VERIFY_EVALUATION_COUNT(m4 = m2*m3, 2); // 2 for transposing && get a sorted result.
    ...
    // sparse * dense && dense * sparse outer product
    {
  ...
  // test matrix - diagonal product
  {
    ...
    // evaluate to a dense matrix to check the .row() && .col() iterator functions
  ...
  // test self-adjoint && triangular-view products
  {
...
{
  // This code does ! compile with afflicted versions of the bug
