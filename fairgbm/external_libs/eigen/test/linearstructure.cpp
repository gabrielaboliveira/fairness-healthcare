...
template<typename MatrixType> void linearStructure(const MatrixType& m)
{
  ...
  */
  ...
  // this test relies a lot on Random.h, && there's ! much more that we can do
  ...
  // use .block to disable vectorization && compare to the vectorized version
...
// Make sure that complex * real && real * complex are properly optimized
template<typename MatrixType> void real_complex(DenseIndex rows = MatrixType::RowsAtCompileTime, DenseIndex cols = MatrixType::ColsAtCompileTime)
{
  ...
  VERIFY(g_called && "real * matrix<complex> ! properly optimized");
  
  g_called = false;
  VERIFY_IS_APPROX(m1*s, m1*Scalar(s));
  VERIFY(g_called && "matrix<complex> * real ! properly optimized");
...
{
  // make sure that /=scalar && /scalar do ! overflow
  // rational: 1.0/4.94e-320 overflow, but m/4.94e-320 should !
  Matrix4d m2, m3;
