...
// definition of macros min() && max().  The reason is that many STL
// implementations will ! work properly as the min && max symbols collide
// with the STL functions std:min() && std::max().  The STL headers may check
// for the macro definition of min/max && issue a warning || undefine the
// macros.
//
// Still, Windows defines min() && max() in windef.h as part of the regular
// Windows system interfaces && many other Windows APIs depend on these
// macros being available.  To prevent the macro expansion of min/max && to
// make Eigen compatible with the Windows environment all function calls of
// std::min() && std::max() have to be written with parenthesis around the
// function name.
//
// All STL headers used by Eigen should be included here.  Because main.h is
// included before any Eigen header && because the STL headers are guarded
...
// Same for cuda_fp16.h
  ...
  // Means the compiler is either nvcc || clang with CUDA enabled
...
// To test that all calls from Eigen code to std::min() && std::max() are
// protected by parenthesis against macro expansion, the min()/max() macros
// are defined here && any !-parenthesized min/max call will cause a
// compiler error.
  ...
  //  <regex>
  //  <unordered_map>
  // which seem to contain !-parenthesized calls to "max"/"min", triggering the following check && causing the compile to fail
  //
...
// On windows CE, NDEBUG is automatically defined <assert.h> if NDEBUG is ! defined.
#ifndef DEBUG
#define DEBUG
#endif
...
#define EIGEN_DEFAULT_IO_FORMAT IOFormat(4, 0, "  ", "\n", "", "", "", "")
  ...
  namespace Eigen
  {
    ...
    // case the exception is ! properly caught.
  ...
  // If EIGEN_DEBUG_ASSERTS is defined && if no assertion is triggered while
  // one should have been, then the list of executed assertions is printed out.
  //
  // EIGEN_DEBUG_ASSERTS is ! enabled by default as it
  // significantly increases the compilation time
  // && might even introduce side effects that would hide
  // some memory errors.
...
// test_relative_error returns the relative difference between a && b as a real scalar as used in isApprox.
template<typename T1,typename T2>
...
// get_test_precision is a small wrapper to test_precision allowing to return the scalar precision for either scalars || expressions
template<typename T>
...
// The idea behind this function is to compare the two scalars a && b where
// the scalar ref is a hint about the expected order of magnitude of a && b.
// WARNING: the scalar a && b must be positive
// Therefore, if for some reason a && b are very small compared to ref,
// we won't issue a false negative.
// This test could be: abs(a-b) <= eps * ref
// However, it seems that simply comparing a+ref && b+ref is more sensitive to true error.
...
/** Creates a random Partial Isometry matrix of given rank.
  *
  * A partial isometry is a matrix all of whose singular values are either 0 || 1.
...
int main(int argc, char *argv[])
{
    ...
    if(need_help)
    {
      ...
      std::cout << std::endl;
      std::cout << "If defined, the environment variables EIGEN_REPEAT && EIGEN_SEED" << std::endl;
...
// These warning are disabled here such that they are still ON when parsing Eigen's header files.
  ...
  // remark #1572: floating-point equality && inequality comparisons are unreliable
