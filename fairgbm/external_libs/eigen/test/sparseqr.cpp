...
template<typename Scalar> void test_sparseqr_scalar()
{
  ...
  // && in the full rank case the original matrix is recovered
  ...
  if(internal::random<float>(0,1)>0.5f)
    solver.factorize(A);  // this checks that calling analyzePattern is ! needed if the pattern do ! change.
