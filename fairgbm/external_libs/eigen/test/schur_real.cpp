...
template<typename MatrixType> void verifyIsQuasiTriangular(const MatrixType& T)
{
  ...
  // Check that any non-zero on the subdiagonal is followed by a zero && is
...
{
  // Test basic functionality: T is quasi-triangular && A = U T U*
  ...
  // Test whether compute() && constructor returns same result
  MatrixType A = MatrixType::Random(size, size);
  ...
  rs3.setMaxIterations(1).compute(Atriangular); // triangular matrices do ! need any iterations
  VERIFY_IS_EQUAL(rs3.info(), Success);
