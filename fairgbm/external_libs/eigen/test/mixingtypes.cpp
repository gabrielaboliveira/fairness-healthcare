...
// ignore double-promotion diagnostic for clang && gcc, if we check for static assertion anyway:
...
#define VERIFY_MIX_SCALAR(XPR,REF) \
  ...
  VERIFY( g_called && #XPR" ! properly optimized");

template<int SizeAtCompileType>
void raise_assertion(Index size = SizeAtCompileType)
{
  // VERIFY_RAISES_ASSERT(mf+md); // does ! even compile
  ...
  //   vd.asDiagonal() * mf;    // does ! even compile
  //   vcd.asDiagonal() * mf;   // does ! even compile
