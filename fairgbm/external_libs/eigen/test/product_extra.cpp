...
void bug_127()
{
  ...
  // max-sizes to detect size 1 indicating vectors, && that didn't account for 0-sized object with max-size 1.
...
void unaligned_objects()
{
  ...
  // Recall the matrix*vector kernel avoid unaligned loads by loading two packets && then reassemble then.
  // There was a mistake in the computation of the valid range for fully unaligned objects: in some rare cases,
  // memory was read outside the allocated matrix memory. Though the values were ! used, this might raise segfault.
