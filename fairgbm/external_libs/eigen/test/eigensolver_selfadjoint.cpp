...
template<typename MatrixType> void selfadjointeigensolver(const MatrixType& m)
{
  /* this test covers the following files:
     EigenSolver.h, SelfAdjointEigenSolver.h (&& indirectly: Tridiagonalization.h)
  */
  Index rows = m.rows();
  Index cols = m.cols();
  ...
  if(rows>1 && cols>1) {
    // FIXME check that upper && lower part are 0:
...
EIGEN_DECLARE_TEST(eigensolver_selfadjoint)
{
  ...
  for(int i = 0; i < g_repeat; i++) {
    ...
    // very important to test 3x3 && 2x2 matrices since we provide special paths for them
