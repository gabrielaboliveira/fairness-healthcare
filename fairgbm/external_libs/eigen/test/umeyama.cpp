...
Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> randMatrixUnitary(int size)
{
  ...
  while (!is_unitary && max_tries > 0)
  {
    ...
    // this additional orthogonalization is ! necessary in theory but should enhance
  ...
  if (max_tries == 0)
    eigen_assert(false && "randMatrixUnitary: Could ! construct unitary matrix!");
...
EIGEN_DECLARE_TEST(umeyama)
{
  ...
  // Those two calls don't compile && result in meaningful error messages!
