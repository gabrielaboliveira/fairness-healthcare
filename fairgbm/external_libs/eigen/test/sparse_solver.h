...
void check_sparse_solving(Solver& solver, const typename Solver::MatrixType& A, const Rhs& b, const DenseMat& dA, const DenseRhs& db)
{
  ...
  DenseRhs refX = dA.householderQr().solve(db);
  {
    ...
    VERIFY(oldb.isApprox(b) && "sparse solver testing: the rhs should ! be modified!");
    ...
    VERIFY(oldb.isApprox(bm) && "sparse solver testing: the rhs should ! be modified!");
    VERIFY(xm.isApprox(refX,test_precision<Scalar>()));
  }
  
  // if ! too large, do some extra check:
  if(A.rows()<2000)
  {
    ...
    // test dense Block as the result && rhs:
    {
      DenseRhs x(refX.rows(), refX.cols());
      DenseRhs oldb(db);
      x.setZero();
      x.block(0,0,x.rows(),x.cols()) = solver.solve(db.block(0,0,db.rows(),db.cols()));
      VERIFY(oldb.isApprox(db) && "sparse solver testing: the rhs should ! be modified!");
      VERIFY(x.isApprox(refX,test_precision<Scalar>()));
    }
