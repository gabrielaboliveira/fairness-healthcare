...
template<typename Scalar, int Mode, int Options> void transformations()
{
  ...
  */
  ...
  // The following test is stable only if 2*angle != angle && v1 is ! colinear with axis
  ...
  // first test setIdentity() && Identity()
  t0.setIdentity();
  VERIFY_IS_APPROX(t0.matrix(), Transform3::MatrixType::Identity());
  t0.matrix().setZero();
  t0 = Transform3::Identity();
  ...
  // mat * aligned scaling && mat * translation
  ...
  VERIFY_IS_APPROX(t0.matrix(), t1.matrix());
  // mat * transformation && aligned scaling * translation
  t1 = Matrix3(q1) * (AlignedScaling3(v0) * Translation3(v0));
  VERIFY_IS_APPROX(t0.matrix(), t1.matrix());
  ...
  // translation * aligned scaling && transformation * mat
  t1 = (Translation3(v0) * AlignedScaling3(v0)) * Transform3(q1);
  VERIFY_IS_APPROX(t0.matrix(), t1.matrix());
  // scaling * mat && translation * mat
  t1 = Translation3(v0) * (AlignedScaling3(v0) * Transform3(q1));
  VERIFY_IS_APPROX(t0.matrix(), t1.matrix());

  t0.setIdentity();
  t0.scale(v0).translate(v0).rotate(q1);
  // translation * mat && aligned scaling * transformation
