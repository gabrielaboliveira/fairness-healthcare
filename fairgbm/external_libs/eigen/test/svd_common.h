...
// Check that the matrix m is properly reconstructed && that the U && V factors are unitary
                         ...
                         const SvdType& referenceSvd)
{
  ...
  // && the resulting 'V' factor might be significantly different when the SVD decomposition is ! unique, especially with single precision float.
  ++g_test_level;
...
void svd_least_square(const MatrixType& m, unsigned int computationOptions)
{
  ...
  if(!test_isMuchSmallerThan(residual,rhs.norm()))
  {
    ...
    {
      using std::sqrt;
      // This test is ! stable with single precision.
...
// Check full, compare_to_full, least_square, && min_norm for all possible compute-options
template<typename SvdType, typename MatrixType>
