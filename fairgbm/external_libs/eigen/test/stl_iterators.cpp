...
void test_stl_iterators(int rows=Rows, int cols=Cols)
{
  ...
  // check std::sort
  {
    ...
    // on a column of a column-major matrix -> pointer-based iterator && default increment
    {
      j = internal::random<Index>(0,A.cols()-1);
      // std::sort(begin(A.col(j)),end(A.col(j))); // does ! compile because this returns const iterators
    ...
    // on a row of a rowmajor matrix -> pointer-based iterator && runtime increment
    {
      i = internal::random<Index>(0,A.rows()-1);
...
void test_stl_container_detection(int rows=Rows, int cols=Cols)
{
  ...
  // Vector && matrix col/row are valid Stl-style container.
  ...
  // But the matrix itself is ! a valid Stl-style container.
