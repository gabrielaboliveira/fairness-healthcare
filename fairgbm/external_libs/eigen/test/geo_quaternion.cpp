...
template<typename Scalar, int Options> void quaternion(void)
{
  ...
  */
  ...
  // Do ! execute the test if the rotation angle is almost zero, ||
  // the rotation axis && v1 are almost parallel.
...
//  std::cerr << array1 << " " << array2 << " " << array3 << "\n";
  ...
  // Bug 1461, compilation issue with Map<const Quat>::w(), && other reference/constness checks:
  ...
  // is used to determine whether we can return a coeff by reference || !, which is ! enough for Map<const ...>.
...
template<typename PlainObjectType> void check_const_correctness(const PlainObjectType&)
{
  ...
  // the only possible approach would be to run a script trying to compile stuff && checking that it fails.
...
struct MovableClass {
  // The following line is a workaround for gcc 4.7 && 4.8 (see bug 1573 comments).
