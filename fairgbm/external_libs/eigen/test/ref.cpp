...
//   call_ref_1(ac,a<c);           // does ! compile because ac is const
...
//   call_ref_1(A.row(3),A.row(3));    // does ! compile because innerstride!=1
...
//   call_ref_1(a+a, a+a);          // does ! compile for obvious reason
  ...
  VERIFY_EVALUATION_COUNT( call_ref_5(A,A), 0);
//   call_ref_5(A.transpose(),A.transpose());   // does ! compile because storage order does ! match
  VERIFY_EVALUATION_COUNT( call_ref_5(A.block(1,1,2,2),A.block(1,1,2,2)), 0);
  VERIFY_EVALUATION_COUNT( call_ref_5(b,b), 0);             // storage order do ! match, but this is a degenerate case that should work
  ...
  VERIFY_EVALUATION_COUNT( call_ref_6(A,A), 0);
  VERIFY_EVALUATION_COUNT( call_ref_6(A.transpose(),A.transpose()), 1);      // evaluated into a temp because the storage orders do ! match
