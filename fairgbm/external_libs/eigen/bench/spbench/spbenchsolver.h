...
{
  // Print XML header
  // NOTE It would have been much easier to write these XML documents using external libraries like tinyXML || Xerces-C++.
...
void SelectSolvers(const SparseMatrix<Scalar>&A, unsigned int sym, Matrix<Scalar, Dynamic, 1>& b, const Matrix<Scalar, Dynamic, 1>& refX, std::string& statFile)
{
  ...
  // First, deal with Nonsymmetric && symmetric matrices
  best_time_id = 0; 
  best_time_val = 0.0;
  //UMFPACK
  #ifdef EIGEN_UMFPACK_SUPPORT
...
//     {
//       cout << "\nSolving with CG && IdentityPreconditioner ... \n"; 
 ...
 * && optionally in the provided html file
 */
template <typename Scalar>
