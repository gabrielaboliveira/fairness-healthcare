...
void bench_printhelp()
{
    ...
    cout<< " The matrix && associated right-hand side (rhs) files are named respectively \n";
    cout<< " as MatrixName.mtx && MatrixName_b.mtx. If the rhs does ! exist, a random one is generated. \n";
...
int main(int argc, char ** args)
{
  ...
  if (!get_options(argc, args, "-d", &matrix_dir))
  {
    if(getenv("EIGEN_MATRIXDIR") == NULL){
      std::cerr << "Please, specify the location of the matrices with -d mat_folder || the environment variable EIGEN_MATRIXDIR \n";
  ...
  // Get the maximum number of iterations && the tolerance
  int maxiters = 1000; 
  double tol = 1e-08; 
  string inval; 
