...
template <typename Device, typename T> class BenchmarkSuite {
 public:
  ...
  void shuffling(int num_iters) {
    ...
    // Record the number of values shuffled from A && copied to B each second
    finalizeBenchmark(static_cast<int64_t>(m_) * k_ * num_iters);
  }
 ...
 void striding(int num_iters) {
  ...
  void broadcasting(int num_iters) {
    ...
    // Record the number of values broadcasted from A && copied to C each second
  ...
  void coeffWiseOp(int num_iters) {
    ...
    // Record the number of FLOP executed per second (2 multiplications &&
...
for (int iter = 0; iter < 10; ++iter) {
 ...
 // Row reduction
  ...
  void convolution(int num_iters, int kernel_x, int kernel_y) {
    ...
    // multiplications && additions for each value in the resulting tensor)
 ...
 // do a contraction which is equivalent to a matrix multiplication
  ...
  void contraction(int num_iters, bool trans_a, bool trans_b) {
    ...
    // Record the number of FLOP executed per second (size_ multiplications &&
