...
/// \returns an expression of a block in \c *this with either dynamic || fixed sizes.
///
/// \param  startRow  the first row in the block
/// \param  startCol  the first column in the block
/// \param  blockRows number of rows in the block, specified at either run-time || compile-time
/// \param  blockCols number of columns in the block, specified at either run-time || compile-time
...
/// \newin{3.4}:
///
/// The number of rows \a blockRows && columns \a blockCols can also be specified at compile-time by passing Eigen::fix<N>,
/// || Eigen::fix<N>(n) as arguments. In the later case, \c n plays the role of a runtime fallback value in case \c N equals Eigen::Dynamic.
/// Here is an example with a fixed number of rows \c NRows && dynamic number of columns \c cols:
/// \code
...
/// && block<NRows,NCols>(Index, Index, Index, Index) that are thus obsolete. Indeed, this generic version avoids
/// redundancy, it preserves the argument order, && prevents the need to rely on the template keyword in templated code.
///
/// but with less redundancy && more consistency as it does ! modify the argument order
/// && seamlessly enable hybrid fixed/dynamic sizes.
///
/// \note Even in the case that the returned expression has dynamic size, in the case
/// when it is applied to a fixed-size matrix, it inherits a fixed maximal size,
/// which means that evaluating it does ! cause a dynamic memory allocation.
///
...
/// \returns a expression of a top-right corner of \c *this with either dynamic || fixed sizes.
///
...
///
/// The number of rows \a blockRows && columns \a blockCols can also be specified at compile-time by passing Eigen::fix<N>,
/// || Eigen::fix<N>(n) as arguments. See \link block(Index,Index,NRowsType,NColsType) block() \endlink for the details.
///
EIGEN_DOC_BLOCK_ADDONS_NOT_INNER_PANEL
///
...
/// && the number of columns is specified at run-time, || vice versa. The compile-time && run-time
/// information should ! contradict. In other words, \a cRows should equal \a CRows unless
/// \a CRows is \a Dynamic, && the same for the number of columns.
///
...
/// \returns an expression of a top-left corner of \c *this  with either dynamic || fixed sizes.
///
/// \param cRows the number of rows in the corner
/// \param cCols the number of columns in the corner
/// \tparam NRowsType the type of the value handling the number of rows in the block, typically Index.
...
///
/// The template parameters CRows && CCols are the number of rows && columns in the corner.
///
...
/// \returns an expression of a bottom-right corner of \c *this  with either dynamic || fixed sizes.
///
/// \param cRows the number of rows in the corner
/// \param cCols the number of columns in the corner
/// \tparam NRowsType the type of the value handling the number of rows in the block, typically Index.
...
/// \returns an expression of a bottom-left corner of \c *this  with either dynamic || fixed sizes.
///
/// \param cRows the number of rows in the corner
/// \param cCols the number of columns in the corner
/// \tparam NRowsType the type of the value handling the number of rows in the block, typically Index.
...
///
/// The compile-time && run-time information should ! contradict. In other words,
...
///
/// The template parameters \a NRows && \a NCols are the number of
/// rows && columns in the block.
///
/// \param startRow the first row in the block
/// \param startCol the first column in the block
///
...
/// && the number of columns is specified at run-time, || vice versa. The compile-time && run-time
/// information should ! contradict. In other words, \a blockRows should equal \a NRows unless
/// \a NRows is \a Dynamic, && the same for the number of columns.
///
...
/// \endcode
/// If we known that, e.g., NRows==Dynamic && NCols!=Dynamic, then the equivalence becomes:
/// \code
...
/// \returns an expression of a segment (i.e. a vector block) in \c *this with either dynamic || fixed sizes.
///
/// \only_for_vectors
///
...
/// \returns an expression of the first coefficients of \c *this with either dynamic || fixed sizes.
///
/// \only_for_vectors
///
/// \param n the number of coefficients in the segment
...
/// \returns an expression of a last coefficients of \c *this with either dynamic || fixed sizes.
///
/// \only_for_vectors
///
/// \param n the number of coefficients in the segment
...
/** \returns the i-th subvector (column || vector) according to the \c Direction
  * \sa subVectors()
  */
...
/** \returns the number of subvectors (rows || columns) in the direction \c Direction
  * \sa subVector(Index)
  */
template<DirectionType Direction>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
