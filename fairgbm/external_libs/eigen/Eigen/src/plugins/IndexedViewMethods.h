...
// This file is automatically included twice to generate const && non-const versions
...
/**
  * \returns a generic submatrix view defined by the rows && columns indexed \a rowIndices && \a colIndices respectively.
  *
  * Each parameter must either be:
  *  - An integer indexing a single row || column
  *  - Eigen::all indexing the full set of respective rows || columns in increasing order
  *  - An ArithmeticSequence as returned by the Eigen::seq && Eigen::seqN functions
  *  - Any %Eigen's vector/array of integers || expressions
  ...
  *
  * If the submatrix can be represented using a starting position \c (i,j) && positive sizes \c (rows,columns), then this
  ...
  *  - An integer
  *  - Eigen::all
  *  - An ArithmeticSequence with compile-time increment strictly equal to 1, as returned by Eigen::seq(a,b), && Eigen::seqN(a,N).
  *
  * Otherwise a more general IndexedView<Derived,RowIndices',ColIndices'> object will be returned, after conversion of the inputs
  * to more suitable types \c RowIndices' && \c ColIndices'.
  *
  * For 1D vectors && arrays, you better use the operator()(const Indices&) overload, which behave the same way but taking a single parameter.
  *
  * See also this <a href="https://stackoverflow.com/questions/46110917/eigen-replicate-items-along-one-dimension-without-useless-allocations">question</a> && its answer for an example of how to duplicate coefficients.
  *
...
/** This is an overload of operator()(const RowIndices&, const ColIndices&) for 1D vectors || arrays
  *
  * \only_for_vectors
  */
template<typename Indices>
