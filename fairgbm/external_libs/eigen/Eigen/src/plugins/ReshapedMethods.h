...
///
/// \param nRows the number of rows in the reshaped expression, specified at either run-time || compile-time, || AutoSize
/// \param nCols the number of columns in the reshaped expression, specified at either run-time || compile-time, || AutoSize
...
///               || follows the \em natural order of the nested expression (AutoOrder). The default is ColMajor.
...
///
/// The number of rows \a nRows && columns \a nCols can also be specified at compile-time by passing Eigen::fix<N>,
...
/// Here is an example with a fixed number of rows && columns:
...
/// AutoSize does preserve compile-time sizes when possible, i.e., when the sizes of the input are known at compile time \b &&
...
/// \returns an expression of \c *this with columns (|| rows) stacked to a linear column vector
///
/// \tparam Order specifies whether the coefficients should be processed in column-major-order (ColMajor), in row-major-order (RowMajor),
///               || follows the \em natural order of the nested expression (AutoOrder). The default is ColMajor.
///
...
///   This mode is the recommended one when the particular ordering of the element is ! relevant.
///
/// Example:
