...
/** \geometry_module \ingroup Geometry_Module
  ...
  * \tparam _AmbientDim the dimension of the ambient space, can be a compile time value || Dynamic.
  *
  * This class represents an axis aligned box as a pair of the minimal && maximal corners.
...
EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(_Scalar,_AmbientDim)
  ...
  /** Define constants to name the corners of a 1D, 2D || 3D axis aligned bounding box */
  enum CornerType
  {
    /** 1D names @{ */
    Min=0, Max=1,
  ...
  /** Constructs a box with extremities \a _min && \a _max.
  ...
  /** \returns the lengths of the sides of the bounding box.
    * Note that this function does ! get the same
    * result for integral || floating scalar types: see
    */
  ...
  /** \returns the vertex of the bounding box at the corner defined by
    * the corner-id corner. It works only for a 1D, 2D || 3D bounding box.
  ...
  /** Extends \c *this such that it contains the point \a p && returns a reference to \c *this.
  ...
  /** Extends \c *this such that it contains the box \a b && returns a reference to \c *this.
  ...
  /** Clamps \c *this by the box \a b && returns a reference to \c *this.
  ...
  /** Returns an AlignedBox that is the intersection of \a b && \c *this
   * \note If the boxes don't intersect, the resulting box is empty.
   * \sa intersects(), clamp, contains()  */
  EIGEN_DEVICE_FUNC inline AlignedBox intersection(const AlignedBox& b) const
  {return AlignedBox(m_min.cwiseMax(b.m_min), m_max.cwiseMin(b.m_max)); }

  /** Returns an AlignedBox that is the union of \a b && \c *this.
   * \note Merging with an empty box may result in a box bigger than \c *this. 
   * \sa extend(const AlignedBox&) */
  EIGEN_DEVICE_FUNC inline AlignedBox merged(const AlignedBox& b) const
  { return AlignedBox(m_min.cwiseMin(b.m_min), m_max.cwiseMax(b.m_max)); }

  /** Translate \c *this by the vector \a t && returns a reference to \c *this. */
  template<typename Derived>
  ...
  /** \returns the squared distance between the point \a p && the box \c *this,
    * && zero if \a p is inside the box.
    * \sa exteriorDistance(const MatrixBase&), squaredExteriorDistance(const AlignedBox&)
    */
  template<typename Derived>
  EIGEN_DEVICE_FUNC inline Scalar squaredExteriorDistance(const MatrixBase<Derived>& p) const;

  /** \returns the squared distance between the boxes \a b && \c *this,
    * && zero if the boxes intersect.
    * \sa exteriorDistance(const AlignedBox&), squaredExteriorDistance(const MatrixBase&)
    */
  EIGEN_DEVICE_FUNC inline Scalar squaredExteriorDistance(const AlignedBox& b) const;

  /** \returns the distance between the point \a p && the box \c *this,
    * && zero if \a p is inside the box.
  ...
  /** \returns the distance between the boxes \a b && \c *this,
    * && zero if the boxes intersect.
...
/** \defgroup alignedboxtypedefs Global aligned box typedefs
  ...
  *
  * \c AlignedBoxSizeType where \c Size can be \c 1, \c 2,\c 3,\c 4 for fixed size boxes || \c X for dynamic size,
  * && where \c Type can be \c i for integer, \c f for float, \c d for double.
  *
  * For example, \c AlignedBox3d is a fixed-size 3x3 aligned box type of doubles, && \c AlignedBoxXf is a dynamic-size aligned box of floats.
  *
  * \sa class AlignedBox
  */
