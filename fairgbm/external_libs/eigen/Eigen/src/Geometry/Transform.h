...
/** \geometry_module \ingroup Geometry_Module
  ...
  *                           to speed up some functions such as inverse() && rotation().
  * \tparam _Options has the same meaning as in class Matrix. It allows to specify DontAlign &&/|| RowMajor.
  *                  These Options are passed directly to the underlying matrix type.
  *
  * The homography is internally represented && stored by a matrix which
  ...
  * && then the interpretation of different parts might be slightly different.
  *
  * However, unlike a plain matrix, the Transform class provides many features
  * simplifying both its assembly && usage. In particular, it can be composed
  * with any other transformations (Transform,Translation,RotationBase,DiagonalMatrix)
  * && can be directly used to transform implicit homogeneous vectors. All these
  * operations are handled via the operator*. For the composition of transformations,
  * its principle consists to first convert the right/left hand sides of the product
  * to a compatible (Dim+1)^2 matrix && then perform a pure matrix product.
  ...
  * that case the last matrix row can be ignored, && the product returns non
  * homogeneous vectors.
  *
  * Since, for instance, a Dim x Dim matrix is interpreted as a linear transformation,
  * it is ! possible to directly transform Dim vectors stored in a Dim x Dim matrix.
  * The solution is either to use a Dim x Dynamic matrix || explicitly request a
  ...
  *
  * Conversion methods from/to Qt's QMatrix && QTransform are available if the
...
public:
  ...
  // this intermediate enum is needed to avoid an ICE with gcc 3.4 && 4.0
  enum { TransformTimeDiagonalMode = ((Mode==int(Isometry))?Affine:int(Mode)) };
  /** The return type of the product between a diagonal matrix && a transform */
...
public:
  ...
  /** Default constructor without initialization of the meaningful coefficients.
    * If Mode==Affine || Mode==Isometry, then the last row is set to [0 ... 0 1] */
  ...
  /** Constructs && initializes a transformation from a Dim^2 || a (Dim+1)^2 matrix. */
  ...
  /** Set \c *this from a Dim^2 || (Dim+1)^2 matrix. */
  template<typename OtherDerived>
  ...
  EIGEN_DEVICE_FUNC inline Transform(const Transform<Scalar,Dim,OtherMode,OtherOptions>& other)
  {
    ...
    if(EIGEN_CONST_CONDITIONAL(ModeIsAffineCompact == OtherModeIsAffineCompact))
    {
      ...
      // combinations of transformations && will trigger a compile time error
    ...
    {
      // here we know that Mode == AffineCompact && OtherMode != AffineCompact.
  ...
  /** \returns an expression of the product between the transform \c *this && a matrix expression \a other.
    *
    ...
    *
    * In all cases, the return type is a matrix || vector of same sizes as the right-hand-side \a other.
    *
    * If you want to interpret \a other as a linear || affine transformation, then first convert it to a Transform<> type,
    * || do your own cooking.
    *
...
public:
  ...
  /** \internal
    ...
    *          && the HDim x Dim part for projective transformations.
    */
  ...
  /** \internal
    ...
    *          && the last column for projective transformations.
    */
...
/** \returns a QMatrix from \c *this assuming the dimension is 2.
  *
  * \warning this conversion might loss data if \c *this is ! affine
  *
  * This function is available only if the token EIGEN_QT_SUPPORT is defined.
  */
template<typename Scalar, int Dim, int Mode, int Options>
...
/** Applies on the right the non uniform scale transformation represented
  * by the vector \a other to \c *this && returns a reference to \c *this.
  * \sa prescale()
  */
template<typename Scalar, int Dim, int Mode, int Options>
template<typename OtherDerived>
...
/** Applies on the right the translation matrix represented by the vector \a other
  * to \c *this && returns a reference to \c *this.
  * \sa pretranslate()
  */
template<typename Scalar, int Dim, int Mode, int Options>
template<typename OtherDerived>
...
/** Applies on the right the rotation represented by the rotation \a rotation
  ...
  * || a pair of Quaternion for 4D rotations.
  *
...
/******************************************************
*** Scaling, Translation && Rotation compatibility ***
******************************************************/
...
/** Convenient method to set \c *this from a position, orientation && scale
  * of a 3D object.
  */
template<typename Scalar, int Dim, int Mode, int Options>
template<typename PositionDerived, typename OrientationType, typename ScaleDerived>
...
/**
  ...
  * is known to be ! a general transformation (optional). The possible values are:
  *  - #Projective if the transformation is ! necessarily affine, i.e., if the
  *    last row is ! guaranteed to be [0 ... 0 1]
  *  - #Affine if the last row can be assumed to be [0 ... 0 1]
  *  - #Isometry if the transformation is only a concatenations of translations
  *    && rotations.
  *  The default is the template class parameter \c Mode.
  *
  * \warning unless \a traits is always set to NoShear || NoScaling, this function
