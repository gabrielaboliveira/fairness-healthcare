...
/** \ingroup QR_Module
  ...
  *
  * This class performs a rank-revealing QR decomposition of a matrix \b A into matrices \b P, \b P', \b Q && \b R
  * such that 
  * \f[
  *  \mathbf{P} \, \mathbf{A} \, \mathbf{P}' = \mathbf{Q} \, \mathbf{R}
  * \f]
  * by using Householder transformations. Here, \b P && \b P' are permutation matrices, \b Q a unitary matrix 
  * && \b R an upper triangular matrix.
  *
  * This decomposition performs a very prudent full pivoting in order to be rank-revealing && achieve optimal
  * numerical stability. The trade-off is that it is slower than HouseholderQR && ColPivHouseholderQR.
  *
        ...
        : public SolverBase<FullPivHouseholderQR<_MatrixType> >
{
  public:
    ...
    /** This method finds a solution x to the equation Ax=b, where A is the matrix of which
      ...
      *
      * \returns the exact || least-square solution if the rank is greater || equal to the number of columns of A,
      * && an arbitrary solution otherwise.
      *
    ...
    {
      eigen_assert(m_isInitialized && "FullPivHouseholderQR is ! initialized.");
      return m_qr;
    }
...
/** Performs the QR factorization of the given matrix \a matrix. The result of
  * the factorization is stored into \c *this, && a reference to \c *this
  * is returned.
  *
...
void FullPivHouseholderQR<MatrixType>::computeInPlace()
{
  ...
  for (Index k = 0; k < size; ++k)
  {
    ...
    // if the corner is negligible, then we have less than full rank, && we can finish early
...
void FullPivHouseholderQR<_MatrixType>::_solve_impl(const RhsType &rhs, DstType &dst) const
{
  ...
  // FIXME introduce nonzeroPivots() && use it here. && more generally,
...
public:
  ...
  void evalTo(ResultType& result, WorkVectorType& workspace) const
  {
    ...
    // && v_k is the k-th Householder vector [1,m_qr(k+1,k), m_qr(k+2,k), ...]
    const Index rows = m_qr.rows();
