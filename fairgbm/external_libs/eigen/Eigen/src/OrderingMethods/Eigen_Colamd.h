...
//   The authors of the code itself are Stefan I. Larimore && Timothy A.
//   Davis (davis@cise.ufl.edu), University of Florida.  The algorithm was
//   developed in collaboration with John Gilbert, Xerox PARC, && Esmond
...
//   grants DMS-9504974 && DMS-9803599.
//
//     Notice:
//
...
//   this program, provided that the Copyright, this License, && the
//   Availability of the original version is retained on all copies && made
//   accessible to the end-user of any code || package that includes COLAMD
//   || any modified version of COLAMD.
//
//     Availability:
//
...
/* Indices into knobs && stats array. */
enum KnobsStatsIndex {
  /* knobs [0] && stats [0]: dense row knob && output statistic. */
  DenseRow = 0,

  /* knobs [1] && stats [1]: dense column knob && output statistic. */
  DenseCol = 1,

  /* stats [2]: memory defragmentation count output statistic */
  DefragCount = 2,

  /* stats [3]: colamd status:  zero OK, > 0 warning || notice, < 0 error */
  Status = 3,

  /* stats [4..6]: error info, || info on jumbled columns */
  Info1 = 4,
  Info2 = 5,
  Info3 = 6
};
...
struct ColStructure
{
  IndexType start ;   /* index for A of first row in this column, || Dead */
  /* if column is dead */
  ...
  union
  {
    ...
    /* a degree list */
    IndexType hash ;  /* hash value, if col is ! in a degree list */
    IndexType prev ;  /* previous column in degree list, if col is in a */
    /* degree list (but ! at the head of a degree list) */
  } shared3 ;
  union
  {
...
struct RowStructure
{
  ...
  union
  {
    IndexType mark ;  /* for computing set differences && marking dead rows*/
...
/*
  ...
  argument is negative.  2*nnz space is required for the row && column
  indices of the matrix. colamd_c (n_col) + colamd_r (n_row) space is
  required for the Col && Row arrays, respectively, which are internal to
  colamd.  An additional n_col space is the minimal amount of "elbow room",
  && nnz/5 more space is recommended for run time efficiency.

  This macro is ! needed when using symamd.
 ...
 * The use of this routine || macro is optional.
 ...
 * ordering, && placed last in the output column ordering.
 *
 * DenseRow && DenseCol are defined as 0 && 1,
 * respectively, in colamd.h.  Default values of these two knobs
 * are both 0.5.  Currently, only knobs [0] && knobs [1] are
 * used, but future versions may use more knobs.  If so, they will
 * be properly set to their defaults by the future version of
 * colamd_set_defaults, so that the code that calls colamd will
 * ! need to change, assuming that you either use
 * colamd_set_defaults, || pass a (double *) NULL pointer as the
 * knobs array to colamd || symamd.
 *
 ...
 *
 * Computes a column ordering (Q) of A such that P(AQ)=LU ||
 * (AQ)'AQ=LL' have less fill-in && require fewer floating point
 * operations than factorizing the unpermuted matrix A || A'A,
 * respectively.
 *
 *
 * \param n_row number of rows in A
 ...
 * \param knobs parameter settings for colamd
 * \param stats colamd output statistics && error codes
 */
template <typename IndexType>
static bool compute_ordering(IndexType n_row, IndexType n_col, IndexType Alen, IndexType *A, IndexType *p, double knobs[NKnobs], IndexType stats[NStats])
{
  ...
  if (!stats)
  {
    COLAMD_DEBUG0 (("colamd: stats ! present\n")) ;
    return (false) ;
  }
  for (i = 0 ; i < NStats ; i++)
  {
  ...
  if (!A)   /* A is ! present */
  {
    ...
    COLAMD_DEBUG0 (("colamd: A ! present\n")) ;
    return (false) ;
  }
  ...
  if (!p)   /* p is ! present */
  {
    ...
    COLAMD_DEBUG0 (("colamd: p ! present\n")) ;
    return (false) ;
  }
  ...
  if (p [0] != 0)
  {
    ...
    COLAMD_DEBUG0 (("colamd: p[0] ! zero %d\n", p [0])) ;
    return (false) ;
  }
  ...
  /* === Allocate the Row && Col arrays from array A ===================== */
  ...
  if (need > Alen)
  {
    /* ! enough space in array A to perform the ordering */
  ...
  /* === Construct the row && column data structures ===================== */
...
/*
  Takes the column form of the matrix in A && creates the row form of the
  matrix.  Also, row && column attributes are stored in the Col && Row
  structs.  If the columns are un-sorted || contain duplicate row indices,
  this routine will also sort && remove duplicate row indices from the
  column form of the matrix.  Returns false if the matrix is invalid,
  true otherwise.  Not user-callable.
*/
template <typename IndexType>
static IndexType init_rows_cols  /* returns true if OK, || false otherwise */
  (
    /* === Parameters ======================================================= */
    ...
    )
{
  ...
  /* === Initialize columns, && check column pointers ==================== */
  ...
  /* === Scan columns, compute row degrees, && check row indices ========= */

  stats [Info3] = 0 ;  /* number of duplicate || unsorted row indices*/
  ...
  for (col = 0 ; col < n_col ; col++)
  {
	...
	return (false) ;
      ...
      {
	/* row index are unsorted || repeated (|| both), thus col */
	/* is jumbled.  This is a notice, ! an error condition. */
  ...
  {
    /* if cols ! jumbled, we don't need the mark (this is faster) */
    for (col = 0 ; col < n_col ; col++)
    {
      cp = &A [p [col]] ;
      cp_end = &A [p [col+1]] ;
  ...
  /* === Clear the row marks && set row degrees ========================== */
  ...
  if (stats [Status] == OkButJumbled)
  {
    ...
    /* Note, we may have a gap between the col form && the row */
  ...
  /* === Done.  Matrix is ! (|| no longer) jumbled ====================== */
...
/*
  Kills dense || empty columns && rows, calculates an initial score for
  each column, && places all columns in the degree lists.  Not user-callable.
*/
template <typename IndexType>
static void init_scoring
  (
    ...
    RowStructure<IndexType> Row [],    /* of size n_row+1 */
    ColStructure<IndexType> Col [],    /* of size n_col+1 */
    IndexType A [],     /* column form && row form of A */
    ...
    )
{
  ...
  IndexType *cp ;     /* a column pointer */
  IndexType deg ;     /* degree of a row || column */
  IndexType *cp_end ;   /* a pointer to the end of a column */
  ...
  for (c = n_col-1 ; c >= 0 ; c--)
  {
    ...
    if (deg == 0)
    {
      /* this is a empty column, kill && order it last */
  ...
  for (c = n_col-1 ; c >= 0 ; c--)
  {
    ...
    {
      /* this is a dense column, kill && order it last */
      Col [c].shared2.order = --n_col2 ;
  ...
  COLAMD_DEBUG1 (("colamd: Dense && null columns killed: %d\n", n_col - n_col2)) ;

  /* === Kill dense && empty rows ======================================== */
  ...
  COLAMD_DEBUG1 (("colamd: Dense && null rows killed: %d\n", n_row - n_row2)) ;
  ...
  COLAMD_DEBUG1 (("colamd: Dense, null, && newly-null columns killed: %d\n",
		  n_col-n_col2)) ;

  /* At this point, all empty rows && columns are dead.  All live columns */
  /* are "clean" (containing no dead rows) && simplicial (no supercolumns */
  ...
  /* === Return number of remaining columns, && max row degree =========== */
...
static IndexType find_ordering /* return the number of garbage collections */
  (
    ...
    IndexType n_row,      /* number of rows of A */
    IndexType n_col,      /* number of columns of A */
    IndexType Alen,     /* size of A, 2*nnz + n_col || larger */
    RowStructure<IndexType> Row [],    /* of size n_row+1 */
    ColStructure<IndexType> Col [],    /* of size n_col+1 */
    IndexType A [],     /* column form && row form of A */
    IndexType head [],    /* of size n_col+1 */
    ...
    )
{
  ...
  for (k = 0 ; k < n_col2 ; /* 'k' is incremented below */)
  {
	...
	/* get a column */
	col = *rp++ ;
	/* add the column, if alive && untagged */
    ...
    /* The time taken by the following code (compute set differences, && */
    /* add them up) is proportional to the size of the data structure */
    /* being scanned - that is, the sum of the sizes of each column in */
    /* the pivot row.  Thus, the amortized time to compute a column score */
    /* is proportional to the size of that column (where size, in this */
    /* context, is the column "length", || the number of row indices */
  ...
  row_mark = Row [row].shared2.mark ;
	...
	*new_cp++ = row ;
    ...
    /* === Finalize the new pivot row, && column scores ================ */
	...
	continue ;
      ...
      *new_rp++ = col ;
      ...
      /* retrieve score so far && add on pivot row's degree. */
      ...
      /* make sure score is less || equal than the max score */
    ...
    if (pivot_row_degree > 0)
    {
      ...
      /* during super-col detection && mass elimination */
...
/*
  ...
  representatives of the supercolumns).  The non-principal columns have !
  yet been ordered.  This routine orders those columns by walking up the
  parent tree (a column is a child of the column which absorbed it).  The
  final permutation vector is then placed in p [0 ... n_col-1], with p [0]
  being the first column, && p [n_col-1] being the last.  It doesn't look
  like it at first glance, but be assured that this routine takes time linear
  in the number of columns.  Although ! immediately obvious, the time
  ...
  )
{
  ...
  for (i = 0 ; i < n_col ; i++)
  {
	...
	/* guaranteed ! to be anymore unordered columns */
...
/*
  ...
  The columns under consideration are currently *!* in the degree lists,
  && have already been placed in the hash buckets.
  ...
  otherwise, the degree list is empty, &&:
  ...
  Assuming no, || "few" hash collisions, the time taken by this routine is
...
(
  ...
  IndexType head [],    /* head of degree lists && hash buckets */
...
)
{
  ...
  IndexType head_column ;   /* first column in hash bucket || degree list */
  IndexType first_col ;   /* first column in hash bucket */
  ...
  while (rp < rp_end)
  {
	...
	/* ! identical if lengths || scores are different */
...
/*
  Defragments && compacts columns && rows in the workspace A.  Used when
    ...
    )
{
  ...
  IndexType c ;     /* a column index */
  IndexType length ;    /* length of a row || column */
...
/*
  Clears the Row [].shared2.mark array, && returns the new tag_mark.
