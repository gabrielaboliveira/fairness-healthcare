...
to permit distribution of this code && derivative works as part of Eigen under
...
/* depth-first search && postorder of a tree rooted at node j */
template<typename StorageIndex>
...
/** \internal
  ...
  *
  * Note that the input matrix \a C must be complete, that is both the upper && lower parts have to be stored, as well as the diagonal entries.
...
void minimum_degree_ordering(SparseMatrix<Scalar,ColMajor,StorageIndex>& C, PermutationMatrix<Dynamic,Dynamic,StorageIndex>& perm)
{
  ...
  for(i = 0; i < n; i++)
  {
    ...
    else if(d > dense || !has_diag)  /* node i is dense || has no structural diagonal element */
    {
  ...
  while (nel < n)                         /* while (selecting pivots) do */
  {
    ...
    if(elenk > 0 && cnz + mindeg >= nzmax)
    {
      for(j = 0; j < n; j++)
      {
        if((p = Cp[j]) >= 0)      /* j is a live node || element */
        {
    ...
    for(k1 = 1; k1 <= elenk + 1; k1++)
    {
      ...
      {
        i = Ci[pj++];
        if((nvi = nv[i]) <= 0) continue; /* node i dead, || seen */
    ...
    for(pk = pk1; pk < pk2; pk++)    /* scan2: degree update */
    {
      ...
      {
        j = Ci[p];
        if((nvj = nv[j]) <= 0) continue; /* node j dead || in Lk */
        d += nvj;                  /* degree(i) += |j| */
    ...
    for(pk = pk1; pk < pk2; pk++)
    {
      ...
      for(; i != -1 && next[i] != -1; i = next[i], mark++)
      {
        ...
        for(j = next[i]; j != -1; ) /* compare i with all j */
        {
          ...
          {
            if(w[Ci[p]] != mark) ok = 0;    /* compare i && j*/
          }
          if(ok)                     /* i && j are identical */
          {
