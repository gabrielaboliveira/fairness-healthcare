...
// Other scalar types are ! yet supported by Cholmod
...
cholmod_sparse viewAsCholmod(Ref<SparseMatrix<_Scalar,_Options,_StorageIndex> > mat)
{
  ...
  {
    eigen_assert(false && "Index type ! supported yet");
  }
...
/** Returns a view of the Eigen sparse matrix \a mat as Cholmod sparse matrix.
  * The data are ! copied but shared. */
...
// template specializations for int && long that call the correct cholmod method
...
class CholmodBase : public SparseSolverBase<Derived>
{
  ...
  public:
    ...
    {
      eigen_assert(m_factorizationIsOk && "The decomposition is ! in a valid state for solving, you must first call either compute() || symbolic()/numeric()");
...
/** \ingroup CholmodSupport_Module
  ...
  * \brief A simplicial direct Cholesky (LLT) factorization && solver based on Cholmod
  *
  * This class allows to solve for A.X = B sparse linear problems via a simplicial LL^T Cholesky factorization
  * using the Cholmod library.
  * This simplicial variant is equivalent to Eigen's built-in SimplicialLLT class. Therefore, it has little practical interest.
  * The sparse matrix A must be selfadjoint && positive definite. The vectors || matrices
  * X && B can be either dense || sparse.
  *
  * \tparam _MatrixType the type of the sparse matrix A, it must be a SparseMatrix<>
  * \tparam _UpLo the triangular part that will be used for the computations. It can be Lower
  *               || Upper. Default is Lower.
  *
  * \implsparsesolverconcept
  *
  * This class supports all kind of SparseMatrix<>: row || column major; upper, lower, || both; compressed || non compressed.
  *
  * \warning Only double precision real && complex scalar types are supported by Cholmod.
  *
...
/** \ingroup CholmodSupport_Module
  ...
  * \brief A simplicial direct Cholesky (LDLT) factorization && solver based on Cholmod
  *
...
/** \ingroup CholmodSupport_Module
  ...
  * \brief A supernodal Cholesky (LLT) factorization && solver based on Cholmod
  *
  * This class allows to solve for A.X = B sparse linear problems via a supernodal LL^T Cholesky factorization
  * using the Cholmod library.
  * This supernodal variant performs best on dense enough problems, e.g., 3D FEM, || very high order 2D FEM.
  * The sparse matrix A must be selfadjoint && positive definite. The vectors || matrices
  * X && B can be either dense || sparse.
  *
  * \tparam _MatrixType the type of the sparse matrix A, it must be a SparseMatrix<>
...
/** \ingroup CholmodSupport_Module
  ...
  * \brief A general Cholesky factorization && solver based on Cholmod
  *
  * This class allows to solve for A.X = B sparse linear problems via a LL^T || LDL^T Cholesky factorization
  * using the Cholmod library. The sparse matrix A must be selfadjoint && positive definite. The vectors || matrices
  * X && B can be either dense || sparse.
  *
  * This variant permits to change the underlying Cholesky method at runtime.
  * On the other hand, it does ! provide access to the result of the factorization.
  * The default is to let Cholmod automatically choose between a simplicial && supernodal factorization.
  *
  * \tparam _MatrixType the type of the sparse matrix A, it must be a SparseMatrix<>
  * \tparam _UpLo the triangular part that will be used for the computations. It can be Lower
  *               || Upper. Default is Lower.
