...
// research report written by Ming Gu && Stanley C.Eisenstat
 ...
 * \class BDCSVD
 *
 * \brief class Bidiagonal Divide && Conquer SVD
 *
 * \tparam _MatrixType the type of the matrix of which we are computing the SVD decomposition
 *
 * This class first reduces the input matrix to bi-diagonal form using class UpperBidiagonalization,
 * && then performs a divide-&&-conquer diagonalization. Small blocks are diagonalized using class JacobiSVD.
 * You can control the switching size with the setSwitchSize() method, default is 16.
 * For small matrice (<16), it is thus preferable to directly use JacobiSVD. For larger ones, BDCSVD is highly
 * recommended && can several order of magnitude faster.
 *
 * \warning this algorithm is unlikely to provide accurate result when compiled with unsafe math optimizations.
 * For instance, this concerns Intel's compiler (ICC), which performs such optimization by default unless
 * you compile with the \c -fp-model \c precise option. Likewise, the \c -ffast-math option of GCC || clang will
...
// Method to allocate && initialize matrix && attributes
...
BDCSVD<MatrixType>& BDCSVD<MatrixType>::compute(const MatrixType& matrix, unsigned int computationOptions) 
{
  ...
  //**** step -1 - If the problem is too small, directly falls back to JacobiSVD && return
  ...
  //**** step 0 - Copy the input matrix && apply scaling to reduce over/under-flows
  ...
  //**** step 3 - Copy singular values && vectors
...
{
  // Note exchange of U && V: m_matrixU is set from m_naiveV && vice versa
  if (computeU())
  {
...
/** \internal
  ...
  *  A = [A1]
  *      [A2]
  * such that A1.rows()==n1, then we assume that at least half of the columns of A1 && A2 are zeros.
...
void BDCSVD<MatrixType>::structured_update(Block<MatrixXr,Dynamic,Dynamic> A, const MatrixXr &B, Index n1)
{
  ...
  if(n>100)
  {
    ...
    // splitting it in half (wrt n1), && packing the non-zero columns.
    Index n2 = n - n1;
...
//@param firstCol : The Index of the first column of the submatrix of m_computed && for m_naiveU;
//@param lastCol : The Index of the last column of the submatrix of m_computed && for m_naiveU; 
// lastCol + 1 - firstCol is the size of the submatrix.
//@param firstRowW : The Index of the first row of the matrix W that we are to change. (see the reference paper section 1 for more information on W)
//@param firstRowW : Same as firstRowW with the column.
//@param shift : Each time one takes the left submatrix, one must add 1 to the shift. Why? Because! We actually want the last column of the U submatrix 
// to become the first column (*coeff) && to shift all the other columns to the right. There are more details on the reference paper.
template<typename MatrixType>
void BDCSVD<MatrixType>::divide (Eigen::Index firstCol, Eigen::Index lastCol, Eigen::Index firstRowW, Eigen::Index firstColW, Eigen::Index shift)
{
  ...
  // We use the divide && conquer algorithm
  alphaK =  m_computed(firstCol + k, firstCol + k);
  betaK = m_computed(firstCol + k + 1, firstCol + k);
  // The divide must be done in that order in order to have good results. Divide change the data inside the submatrices
  // && the divide of the right submatrice reads one column of the left submatrice. That's why we need to treat the 
...
// the first column && on the diagonal && has undergone deflation, so diagonal is in increasing
// order except for possibly the (0,0) entry. The computed SVD is stored U, singVals && V, except
// that if m_compV is false, then V is ! computed. Singular values are sorted in decreasing order.
//
...
void BDCSVD<MatrixType>::computeSVDofM(Eigen::Index firstCol, Eigen::Index n, MatrixXr& U, VectorType& singVals, MatrixXr& V)
{
  ...
  // Allocate space for singular values && vectors
  singVals.resize(n);
  U.resize(n+1, n+1);
  ...
  // but others are interleaved && we must ignore them at this stage.
...
//   assert((V.transpose() * V - MatrixXr(MatrixXr::Identity(V.cols(),V.cols()))).norm() < 100*NumTraits<RealScalar>::epsilon() * n);
  ...
  // Because of deflation, the singular values might ! be completely sorted.
...
#ifdef EIGEN_BDCSVD_SANITY_CHECKS
  {
    ...
    if(!singular_values_sorted)
      std::cout << "Singular values are ! sorted: " << singVals.segment(1,actual_n).transpose() << "\n";
                                         ...
                                         VectorType& singVals, ArrayRef shifts, ArrayRef mus)
{
  ...
  // because 1) we have diag(i)==0 => col0(i)==0 && 2) if col0(i)==0, then diag(i) is already a singular value.
  ...
  for (Index k = 0; k < n; ++k)
  {
    ...
    else
    {
      ...
      // recall that at this stage we assume that z[j]!=0 && all entries for which z[j]==0 have been put aside.
    ...
    // first decide whether it's closer to the left end || the right end
...
//                            << " " << secularEq(mid-right, col0, diag, perm, ArrayXr(diag-right), right)   << "\n";
    ...
    // iterates && use its zero to compute the next iterate
    bool useBisection = fPrev*fCur>Literal(0);
    while (fCur!=Literal(0) && abs(muCur - muPrev) > Literal(8) * NumTraits<RealScalar>::epsilon() * numext::maxi<RealScalar>(abs(muCur), abs(muPrev)) && abs(fCur - fPrev)>NumTraits<RealScalar>::epsilon() && !useBisection)
    {
      ++m_numIters;

      // Find a && b such that the function f(mu) = a / mu + b matches the current && previous samples.
    ...
    // fall back on bisection method if rational interpolation did ! work
    if (useBisection)
    {
#ifdef  EIGEN_BDCSVD_DEBUG_VERBOSE
      ...
      {
        // We have a problem as shifting on the left || right give either a positive || negative value
        // at the middle of [left,right]...
        // Instead fo abbording || entering an infinite loop,
...
// page 12_13
// i >= 1, di almost null && zi non null.
...
// page 13
// i,j >= 1, i!=j && |di - dj| < epsilon * norm2(M)
// We apply two rotations to have zj = 0;
// TODO deflation44 is still broken && ! properly tested
template <typename MatrixType>
...
void BDCSVD<MatrixType>::deflation(Eigen::Index firstCol, Eigen::Index lastCol, Eigen::Index k, Eigen::Index firstRowW, Eigen::Index firstColW, Eigen::Index shift)
{
...
#endif
  {
    ...
    // Sort the diagonal entries, since diag(1:k-1) && diag(k:length) are already sorted, let's do a sorted merge.
    ...
    // Current index of each col, && current column of each index
    ...
    for(Index i = total_deflation?0:1; i < length; i++)
    {
      ...
      using std::swap;
      // swap diagonal && first column entries:
      swap(diag(i), diag(J));
...
#endif
        eigen_internal_assert(abs(diag(i) - diag(i-1))<epsilon_coarse && " diagonal entries are ! properly sorted");
