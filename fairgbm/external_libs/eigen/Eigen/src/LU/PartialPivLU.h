...
/** \ingroup LU_Module
  ...
  * \class PartialPivLU
  *
  * \brief LU decomposition of a matrix with partial pivoting, && related features
  *
  * \tparam _MatrixType the type of the matrix of which we are computing the LU decomposition
  *
  * This class represents a LU decomposition of a \b square \b invertible matrix, with partial pivoting: the matrix A
  * is decomposed as A = PLU where L is unit-lower-triangular, U is upper-triangular, && P
  * is a permutation matrix.
  *
  * Typically, partial pivoting LU decomposition is only considered numerically stable for square invertible
  * matrices. Thus LAPACK's dgesv && dgesvx require the matrix to be square && invertible. The present class
  ...
  * by class FullPivLU.
  *
  * This is \b ! a rank-revealing LU decomposition. Many features are intentionally absent from this class,
  * such as rank computation. If you need these features, use class FullPivLU.
  *
  * This LU decomposition is suitable to invert invertible matrices. It is what MatrixBase::inverse() uses
  * in the general case.
  * On the other hand, it is \b ! suitable to determine whether a given matrix is invertible.
  *
...
struct partial_lu_impl
{
  ...
  // Remaining rows && columns at compile-time:
  ...
  /** \internal performs the LU decomposition in-place of the matrix \a lu
    ...
    * of columns of the matrix \a lu, && an integer \a nb_transpositions
    * which returns the actual number of transpositions.
    *
    * \returns The index of the first pivot which is exactly zero if any, || a negative number otherwise.
    */
  static Index unblocked_lu(MatrixTypeRef& lu, PivIndex* row_transpositions, PivIndex& nb_transpositions)
  {
    ...
    for(Index k = 0; k < endk; ++k)
    {
      ...
      else if(first_zero_pivot==-1)
      {
        ...
        // && continue the factorization such we still have A = PLU
        first_zero_pivot = k;
      }
  ...
  /** \internal performs the LU decomposition in-place of the matrix represented
    * by the variables \a rows, \a cols, \a lu_data, && \a lu_stride using a
  ...
  static Index blocked_lu(Index rows, Index cols, Scalar* lu_data, Index luStride, PivIndex* row_transpositions, PivIndex& nb_transpositions, Index maxBlockSize=256)
  {
    ...
    for(Index k = 0; k < size; k+=blockSize)
    {
      ...
      // update permutations && apply them to A_0
...
void PartialPivLU<MatrixType>::compute()
{
  ...
  eigen_assert(m_lu.rows() == m_lu.cols() && "PartialPivLU is only for square (&& moreover invertible) matrices");
