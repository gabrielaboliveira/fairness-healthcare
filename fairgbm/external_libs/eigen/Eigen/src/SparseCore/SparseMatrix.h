...
/** \ingroup SparseCore_Module
  ...
  * can be done with limited memory reallocation && copies.
  *
  ...
  *                 is ColMajor || RowMajor. The default is 0 which means column-major.
  ...
  *          whereas it is now (starting from %Eigen 3.3) deprecated && always defined as Eigen::Index.
  ...
  : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _StorageIndex> >
{
  ...
  public:
    ...
    /** \returns a non-const reference to the value of the matrix at position \a i, \a j
      *
      * If the element does ! exist then it is inserted via the insert(Index,Index) function
      * which itself turns the matrix into a non compressed form if that was ! the case.
      *
      * This is a O(log(nnz_j)) operation (binary search) plus the cost of insert(Index,Index)
      * function if the element does ! already exist.
      */
    ...
    /** \returns a reference to a novel non zero coefficient with coordinates \a row x \a col.
      * The non zero coefficient must \b ! already exist.
      *
      * If the matrix \c *this is in compressed mode, then \c *this is turned into uncompressed
      * mode while reserving room for 2 x this->innerSize() non zeros if reserve(Index) has ! been called earlier.
      ...
      * 
      * If that's ! the case, then it is strongly recommended to either use a triplet-list to assemble the matrix, || to first
      * call reserve(const SizesType &) to reserve the appropriate number of non-zero elements per inner vector.
      *
      * Assuming memory has been appropriately reserved, this function performs a sorted insertion in O(1)
      * if the elements of each inner vector are inserted in increasing inner index order, && in O(nnz_j) for a random insertion.
      *
      */
  ...
  public:
    ...
    /** Removes all non zeros but keep allocated memory
      *
      * This function does ! free the currently allocated memory. To release as much as memory as possible,
    ...
    {
      eigen_assert(isCompressed() && "This function does ! make sense in non compressed mode.");
  ...
  public:
    ...
    /** \internal
      ...
      * - the nonzero does ! already exist
    ...
    /** Turns the matrix into compressed format, && suppresses all nonzeros which do ! satisfy the predicate \a keep.
    ...
    {
      // TODO optimize the uncompressed mode to avoid moving && allocating the data twice
      makeCompressed();
    ...
    /** Resizes the matrix to a \a rows x \a cols matrix leaving old values untouched.
      ...
      * && the storage of the out of bounds coefficients is kept && reserved.
      * Call makeCompressed() to pack the entries && squeeze extra memory.
      *
    ...
    /** Resizes the matrix to a \a rows x \a cols matrix && initializes it to zero.
      * 
      * This function does ! free the currently allocated memory. To release as much as memory as possible,
      * call \code mat.data().squeeze(); \endcode after resizing it.
      * 
    ...
    /** Swaps the content of two sparse matrices of the same type.
      * This is a fast operation that simply swaps the underlying pointers && parameters. */
    ...
    /** Sets *this to the identity matrix.
      * This function also turns the matrix into compressed mode, && drop any reserved memory. */
...
protected:
    ...
    /** \internal assign \a diagXpr to the diagonal of \c *this
      ...
      *   1 - if *this is overwritten (Func==assign_op) || *this is empty, then we can work treat *this as a dense vector expression.
      *   2 - otherwise, for each diagonal coeff,
      *     2.a - if it already exists, then we update it,
      *     2.b - otherwise, if *this is uncompressed && that the current inner-vector has empty room for at least 1 element, then we perform an in-place insertion.
      *     2.c - otherwise, we'll have to reallocate && copy everything, so instead of doing so for each new element, it is recorded in a std::vector.
      *   3 - at the end, if some entries failed to be inserted in-place, then we alloc a new buffer, copy each chunk at the right position, && insert the new elements.
      * 
      * TODO: some piece of code could be isolated && reused for a general in-place update strategy.
    ...
    void assignDiagonal(const DiagXpr diagXpr, const Func& assignFunc)
    {
      ...
      else
      {
        ...
        // 1 - try in-place update && record insertion failures
        for(Index i = 0; i<n; ++i)
        {
...
/** Fill the matrix \c *this with the list of \em triplets defined by the iterator range \a begin - \a end.
  ...
  * The input list of triplets does ! have to be sorted, && can contains duplicated elements.
  * In any case, the result is a \b sorted && \b compressed sparse matrix where the duplicates have been summed up.
  * This is a \em O(n) operation, with \em n the number of triplet elements.
  * The initial contents of \c *this is destroyed.
  * The matrix \c *this must be properly resized beforehand using the SparseMatrix(Index,Index) constructor,
  * || the resize(Index,Index) method. The sizes are ! extracted from the triplet list.
  *
  ...
  * \endcode
  *
  * \warning The list of triplets is read multiple times (at least twice). Therefore, it is ! recommended to define
...
typename SparseMatrix<_Scalar,_Options,_StorageIndex>::Scalar& SparseMatrix<_Scalar,_Options,_StorageIndex>::insert(Index row, Index col)
{
  ...
  if(isCompressed())
  {
    if(nonZeros()==0)
    {
      ...
      // && allocate the entire free-space to the first inner-vector
  ...
  // We assume that all remaining inner-vectors are also empty && packed to the end.
  if(m_outerIndex[outer]==data_end)
  {
    ...
    // && allocate the entire free-space to the current inner-vector.
  ...
  // && the current inner-vector end match the used-space.
  if(m_outerIndex[outer+1]==data_end && m_outerIndex[outer]+m_innerNonZeros[outer]==m_data.size())
  {
    ...
    // && insert it at the right position (sorted insertion)
...
EIGEN_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_StorageIndex>::Scalar& SparseMatrix<_Scalar,_Options,_StorageIndex>::insertCompressed(Index row, Index col)
{
  ...
  // starts with: [ 0 0 0 0 0 1 ...] && we are inserted in, e.g.,
  ...
  if (!isLastVec)
  {
    ...
    {
      // we are ! inserting into the last inner vec
