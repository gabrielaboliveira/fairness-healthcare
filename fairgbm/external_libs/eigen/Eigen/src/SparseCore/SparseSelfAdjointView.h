...
/** \ingroup SparseCore_Module
  ...
  * \param Mode can be either \c #Lower || \c #Upper
  *
  * This class is an expression of a sefladjoint matrix from a triangular part of a matrix
  * with given dense storage of the coefficients. It is the return type of MatrixBase::selfadjointView()
  * && most of the time this is the only way that it is used.
  *
  ...
  : public EigenBase<SparseSelfAdjointView<MatrixType,_Mode> >
{
  public:
    ...
    /** \returns an expression of the matrix product between a sparse self-adjoint matrix \c *this && a sparse matrix \a rhs.
      *
    ...
    /** \returns an expression of the matrix product between a sparse matrix \a lhs && a sparse self-adjoint matrix \a rhs.
      *
      * Note that there is no algorithmic advantage of performing such a product compared to a general sparse-sparse matrix product.
      * Indeed, the SparseSelfadjointView operand is first copied into a temporary SparseMatrix before computing the product.
      */
    ...
    /** Perform a symmetric rank K update of the selfadjoint matrix \c *this:
      * \f$ this = this + \alpha ( u u^* ) \f$ where \a u is a vector || matrix.
      *
    ...
    void resize(Index rows, Index cols)
    {
      ...
      eigen_assert(rows == this->rows() && cols == this->cols()
                && "SparseSelfadjointView::resize() does ! actually allow to resize.");
    }
...
struct Assignment<DstXprType, SrcXprType, Functor, SparseSelfAdjoint2Sparse>
{
  ...
  // FIXME: the handling of += && -= in sparse matrices should be cleanup so that next two overloads could be reduced to:
...
// TODO: maybe the copy could be handled by generic_product_impl so that these overloads would ! be needed anymore
...
/***************************************************************************
* Implementation of symmetric copies && permutations
***************************************************************************/
namespace internal {
