...
// * Finds the gradient in x && y-axes
// * Three step process
...
// This example shows how to apply sobel filter on images || on camera frames
// with OpenCV, transfer the frames to the GPU, && apply a sobel filter
// written in OpenCL
int main(int argc, char *argv[])
{
    ...
    desc.add_options()
            ...
            ("camera", po::value<int>()->default_value(-1),
                                 "if ! default camera, specify a camera id")
    ...
    //OpenCL variables
    // Get default device && setup context
    ...
    // create fliter kernel && set arguments
