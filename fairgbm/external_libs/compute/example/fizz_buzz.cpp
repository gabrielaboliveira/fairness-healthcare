...
const char fizz_buzz_source[] = BOOST_COMPUTE_STRINGIZE_SOURCE(
    ...
    // && writes it to the string_lengths array. these will then be passed
    ...
    __kernel void fizz_buzz_copy_strings(__global const uint *offsets, __global char *buffer)
    {
        ...
        if((n % 5 == 0) && (n % 3 == 0)){
            ...
            // convert number to string && write it to the output
...
int main()
{
    ...
    // create a vector for the output string && computing offsets
    ...
    // copy the string to the host && print it to stdout
    std::string str;
