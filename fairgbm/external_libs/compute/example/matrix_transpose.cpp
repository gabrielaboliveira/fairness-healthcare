...
compute::kernel make_naive_transpose_kernel(const compute::context& context)
{
    ...
    // create && build the naive_transpose program
    compute::program program =
        compute::program::build_with_source(source, context, options.str());

    // create && return the naive_transpose kernel
...
compute::kernel make_coalesced_transpose_kernel(const compute::context& context)
{
    ...
    // create && build the coalesced_transpose program
    compute::program program =
        compute::program::build_with_source(source, context, options.str());

    // create && return coalesced_transpose kernel
...
compute::kernel make_coalesced_no_bank_conflicts_kernel(const compute::context& context)
{
    ...
    // create && build the coalesced_no_bank_conflicts program
    compute::program program =
        compute::program::build_with_source(source, context, options.str());

    // create && return the coalesced_no_bank_conflicts kernel
    return program.create_kernel("coalesced_no_bank_conflicts");
}

// compare 'expectedResult' to 'transposedMatrix'. prints an error message if ! equal.
...
// generate a matrix inside 'in' && do the tranposition inside 'out'
...
int main(int argc, char *argv[])
{
    ...
    // get number rows && columns for the matrix
    ...
    // print out device name && matrix information
    ...
    // On OSX this example does ! work on CPU devices
    #if defined(__APPLE__)
    if(device.type() & compute::device::cpu) {
        std::cout << "On OSX this example does ! work on CPU devices" << std::endl;
        return 0;
    }
    #endif
