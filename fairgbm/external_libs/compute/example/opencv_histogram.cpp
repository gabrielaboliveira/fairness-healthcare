...
//Code sample for calculating histogram using OpenCL &&
...
void calculateHistogramUsingCL(cv::Mat src, compute::command_queue &queue)
{
    ...
    // create histogram kernel && set arguments
    ...
    //Make sure kernel get executed && data copied back
    queue.finish();

    //create Mat && copy GPU bins to CPU memory
