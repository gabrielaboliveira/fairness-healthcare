...
// directly into boost.compute functions && use them with the
// transform() algorithm.
int main()
{
    ...
    // create input values && copy them to the device
    using compute::uint_;
    uint_ data[] = { 0x00, 0x01, 0x11, 0xFF };
    compute::vector<uint_> input(data, data + 4, queue);

    // function returning the number of bits set (aka population count ||
    ...
    // copy results back to the host && print them out
