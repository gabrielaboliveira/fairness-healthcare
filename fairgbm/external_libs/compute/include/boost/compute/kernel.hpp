...
public:
    ...
    /// optional if \p device is ! 2.1 device, || is ! 2.0 device with support
                                          ...
                                          const size_t input_size, const void * input) const
    {
        ...
        // Only CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE && CL_KERNEL_SUB_GROUP_COUNT_FOR_NDRANGE
    ...
    /// Returns sub-group information for the kernel with \p device. Returns a null
    /// optional if cl_khr_subgroups extension is ! supported by \p device.
    ///
    ...
    /// such as buffer && image2d. This allows for them to be passed directly
    ...
    /// vector<T> && array<T, N>. This allows for them to be passed directly
