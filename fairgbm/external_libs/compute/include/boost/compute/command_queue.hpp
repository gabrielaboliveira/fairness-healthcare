...
/// && from a compute device as well as execute compute kernels.
///
/// Command queues are created for a compute device within a compute
/// context.
///
/// For example, to create a context && command queue for the default device
...
///
/// // set up a compute context && command queue
...
public:
                             ...
                             const wait_list &events = wait_list())
    {
        ...
        BOOST_ASSERT_MSG(src_image.format() == dst_image.format(),
                         "Source && destination image formats must match.");
    ...
    /// Enqueues a marker in the queue && returns an event that can be
    ...
    #if defined(BOOST_COMPUTE_CL_VERSION_1_2) || defined(BOOST_COMPUTE_DOXYGEN_INVOKED)
    /// Enqueues a marker after \p events in the queue && returns an
    ...
    /// should be associated with. The pair \p svm_ptrs[i] && \p sizes[i] together define
    /// the starting address && number of bytes in a range to be migrated.
    ///
    ...
    /// should be associated with. The pair \p svm_ptr && \p size together define
    /// the starting address && number of bytes in a range to be migrated.
    ///
