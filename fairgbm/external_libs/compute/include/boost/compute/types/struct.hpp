...
///
/// For example, to adapt a 2D particle struct with position (x, y) &&
/// velocity (dx, dy):
/// \code
/// // c++ struct definition
/// struct Particle
...
/// \endcode
///
/// Due to differences in struct padding between the host compiler && the
...
#define BOOST_COMPUTE_ADAPT_STRUCT(type, name, members) \
    BOOST_STATIC_ASSERT_MSG( \
        ...
        "BOOST_COMPUTE_ADAPT_STRUCT() does ! support structs with internal padding." \
    ); \
