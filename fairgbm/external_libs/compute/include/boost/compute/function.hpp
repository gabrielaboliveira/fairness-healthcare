...
/// Creates a function object given its \p name && \p source.
///
...
inline std::vector<std::string> parse_argument_names(const char *arguments)
{
    BOOST_ASSERT_MSG(
        ...
        "Arguments should start && end with parentheses"
    );
...
// with the given signature, name, arguments, && source.
template<class Signature>
inline function<Signature>
...
/// Creates a function object with \p name && \p source.
///
...
///
/// The function declaration && signature are automatically created using
/// the \p return_type, \p name, && \p arguments macro parameters.
///
/// The source code for the function is interpreted as OpenCL C99 source code
/// which is stringified && passed to the OpenCL compiler when the function
/// is invoked.
///
