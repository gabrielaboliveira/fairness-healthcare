...
/// \class system
/// \brief Provides access to platforms && devices on the system.
///
/// The system class contains a set of static functions which provide access to
/// the OpenCL platforms && compute devices on the host system.
///
/// The default_device() convenience method automatically selects && returns
/// the "best" compute device for the system following a set of heuristics &&
...
public:
    ...
    ///
    /// The default device is selected based on a set of heuristics && can be
    ...
    ///        type of the compute device (e.g. "GPU" || "CPU")
