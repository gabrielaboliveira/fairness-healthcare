...
/// kernel executions && memory transfers). Event objects are returned by the
/// various \c enqueue_* methods of the command_queue class.
///
/// Events can be used to synchronize operations between the host && the
...
public:
    ...
    /// event object, the event status, && a pointer to arbitrary user data.
    ///
