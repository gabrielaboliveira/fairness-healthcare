...
/// increasing index && \p input_idx is ! used in the algorithm.
                                     ...
                                     command_queue &queue)
{
    ...
    k <<
        ...
        "while( idx < count ){\n" <<
            ...
            // Comparison between currently best element (acc) && next element
                                       ...
                                       command_queue &queue)
{
    ...
    // Getting information about used queue && device
    ...
    // get preferred work group size && preferred number
    // of work groups per compute unit
    size_t work_group_size = parameters->get(cache_key, "wgsize", 256);
    size_t work_groups_per_cu = parameters->get(cache_key, "wgpcu", 100);

    // calculate work group size && number of work groups
    work_group_size = (std::min)(max_work_group_size, work_group_size);
    ...
    // device buffors for extremum candidates && their indices
    ...
    // finding candidates for first extremum && their indices
    ...
    // zero-copy buffers for final result (value && index)
                                       ...
                                       command_queue &queue)
{
    ...
    // mapping candidates && their indices to host
