...
// For signed types we just negate the x && for unsigned types we
...
"#else\n"
     // write key && value if doing sort_by_key
                            ...
                            command_queue &queue)
{
    ...
    // sort by key && have to set up the values buffer
    bool sort_by_key = (values_first.get_buffer().get() != 0);

    // load (|| create) radix sort program
