...
/// be non-deterministic && vary in precision. Notably this affects the
/// \c plus<float>() function as floating-point addition is ! associative
/// && may produce slightly different results than a serial algorithm.
///
/// This algorithm supports both host && device iterators for the
/// result argument. This allows for values to be reduced && copied
/// to the host all with a single function call.
///
/// For example, to calculate the sum of the values in a device vector &&
