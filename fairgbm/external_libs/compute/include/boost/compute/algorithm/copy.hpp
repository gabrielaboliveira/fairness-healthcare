...
// meta-function returning true if copy() between InputIterator &&
...
// meta-function returning true if value_types of HostIterator &&
...
// host -> device (async)
// Type mismatch between InputIterator && OutputIterator value_types
template<class InputIterator, class OutputIterator>
inline future<OutputIterator>
dispatch_copy_async(InputIterator first,
                    InputIterator last,
                    ...
                    >::type* = 0)
{
    ...
    // map [first; last) to device && run copy kernel
              ...
              >::type* = 0)
{
    ...
    //     on host && then perform copy_to_device()
    ...
    // [direct_copy_threshold; inf) -> map [first; last) to device &&
    //     run copy kernel on device for copying & casting
    // At this point we are sure that count > 1 (first != last).

    // Perform async copy to device, wait for it to be finished &&
    // return the result.
                    ...
                    >::type* = 0)
{
    ...
    // update host memory asynchronously by maping && unmaping memory
    event map_event;
...
// device -> host
// Type mismatch between InputIterator && OutputIterator value_types
// OutputIterator is NOT a contiguous iterator || value_type of OutputIterator
// is a boolean type.
template<class InputIterator, class OutputIterator>
inline OutputIterator
dispatch_copy(InputIterator first,
              ...
              >::type* = 0)
{
    ...
    //     then copy (&& convert) to result using std::copy()
              ...
              >::type* = 0)
{
    ...
    //     temporary vector then copy (&& convert) to result using std::copy()
    else if(input_size_bytes < direct_copy_threshold) {
        std::vector<input_type> vector(count);
        copy_to_host(first, last, vector.begin(), queue, events);
        return std::copy(vector.begin(), vector.end(), result);
    ...
    //     device && run copy kernel on device for copying & casting
    // map host memory to device.

    // Perform async copy to host, wait for it to be finished &&
    // return the result.
    // At this point we are sure that count > 1 (first != last), so event
    // returned by dispatch_copy_async() must be valid.
    return dispatch_copy_async(first, last, result, queue, events).get();
...
/// transfer tasks && provides a standard interface to the following
/// OpenCL functions:
///
