                                  ...
                                  command_queue &queue)
{
    ...
    // find each unique element && mark it with a one
    transform(
...
/// Makes a copy of the range [first, last) && removes all consecutive
/// duplicate elements (determined by \p op) from the copy. If \p op is !
