...
// std::vector<>::iterator && float*. examples of non-contiguous iterators
// are std::set<>::iterator && std::insert_iterator<>.
//
// the implementation consists of two phases. the first checks that value_type
// for the iterator is ! void. this must be done as for many containers void
// is ! a valid value_type (ex. std::vector<void>::iterator is ! valid).
...
// iterators types && a default value of false.
template<class Iterator, class Enable = void>
