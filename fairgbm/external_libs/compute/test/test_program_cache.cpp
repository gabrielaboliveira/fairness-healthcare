...
{
    // create cache with capacity of four && insert four programs
    ...
    // check that "a" has been evicted && that "e" is now present
    BOOST_CHECK(cache.get("a") == boost::none);
    BOOST_CHECK(cache.get("b") != boost::none);
    BOOST_CHECK(cache.get("c") != boost::none);
    BOOST_CHECK(cache.get("d") != boost::none);
    ...
    // clear cache && ensure no program objects are found
    cache.clear();
    BOOST_CHECK(cache.get("a") == boost::none);
    BOOST_CHECK(cache.get("b") == boost::none);
    BOOST_CHECK(cache.get("c") == boost::none);
