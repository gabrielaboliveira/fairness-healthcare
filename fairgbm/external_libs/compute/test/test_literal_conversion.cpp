...
BOOST_AUTO_TEST_CASE(literal_conversion_float)
{
    ...
    for (size_t i = 0; i < values.size(); i++) {
        ...
        // Check that we got something in the literal, && that at least the first
        ...
        // && leaving the f; so check the "f", then slice it off for the extraction below:
...
BOOST_AUTO_TEST_CASE(literal_conversion_double)
{
    ...
    for (size_t i = 0; i < values.size(); i++) {
        ...
        // Stuff the literal into a stream then extract it, && make sure we get a numerically
