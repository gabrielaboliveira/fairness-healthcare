...
//! [create_image]
        ...
        // verify image has been created && format is correct
        ...
        if(e.error_code() == CL_IMAGE_FORMAT_NOT_SUPPORTED){
            // image format ! supported by device
            return;
        }
...
BOOST_AUTO_TEST_CASE(get_info)
{
    ...
    if(!compute::image2d::is_supported_format(format, context)){
        std::cerr << "skipping get_info test, image format ! supported" << std::endl;
        return;
    }
...
BOOST_AUTO_TEST_CASE(clone_image)
{
    ...
    if(!compute::image2d::is_supported_format(format, context)){
        std::cerr << "skipping clone_image test, image format ! supported" << std::endl;
        return;
    }
    ...
    // ensure original data && cloned data are the same
...
BOOST_AUTO_TEST_CASE(fill_image)
{
    ...
    if(!compute::image2d::is_supported_format(format, context)){
        std::cerr << "skipping fill_image test, image format ! supported" << std::endl;
        return;
    }
