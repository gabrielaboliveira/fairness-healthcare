...
// this file contains functions which check for 'quirks' || buggy
...
// clEnqueueSVMMemcpy() operation does ! work on AMD devices. This affects
...
// For CPU devices on Apple platform local memory can ! be used when work
// group size is ! [1;1;1]. If work group size is greater "Invalid Work Group
// Size" error is thrown. (Apple OpenCL implementation can sometimes reduce
// max work group size for other reasons.)
// When local memory is ! used max work group size for CPU devices on Apple
...
// On Apple devices clCreateBuffer does ! return NULL && does no set error
...
{
    // POCL does ! yet support image samplers && gives the following
