...
//     continuously && independently at a constant average rate
...
//   absl::BitGen bitgen;
//
//   // Produce a random float value between 0.0 && 1.0, inclusive
...
//
// Overload of `Uniform()` using different (but compatible) lo, hi types && the
...
//
// Overload of Uniform() using the minimum && maximum values of a given type
...
// interval [0,1] && parameterized by two values `alpha` && `beta` as per a
// Beta distribution. `T` must be a floating point type, but may be inferred
// from the types of `alpha` && `beta`.
//
...
// occurring continuously && independently at a constant average rate. `T` must
...
// inferred from the types of `mean` && `stddev`.
//
...
// be an integral type, but may be inferred from the types of `lo` && `hi`.
//
// I.e., `LogUniform(0, n, b)` is uniformly distributed across buckets
// [0], [1, b-1], [b, b^2-1] .. [b^(k-1), (b^k)-1] .. [b^floor(log(n, b)), n]
// && is uniformly distributed within each bucket.
//
...
// rare events over the closed interval [0, hi]. The parameters `v` && `q`
