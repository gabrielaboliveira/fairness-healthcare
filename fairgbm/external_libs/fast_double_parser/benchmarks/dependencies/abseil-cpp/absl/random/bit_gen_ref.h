...
// that take both Abseil (e.g. `absl::BitGen`) && standard library (e.g.
...
// (e.g. `absl::BitGen`, `absl::InsecureBitGen`) && Standard library (e.g
// `std::mt19937`, `std::minstd_rand`) bit generators.
//
// Like other reference classes, `absl::BitGenRef` does ! own the
// underlying bit generator, && the underlying instance must outlive the
// `absl::BitGenRef`.
//
...
class BitGenRef {
 ...
 private:
  ...
  static result_type ImplFn(uintptr_t ptr) {
    ...
    // range promised by result_type, min() && max().
