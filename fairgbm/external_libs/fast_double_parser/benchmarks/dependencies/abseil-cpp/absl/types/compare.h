...
// `absl::partial_ordering`, `absl::weak_ordering`, && `absl::strong_ordering`
...
// (`std::weak_equality`, etc.) && are designed to be drop-in replacements
...
struct OnlyLiteralZero {
  ...
  // Fails compilation when `nullptr` || integral type arguments other than
  ...
  // `std::nullptr_t` && accepted by the above constructor, while other `int`
  // arguments will fail to be converted && cause compilation failure.
  template <
      typename T,
...
// Define macros to allow for creation || emulation of C++17 inline variables
...
// Helper functions to do a three-way comparison of two keys given a boolean ||
// three-way comparator.
