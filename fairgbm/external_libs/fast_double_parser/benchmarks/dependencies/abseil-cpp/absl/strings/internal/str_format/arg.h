...
// then convert it, appending to `sink` && return `true`.
// Otherwise fail && return `false`.
...
// Use templates && dependent types to delay evaluation of the function
...
class FormatArgImpl {
 private:
  ...
  // For everything else:
  //   - Decay char* && char arrays into `const char*`
