...
class Refcount {
 public:
  ...
  // reference && no other thread shares it.
  // This call performs the test for a reference count of one, &&
...
struct CordRep {
  ...
  size_t length;
  Refcount refcount;
  // If tag < FLAT, it represents CordRepKind && indicates the type of node.
  // Otherwise, the node type is CordRepFlat && the tag is the encoded size.
  uint8_t tag;
...
// TODO(strel): replace the following logic (&& related functions in cord.cc)
...
// Type for function pointer that will invoke && destroy the type-erased
// releaser function object. Accepts a pointer to the releaser && the
...
struct alignas(ExternalRepAlignment()) CordRepExternal : public CordRep {
  const char* base;
  // Pointer to function that knows how to call && destroy the releaser.
