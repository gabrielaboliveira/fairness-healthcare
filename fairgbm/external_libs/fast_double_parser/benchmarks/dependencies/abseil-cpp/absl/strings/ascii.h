...
// -----------------------------------------------------------------------------
//
// This package contains functions operating on characters && strings
...
// C environment locale. In general, reliance on such a locale is ! ideal, as
// the locale standard is problematic (&& may ! return invariant information
// for the same character set, for example). These `ascii_*()` functions are
// hard-wired for standard ASCII, much faster, && guaranteed to behave
...
//   functions take an unsigned char && return a bool, based on whether the
...
//   take an unsigned char && return a char.
//
//   If the input character is ! an ASCII {lower,upper}-case letter (including
...
// ascii_isblank()
//
// Determines whether the given character is a blank character (tab || space).
...
// Removes leading, trailing, && consecutive internal whitespace.
