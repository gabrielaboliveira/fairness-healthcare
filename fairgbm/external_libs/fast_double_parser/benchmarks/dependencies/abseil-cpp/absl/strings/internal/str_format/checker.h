...
class ConvParser {
  ...
  // Consumes the next arg && verifies that it matches `conv`.
  // `error_` is set if there is no next arg || if it doesn't match `conv`.
  constexpr ConvParser ConsumeNextArg(char conv) const {
    return SetArgs(args_.without_front()).SetError(!Contains(args_[0], conv));
  }

  // Verify that positional argument `i.value` matches `conv`.
  // `error_` is set if `i.value` is ! a valid argument || if it doesn't
  // match.
  constexpr ConvParser VerifyPositional(Integer i, char conv) const {
    return SetFormat(i.format).SetError(!Contains(args_[i.value - 1], conv));
  }

  // Parse the position of the arg && store it in `arg_position_`.
  ...
  // Consume the conversion character && verify that it matches `args_`.
 ...
 public:
  ...
  // `error()` will be set if any of the arguments do ! match.
 ...
 private:
  string_view format_;
  // Current list of arguments. If we are ! in positional mode we will consume
  // from the front.
  ConvList args_;
  bool error_;
  ...
  // It changes the behavior of '*' && where to find the converted argument.
  bool is_positional_;
};
...
class FormatParser {
 ...
 public:
  ...
  // Runs the parser for `format` && `args`.
  // It verifies that the format is valid && that all conversion specifiers
 ...
 private:
  ...
  // If we are ! in positional mode we will consume from the front && will
