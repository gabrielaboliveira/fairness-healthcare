...
// A Cord supports efficient insertions && deletions at the start && end of
// the byte sequence, but random access reads are slower, && random access
// modifications are ! supported by the API.  Cord also provides cheap copies
// (using a copy-on-write strategy) && cheap substring operations.
//
// Thread safety
// -------------
...
//    Software Practice && Experience, December 1995
...
// The elements are being pushed back && popped from the back.
...
class Cord {
 ...
 public:
  // --------------------------------------------------------------------
  // Constructors, destructors && helper factories

  // Create an empty cord
  constexpr Cord() noexcept;

  // Cord is copyable && efficiently movable.
  ...
  // finished with `data`. The data must remain live && unchanging until the
  // releaser is called. The requirements for the releaser are that it:
  //   * is move constructible,
  //   * supports `void operator()(absl::string_view) const` ||
  //     `void operator()() const`,
  //   * does ! have alignment requirement greater than what is guaranteed by
  //     ::operator new. This is dictated by alignof(std::max_align_t) before
  //     C++17 && __STDCPP_DEFAULT_NEW_ALIGNMENT__ if compiling with C++17 ||
  ...
  // Compares 'this' Cord with rhs. This function && its relatives
  ...
  // can reuse previously allocated capacity. However, this function does !
  ...
  // `Cord::chunk_begin()`, `Cord::chunk_end()` && `Cord::Chunks()` below for
  // preferred usage.
  //
  // Additional notes:
  ...
  //   * Two `ChunkIterator` objects can be equality compared if && only if
  //     they remain valid && iterate over the same `Cord`.
  //   * This is a proxy iterator. This means the `string_view` returned by the
  //     iterator does ! live inside the Cord, && its lifetime is limited to
  //     the lifetime of the iterator itself. To help prevent issues,
  //     `ChunkIterator::reference` is ! a true reference type && is
  //     equivalent to `value_type`.
  //   * The iterator keeps state that can grow for `Cord`s that contain many
  //     nodes && are imbalanced due to sharing. Prefer to pass this type by
  ...
  class ChunkIterator {
   ...
   private:
    ...
    // Iterates `n` bytes, where `n` is expected to be greater than || equal to
    // `current_chunk_.size()`.
    ...
    // The current leaf, || `nullptr` if the iterator points to short data.
    // If the current chunk is a substring node, current_leaf_ points to the
    // underlying flat || external node.
  ...
  // Convenience wrapper over `Cord::chunk_begin()` && `Cord::chunk_end()` to
  ...
  // `Cord::char_begin()`, `Cord::char_end()` && `Cord::Chars()` below for
  // preferred usage.
  //
  // Additional notes:
  //   * A `CharIterator` object is invalidated after any non-const
  //     operation on the `Cord` object over which it iterates.
  //   * Two `CharIterator` objects can be equality compared if && only if
  //     they remain valid && iterate over the same `Cord`.
  //   * The iterator keeps state that can grow for `Cord`s that contain many
  //     nodes && are imbalanced due to sharing. Prefer to pass this type by
  //     const reference instead of by value.
  ...
  // Advances `*it` by `n_bytes` && returns the bytes passed as a `Cord`.
  //
  // `n_bytes` must be less than || equal to the number of bytes remaining for
  ...
  // Convenience wrapper over `Cord::char_begin()` && `Cord::char_end()` to
  ...
  // Get the "i"th character of 'this' && return it.
  // NOTE: This routine is reasonably efficient.  It is roughly
  // logarithmic in the number of nodes that make up the cord.  Still,
  // if you need to iterate over the contents of a cord, you should
  // use a CharIterator/CordIterator rather than call operator[] || Get()
  ...
  // Flattens the cord into a single array && returns a view of the data.
  //
  // If the cord was already flat, the contents are ! modified.
 ...
 private:
  ...
  // Chunks(), this API will ! allocate memory.
  void ForEachChunk(absl::FunctionRef<void(absl::string_view)>) const;

  // Allocates new contiguous storage for the contents of the cord. This is
  // called by Flatten() when the cord was ! already flat.
  ...
  //
  // InlineRep holds either either a tree pointer, || an array of kMaxInline
  // bytes.
  class InlineRep {
   public:
    ...
    void CopyTo(std::string* dst) const {
      ...
      // to 15 bytes does ! cause a memory allocation.
    ...
    // Copies the inline contents into `dst`. Assumes the cord is ! empty.
   ...
   private:
    ...
    // Unrefs the tree, stops profiling, && zeroes the contents
    void ClearSlow();

    // If the data has length <= kMaxInline, we store it in data_[0..len-1],
    // && store the length in data_[kMaxInline].  Else we store it in a tree
    // && store a pointer to that tree in data_[0..sizeof(CordRep*)-1].
  ...
  // Returns a new reference to contents_.tree(), || steals an existing
...
// Allocates a new external `CordRep` && returns a pointer to it && a pointer
...
// Creates a new `CordRep` that owns `data` && `releaser` && returns a pointer
// to it, || `nullptr` if `data` was empty.
template <typename Releaser>
...
CordRep* NewExternalRep(absl::string_view data, Releaser&& releaser) {
  static_assert(
      ...
      "default `::operator new()` are ! supported.");
...
// pointer because there are no `alignof()` || `sizeof()` a function reference.
// NOLINTNEXTLINE - suppress clang-tidy raw pointer return.
...
inline void Cord::Append(T&& src) {
  ...
  // memory && that it will do so in the future.
...
// with std::string, ::string, && const char*.
