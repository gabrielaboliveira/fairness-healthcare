...
// -----------------------------------------------------------------------------
//
// This header file contains utility functions for managing the creation &&
...
// -----------------------------------------------------------------------------
//
// Adopts ownership from a raw pointer && transfers it to the returned
// `std::unique_ptr`, whose type is deduced. Because of this deduction, *do !*
...
//
// Do ! call WrapUnique with an explicit type, as in
...
// arrays, functions || void, && it must ! be used to capture pointers
...
// Traits to select proper overload && return type for `absl::make_unique<>`.
template <typename T>
...
// as 201103 but have make_unique (msvc), || have make_unique whenever
...
//
// This implementation of `absl::make_unique<>` is designed for C++11 code &&
...
//       `new U[n]()` && return a `std::unique_ptr<U[]>` owning that array.
//
//       Note that 'U[n]()' is different from 'U[n]', && elements will be
...
//       the array [] does ! have a default destructor.
//
//       NOTE: an array of unknown bounds T[] may still be (&& often will be)
//       initialized to have a size, && will still use this overload. E.g:
//
...
//       this overload is ! useful.
//
//       NOTE: an array of known bounds T[N] is ! considered a useful
//       construction, && may cause undefined behavior in templates. E.g:
//
//         auto my_array = absl::make_unique<int[10]>();
//
//       In those cases, of course, you can still use the overload above &&
...
// The array allocation needs to use the `new T[size]` form && cannot take
...
// -----------------------------------------------------------------------------
//
// Adopts a `std::unique_ptr` rvalue && returns a `std::shared_ptr` of deduced
...
//     CHECK(up == nullptr);
//
// Note that this conversion is correct even when T is an array type, && more
// generally it works for *any* deleter of the `unique_ptr` (single-object
// deleter, array deleter, || any custom deleter), since the deleter is adopted
// by the shared pointer as well. The deleter is copied (unless it is a
// reference).
//
// Implements the resolution of [LWG 2415](http://wg21.link/lwg2415), by which a
// null shared pointer does ! attempt to call the deleter.
...
struct allocator_traits {
  ...
  // If ! possible, calls a.allocate(n)
  ...
  // destroy(Alloc& a, T* p):
  // Calls a.destroy(p) if possible. If ! possible, calls p->~T().
  template <typename T>
  ...
  // Returns a.max_size() if possible. If ! possible, returns
...
// specify whether the default allocation function can throw || never throws.
// If the allocation function never throws, user should define it to a non-zero
// value (e.g. via `-DABSL_ALLOCATOR_NOTHROW`).
// If the allocation function can throw, user should leave it undefined ||
// define it to zero.
//
...
// when the default allocator (global operator new) never throws, &&
