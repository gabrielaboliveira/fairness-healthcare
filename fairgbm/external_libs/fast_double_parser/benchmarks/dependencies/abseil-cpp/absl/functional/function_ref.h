...
// the reference type in this case avoids a copy && an allocation. Best
...
//  * It doesn't have a null || empty state.
//  * It never performs deep copies || allocations.
//  * It's much faster && cheaper to construct.
//  * It's trivially copyable && destructable.
//
// Generally, `absl::FunctionRef` should ! be used as a return value, data
// member, || to initialize a `std::function`. Such usages will often lead to
...
// would be used without making a copy. ForEach functions && other versions of
// the visitor pattern are a good example of when this class should be used.
//
// This class is trivial to copy && should be passed by value.
...
// as an argument type && should be preferred as an argument over a const
...
//
//   // Assuming that the function is ! stored || otherwise copied, it can be
//   // replaced by an `absl::FunctionRef`:
//   bool Visitor(absl::FunctionRef<void(my_proto&, absl::string_view)>
//                  callback);
//
// Note: the assignment operator within an `absl::FunctionRef` is intentionally
// deleted to prevent misuse; because the `absl::FunctionRef` does ! own the
// underlying type, assignment likely indicates misuse.
template <typename R, typename... Args>
class FunctionRef<R(Args...)> {
 private:
  // Used to disable constructors for objects that are ! compatible with the
 ...
 public:
  ...
  // To help prevent subtle lifetime bugs, FunctionRef is ! assignable.
