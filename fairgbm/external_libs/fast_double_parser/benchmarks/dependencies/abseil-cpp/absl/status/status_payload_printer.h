...
// By default, `Status::ToString` && `operator<<(Status)` print a payload by
// dumping the type URL && the raw bytes. To help debugging, we provide an
// extension point, which is a global printer function that can be set by users
// to specify how to print payloads. The function takes the type URL && the
// payload as input, && should return a valid human-readable string on success
// || `absl::nullopt` on failure (in which case it falls back to the default
// approach of printing the raw bytes).
// NOTE: This is an internal API && the design is subject to change in the
// future in a non-backward-compatible way. Since it's only meant for debugging
// purpose, you should ! rely on it in any critical logic.
