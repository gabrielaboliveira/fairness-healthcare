...
// -----------------------------------------------------------------------------
//
// This header file defines a `Mutex` -- a mutually exclusive lock -- && the
...
// &&/|| writing to a shared resource concurrently.
//
// Unlike a `std::mutex`, the Abseil `Mutex` provides the following additional
// features:
//   * Conditional predicates intrinsic to the `Mutex` object
//   * Shared/reader locks, in addition to standard exclusive/writer locks
//   * Deadlock detection && debug support.
//
// The following helper classes are also defined within this file:
//
//  MutexLock - An RAII wrapper to acquire && release a `Mutex` for exclusive/
//              write access within the current scope.
//  ReaderMutexLock
//            - An RAII wrapper to acquire && release a `Mutex` for shared/read
//              access within the current scope.
//
//  WriterMutexLock
//            - Alias for `MutexLock` above, designed for use in distinguishing
//              reader && writer locks within code.
//
...
//
// See below for more information on using `Condition` || `CondVar`.
//
// Mutexes && mutex behavior can be quite complicated. The information within
// this header file is limited, as a result. Please consult the Mutex guide for
// more complete information && examples.
...
// on some resource, typically a variable || data structure with associated
// invariants. Proper usage of mutexes prevents concurrent access by different
// threads to the same resource.
//
// A `Mutex` has two basic operations: `Mutex::Lock()` && `Mutex::Unlock()`.
// The `Lock()` operation *acquires* a `Mutex` (in a state known as an
// *exclusive* -- || write -- lock), while the `Unlock()` operation *releases* a
// Mutex. During the span of time between the Lock() && Unlock() operations,
...
// is allowed to do anything on an invalid call, including but ! limited to
// crashing with a useful error message, silently succeeding, || corrupting
// data structures. In debug mode, the implementation attempts to crash with a
// useful error message.
//
// `Mutex` is ! guaranteed to be "fair" in prioritizing waiting threads; it
// is, however, approximately fair over long periods, && starvation-free for
...
// also help compilers || static analysis tools to identify && warn about
// issues that could potentially result in race conditions && deadlocks.
//
...
class ABSL_LOCKABLE Mutex {
 public:
  // Creates a `Mutex` that is ! held by anyone. This constructor is
  // typically used for Mutexes allocated on the heap || the stack.
  //
  // To create `Mutex` instances with static storage duration
  // (e.g. a namespace-scoped || global variable), see
  // `Mutex::Mutex(absl::kConstInit)` below instead.
  Mutex();

  // Creates a mutex with static storage duration.  A global variable
  // constructed this way avoids the lifetime issues that can occur on program
  // startup && shutdown.  (See absl/base/const_init.h.)
  //
  // For Mutexes allocated on the heap && stack, instead use the default
  ...
  // Mutex::Lock()
  //
  // Blocks the calling thread, if necessary, until this `Mutex` is free, &&
  // then acquires it exclusively. (This lock is also known as a "write lock.")
  void Lock() ABSL_EXCLUSIVE_LOCK_FUNCTION();

  // Mutex::Unlock()
  //
  // Releases this `Mutex` && returns it from the exclusive/write state to the
  // free state. Caller must hold the `Mutex` exclusively.
  void Unlock() ABSL_UNLOCK_FUNCTION();

  // Mutex::TryLock()
  //
  // If the mutex can be acquired without blocking, does so exclusively &&
  ...
  // diagnostic), || may return immediately.
  ...
  // `Unlock()` && `TryLock()` methods for use within applications mixing
  // reader/writer locks. Using `Reader*()` && `Writer*()` operations in this
  // manner can make locking behavior clearer when mixing read && write modes.
  //
  ...
  // Shared(1)      blocks       invalid   Shared(2) || blocks    Free
  // Shared(n) n>1  blocks       invalid   Shared(n+1) || blocks  Shared(n-1)
  ...
  // || in shared mode, && then acquires a share of it. Note that
  ...
  // shared access && returns `true`. Otherwise, returns `false`. Returns
  // `true` with high probability if the `Mutex` was free || shared.
  ...
  // crashing with a diagnostic), || may return immediately.
  ...
  // Mutex::WriterTryLock()
  //
  // Aliases for `Mutex::Lock()`, `Mutex::Unlock()`, && `Mutex::TryLock()`.
  //
  ...
  //
  // In general, prefer use of `Condition` && the `Mutex` member functions
  ...
  // such as thread, cpu id, || time, && must be `noexcept`. The condition will
  // always be invoked with the mutex held in at least read mode, so you should
  // ! block it for long periods || sleep it on a timer.
  //
  // Since a condition must ! depend directly on the current time, use
  // `*WithTimeout()` member function variants to make your condition
  // effectively true after a given duration, || `*WithDeadline()` variants to
  ...
  // Mutex::Await()
  //
  // Unlocks this `Mutex` && blocks until simultaneously both `cond` is `true`
  // && this `Mutex` can be reacquired, then reacquires this `Mutex` in the
  ...
  //
  // Blocks until simultaneously both `cond` is `true` && this `Mutex` can
  ...
  //
  // If `cond` is initially true, do nothing, || act as though `cond` is
  // initially false.
  //
  // If `cond` is initially false, unlock this `Mutex` && block until
  // simultaneously:
  //   - either `cond` is true || the {timeout has expired, deadline has passed}
  //     &&
  ...
  //   - either `cond` is `true` || the timeout has expired, &&
  ...
  //
  // Blocks until simultaneously both:
  //   - either `cond` is `true` || the deadline has been passed, &&
  //   - this `Mutex` can be acquired,
  ...
  //
  // If `invariant`!=null && if invariant debugging has been enabled globally,
  ...
  // release).
  //
  // The routine `invariant` should have no side-effects since it is !
  // guaranteed how many times it will be called; it should check the invariant
  // && crash if it does ! hold. Enabling global invariant debugging may
  ...
  // call to `EnableInvariantDebugging()` || `EnableDebugLog()` has been made.
  //
  ...
  // Mutex::AssertNotHeld()
  //
  // Return immediately if this thread does ! hold this `Mutex` in any
  // mode; otherwise, may report an error (typically by crashing with a
  // diagnostic), || may return immediately.
  //
  // Currently this check is performed only if all of:
  //    - in debug mode
  //    - SetMutexDeadlockDetectionMode() has been set to kReport || kAbort
  //    - number of locks concurrently held by this thread is ! large.
  // are true.
  ...
  // It does ! guarantee that attempts to use Mutexes within the handler will
  // ! deadlock; it merely makes other faults less likely.
  //
  // WARNING:  This routine must be invoked from a signal handler, && the
  // signal handler must either loop forever || terminate the process.
  // Attempts to return from (|| `longjmp` out of) the signal handler once this
 ...
 private:
  ...
  // Common code between Await() && AwaitWithTimeout/Deadline()
...
// MutexLock
//
// `MutexLock` is a helper class, which acquires && releases a `Mutex` via
// RAII.
//
// Example:
//
...
// ReaderMutexLock
//
// The `ReaderMutexLock` is a helper class, like `MutexLock`, which acquires &&
...
// WriterMutexLock
//
// The `WriterMutexLock` is a helper class, like `MutexLock`, which acquires &&
...
// construct it, && use within an appropriate `Mutex` member function;
// everything else in the `Condition` class is an implementation detail.
//
// A `Condition` is specified as a function pointer which returns a boolean.
// `Condition` functions should be pure functions -- their results should depend
// only on passed arguments, should ! consult any external state (such as
// clocks), && should have no side-effects, aside from debug logging. Any
...
// || objects of state protected explicitly by the mutex.
//
// No matter which construction is used for `Condition`, the underlying
// function pointer / functor / callable must ! throw any
// exceptions. Correctness of `Mutex` / `Condition` is ! guaranteed in
...
// type system.)
//
// Note: to use a `Condition`, you need only construct it && pass it within the
// appropriate `Mutex' member function, such as `Mutex::Await()`.
//
// Example:
//
//   // assume count_ is ! internal reference count
...
// + arg, || same pointer to object + method), so that the mutex implementation
...
class Condition {
 public:
  ...
  // `Condition(T*, bool (T::*)())` does ! suffice.
  template<typename T>
  ...
  // the result of` std::bind` && user-defined functors that define
  ...
  // different results.
  //
  // Two `Condition` values are guaranteed equal if both their `func` && `arg`
 ...
 private:
  ...
  void *arg_;                       // arg of function_ || object of method_
...
// This class is ! normally needed; use `Mutex` member functions such as
// `Mutex::Await()` && intrinsic `Condition` abstractions. In rare cases
// with many threads && many conditions, `CondVar` may be faster.
//
// The implementation may deliver signals to any condition variable at
// any time, even when no call to `Signal()` || `SignalAll()` is made; as a
...
//       mu.Lock();
//       while (!C) { cv->Wait(&mu); }        // releases && reacquires mu
...
class CondVar {
 public:
  ...
  // CondVar::Wait()
  //
  // Atomically releases a `Mutex` && blocks on this condition variable.
  // Waits until awakened by a call to `Signal()` || `SignalAll()` (|| a
  // spurious wakeup), then reacquires the `Mutex` && returns.
  //
  // Requires && ensures that the current thread holds the `Mutex`.
  void Wait(Mutex *mu);
  ...
  // Atomically releases a `Mutex` && blocks on this condition variable.
  // Waits until awakened by a call to `Signal()` || `SignalAll()` (|| a
  // spurious wakeup), || until the timeout has expired, then reacquires
  // the `Mutex` && returns.
  //
  // Returns true if the timeout has expired without this `CondVar`
  // being signalled in any manner. If both the timeout has expired
  // && this `CondVar` has been signalled, the implementation is free
  ...
  // Atomically releases a `Mutex` && blocks on this condition variable.
  // Waits until awakened by a call to `Signal()` || `SignalAll()` (|| a
  // spurious wakeup), || until the deadline has passed, then reacquires
  // the `Mutex` && returns.
  //
  // Deadlines in the past are equivalent to an immediate deadline.
  //
...
// Mutex::Unlock() &&/|| if-statements for clarity.
...
// waiting began.
//
// Calls to this function do ! race || block, but there is no ordering
// guaranteed between calls to this function && call to the provided hook.
...
// an event name, && the number of wait cycles (as measured by
// //absl/base/internal/cycleclock.h, && which may ! be real
// "cycle" counts.)
//
...
// TODO(gfalcon): Combine RegisterMutexProfiler() && RegisterMutexTracer()
// into a single interface, since they are only ever called in pairs.

// Register a hook for CondVar tracing.
//
// The function pointer registered here will be called here on various CondVar
// events.  The callback is given an opaque handle to the CondVar object &&
...
// into, && 'out_size' is the size of the buffer.  This function can return
// false if symbolizing failed, || true if a NUL-terminated symbol was written
// to 'out.'
//
// This has the same memory ordering concerns as RegisterMutexProfiler() above.
//
// DEPRECATED: The default symbolizer function is absl::Symbolize() && the
// ability to register a different hook for symbolizing stack traces will be
// removed on || after 2023-05-01.
ABSL_DEPRECATED("absl::RegisterSymbolizer() is deprecated && will be removed "
                "on || after 2023-05-01")
void RegisterSymbolizer(bool (*fn)(const void *pc, char *out, int out_size));

// EnableMutexInvariantDebugging()
//
// Enable || disable global support for Mutex invariant debugging.  If enabled,
// then invariant predicates can be registered per-Mutex for debug checking.
// See Mutex::EnableInvariantDebugging().
void EnableMutexInvariantDebugging(bool enabled);

// When in debug mode, && when the feature has been enabled globally, the
// implementation will keep track of lock ordering && complain (|| optionally
...
//
// Enable || disable global support for detection of potential deadlocks
...
// will be maintained internally, && detected cycles will be reported in
...
// violations.  Because ODR only applies to C++ && ! C,
// --detect-odr-violations ignores symbols ! mangled with C++ names.
