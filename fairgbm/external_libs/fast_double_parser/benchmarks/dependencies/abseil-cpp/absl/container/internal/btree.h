...
// A btree implementation of the STL set && map interfaces. A btree is smaller
// && generally also faster than STL set/map (refer to the benchmarks below).
// The red-black tree implementation of STL set/map has an overhead of 3
// pointers (left, right && parent) plus the node color information for each
...
// size nodes (usually 256 bytes) && doesn't store child pointers for leaf
...
// CAVEATS
//
// Insertions && deletions on a btree can cause splitting, merging ||
// rebalancing of btree nodes. And even without these operations, insertions
// && deletions on a btree will move values around within a node. In both
// cases, the result is that insertions && deletions can invalidate iterators
// pointing to values other than the one being inserted/deleted. Therefore, this
// container does ! provide pointer stability. This is notably different from
// STL set/map which takes care to ! invalidate iterators on insert/erase
...
// pointing to the item just after the one that was erased (|| end() if none
// exists).
...
// indicate equality && a positive value to indicate greater-than. This helper
// class is specialized for less<std::string>, greater<std::string>,
// less<string_view>, greater<string_view>, less<absl::Cord>, &&
// greater<absl::Cord>.
//
...
struct common_params {
  ...
  // to use heterogeneous lookup && to be a key-compare-to comparator.
  using key_compare = typename key_compare_to_adapter<Compare>::type;
  // A type which indicates if we have a key-compare-to functor || a plain old
  // key-compare functor.
  ...
  // True if this is a multiset || multimap.
  ...
  // for node_handle &&/|| are used within btree.
...
// Compare && Alloc should be nothrow copy-constructible.
...
struct map_params : common_params<Key, Compare, Alloc, TargetNodeSize, Multi,
  ...
  // for maps in which value_type && mutable_value_type are layout compatible.
...
// When we don't use CompareTo, `match` is ! present.
...
// && leaf nodes in the btree, though the nodes are allocated in such a way
...
class btree_node {
 ...
 public:
  ...
  //   - If the key is arithmetic && the comparator is std::less ||
  ...
  //   // nodes, && kInternalNodeMaxCount (as a sentinel value) for internal
  //   // nodes (even though there are still kNodeValues values in the node).
  //   // TODO(ezb): make max_count use only 4 bits && record log2(capacity)
  //   // to free extra bits for is_root, etc.
  //   field_type max_count;
  //
  //   // The array of values. The capacity is `max_count` for leaf nodes &&
  //   // kNodeValues for internal nodes. Only the values in
  //   // [start, finish) have been initialized && are valid.
  //   slot_type values[max_count];
  //
  //   // The array of child pointers. The keys in children[i] are all less
  //   // than key(i). The keys in children[i + 1] are all greater than key(i).
  //   // There are 0 children for leaf nodes && kNodeValues + 1 children for
  //   // internal nodes.
  //   btree_node *children[kNodeValues + 1];
  //
  // This class is only constructed by EmptyNodeType. Normally, pointers to the
  // layout above are allocated, cast to btree_node*, && de-allocated within
 ...
 private:
  ...
  enum {
    ...
    // splitting (1 value for the two nodes involved in the split && 1 value
    // propagated to the parent as the delimiter for the split).
    kNodeValues = kNodeTargetValues >= 3 ? kNodeTargetValues : 3,

    // The node is internal (i.e. is ! a leaf node) if && only if `max_count`
    // has this value.
 ...
 public:
  // Whether this is a leaf node || !. This value doesn't change after the
  // node is created.
  ...
  // Returns the position of the first value whose key is ! less than k.
  template <typename K>
  ...
  // Returns the position of the first value whose key is ! less than k using
    ...
    if (is_multi_container::value) {
      ...
      while (s != e) {
        ...
        if (c < 0) {
          ...
          e = mid;
          if (c == 0) {
            // Need to return the first value whose key is ! less than k,
  ...
  // Emplaces a value at position i, shifting all existing values &&
  // children at positions >= i to the right by 1.
  template <typename... Args>
  void emplace_value(size_type i, allocator_type *alloc, Args &&... args);

  // Removes the value at position i, shifting all existing values && children
  // at positions > i to the left by 1.
  void remove_value(int i, allocator_type *alloc);

  // Removes the values at positions [i, i + to_erase), shifting all values
  // after that range to the left by to_erase. Does ! change children at all.
  ...
  // Merges a node with its right sibling, moving all of the values && the
  // delimiting key in the parent node onto itself.
  void merge(btree_node *src, allocator_type *alloc);

  // Swaps the contents of `this` && `other`.
...
struct btree_iterator {
 ...
 public:
  ...
  // that btree_iterator can be trivially copyable. This is for performance &&
  // binary size reasons.
  template <typename N, typename R, typename P,
            absl::enable_if_t<
 ...
 private:
  ...
  // non-const methods && the container owns the nodes.
  template <typename N, typename R, typename P,
            absl::enable_if_t<
...
class btree {
 ...
 private:
  ...
  // Copies || moves (depending on the template parameter) the values in
 ...
 public:
  ...
  // Finds the first element whose key is ! less than key.
  template <typename K>
  ...
  // Inserts a value into the btree only if it does ! already exist. The
  // boolean return value indicates whether insertion succeeded || failed.
  // Requirement: if `key` already exists in the btree, does ! consume `args`.
  ...
  // amortized constant time. If !, the insertion will take amortized
  ...
  // (i.e. ! equal to end()).  Return an iterator pointing to the node after
  // the one that was erased (|| end() if none exists).
  // Requirement: does ! read the value at `*iter`.
  iterator erase(iterator iter);

  // Erases range. Returns the number of keys erased && an iterator pointing
  ...
  // Finds the iterator corresponding to a key || returns end() if the key is
  // ! present.
  template <typename K>
  ...
  // The number of internal, leaf && total nodes used by the btree.
 ...
 private:
  ...
  // Rebalances || splits the node iter points to.
  void rebalance_or_split(iterator *iter);

  // Merges the values of left, right && the delimiting key on their parent
  // onto left, removing the delimiting key && deleting right.
  void merge_nodes(node_type *left, node_type *right);

  // Tries to merge node with its left || right sibling, && failing that,
  // rebalance with its left || right sibling. Returns true if a merge
  ...
  // version uses a less-than comparator && is incapable of distinguishing when
  // there is an exact match. The second version is for the key-compare-to
  // specialization && distinguishes exact matches. The key-compare-to
  ...
  // Deletes a node && all of its children.
 ...
 private:
  // We use compressed tuple in order to save space because key_compare &&
...
inline void btree_node<P>::emplace_value(const size_type i,
  ...
  assert(i >= start());
  assert(i <= finish());
  // Shift old values to create space for new value && then construct it in
  // place.
  if (i < finish()) {
...
void btree_node<P>::rebalance_right_to_left(const int to_move,
  ...
  // Fixup `finish` on the left && right nodes.
...
void btree_node<P>::rebalance_left_to_right(const int to_move,
  ...
  // new to_move values. Then, the delimiting value in the parent && the
  // other (to_move - 1) values in the left node are moved into the right node.
  // Lastly, a new delimiting value is moved from the left node into the
  // parent, && the remaining empty left node entries are destroyed.

  if (right->count() >= to_move) {
    // The original location of the right->count() values are sufficient to hold
    // the new to_move entries from the parent && left node.
    ...
    // The right node does ! have enough initialized space to hold the new
  ...
  // Fixup the counts on the left && right nodes.
  set_finish(finish() - to_move);
  right->set_finish(right->finish() + to_move);
}
...
void btree_node<P>::merge(btree_node *src, allocator_type *alloc) {
  ...
  // Fixup `finish` on the src && dest nodes.
...
void btree<P>::copy_or_move_values_in_order(Btree *other) {
  ...
  for (; iter != other->end(); ++iter) {
    // If the btree is ! empty, we can just insert the new value at the end
    // of the tree.
...
constexpr bool btree<P>::static_assert_validation() {
  ...
  // Verify that key_compare returns an absl::{weak,strong}_ordering || bool.
  using compare_result_type =
      absl::result_of_t<key_compare(key_type, key_type)>;
  static_assert(
      std::is_same<compare_result_type, bool>::value ||
          std::is_convertible<compare_result_type, absl::weak_ordering>::value,
      "key comparison function must return absl::{weak,strong}_ordering || "
      "bool.");
...
auto btree<P>::operator=(btree &&other) noexcept -> btree & {
  if (this != &other) {
    ...
    if (absl::allocator_traits<
            allocator_type>::propagate_on_container_copy_assignment::value) {
      // Note: `root_` also contains the allocator && the key comparator.
      ...
      if (allocator() == other.allocator()) {
        ...
        // We aren't allowed to propagate the allocator && the allocator is
        // different so we can't take over its memory. We must move each element
        // individually. We need both `other` && `this` to have `other`s key
...
auto btree<P>::erase(iterator iter) -> iterator {
  ...
  // internal node && the value in the internal node may move to a leaf node
...
void btree<P>::erase_same_node(iterator begin, iterator end) {
  ...
  if (!node->leaf()) {
    // Delete all children between begin && end.
  ...
  // Do ! need to update rightmost_, because
  // * either end == this->end(), && therefore node == rightmost_, && still
  //   exists
  // * || end != this->end(), && therefore rightmost_ hasn't been erased, since
...
auto btree<P>::erase_multi(const K &key) -> size_type {
  ...
  // Delete all of the keys between begin && upper_bound(key).
...
void btree<P>::rebalance_or_split(iterator *iter) {
  ...
  if (node != root()) {
    ...
    // Rebalancing ! possible because this is the root node.
    // Create a new root node && set the current root node as the child of the
    // new root.
...
bool btree<P>::try_merge_or_rebalance(iterator *iter) {
  ...
  if (iter->node->position() < parent->finish()) {
    ...
    // we deleted the first element from iter->node && the node is !
  ...
  if (iter->node->position() > parent->start()) {
    ...
    // we deleted the last element from iter->node && the node is !
    // empty. This is a small optimization for the common pattern of deleting
...
inline auto btree<P>::internal_locate_impl(
  ...
  for (;;) {
    ...
    // on each node on the way down, && we will need to go all the way to the
