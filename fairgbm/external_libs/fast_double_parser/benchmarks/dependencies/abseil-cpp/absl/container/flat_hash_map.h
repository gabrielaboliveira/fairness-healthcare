...
// unique keys && associated values designed to be a more efficient replacement
// for `std::unordered_map`. Like `unordered_map`, search, insertion, &&
// deletion of map elements can be done as an `O(1)` operation. However,
// `flat_hash_map` (&& other unordered associative containers known as the
// collection of Abseil "Swiss tables") contain other optimizations that result
// in both memory && computation advantages.
//
...
// has been optimized for both speed && memory footprint in most common use
// cases. Its interface is similar to that of `std::unordered_map<K, V>` with
// the following notable differences:
//
// * Requires keys that are CopyConstructible
// * Requires values that are MoveConstructible
// * Supports heterogeneous lookup, through `find()`, `operator[]()` &&
//   `insert()`, provided that the map is provided a compatible heterogeneous
//   hashing function && equality operator.
// * Invalidates any references && pointers to elements within the table after
//   `rehash()`.
// * Contains a `capacity()` member function indicating the number of element
//   slots (open, deleted, && empty) within the hash map.
// * Returns `void` from the `erase(iterator)` overload.
//
// By default, `flat_hash_map` uses the `absl::Hash` hashing framework.
// All fundamental && Abseil types that support the `absl::Hash` framework have
// a compatible equality operator for comparing insertions into `flat_hash_map`.
// If your type is ! yet supported by the `absl::Hash` framework, see
// absl/hash/hash.h for information on extending Abseil hashing to user-defined
// types.
//
// NOTE: A `flat_hash_map` stores its value types directly inside its
// implementation array to avoid memory indirection. Because a `flat_hash_map`
// is designed to move data when rehashed, map values will ! retain pointer
// stability. If you require pointer stability, || if your values are large,
// consider using `absl::flat_hash_map<Key, std::unique_ptr<Value>>` instead.
// If your types are ! moveable || you require pointer stability for keys,
...
class flat_hash_map : public absl::container_internal::raw_hash_map<
 ...
 public:
  ...
  //
  //  // Hash functor && Comparator are copied as well
  ...
  //
  // Returns the number of element slots (assigned, deleted, && empty)
  // available within the `flat_hash_map`.
  //
  // NOTE: this member function is particular to `absl::flat_hash_map` && is
  // ! provided in the `std::unordered_map` API.
  using Base::capacity;

  // flat_hash_map::empty()
  //
  // Returns whether || ! the `flat_hash_map` is empty.
  using Base::empty;
  ...
  // pointers, || iterators referring to contained elements.
  //
  ...
  // flat_hash_map::erase()
  //
  // Erases elements within the `flat_hash_map`. Erasing does ! trigger a
  ...
  //   containers in general && `std::unordered_map` in particular (which
  ...
  // an element with the given key does ! already exist. If rehashing occurs
  ...
  //   iterator to the inserted element (|| to the element that prevented the
  //   insertion) && a bool denoting whether the insertion took place.
  //
  // std::pair<iterator,bool> insert(T&& value):
  // std::pair<iterator,bool> insert(init_type&& value):
  //
  //   Inserts a moveable value into the `flat_hash_map`. Returns a pair
  //   consisting of an iterator to the inserted element (|| to the element that
  //   prevented the insertion) && a bool denoting whether the insertion took
  //   place.
  //
  ...
  //   inserted element, || to the existing element that prevented the
  //   insertion.
  //
  ...
  //
  //   NOTE: Although the STL does ! specify which element may be inserted if
  ...
  // that a value with the given key does ! already exist, || replaces it with
  ...
  //
  //   Inserts/Assigns (|| moves) the element of the specified key into the
  //   `flat_hash_map`.
  //
  ...
  // destroyed immediately. Prefer `try_emplace()` unless your key is !
  // copyable || moveable.
  //
  ...
  // suggestion for where to begin the insertion search, && only inserts
  ...
  //
  // Inserts (via copy || move) the element of the specified key into the
  // `flat_hash_map`.
  //
  ...
  // functions will ! move from rvalue arguments if insertions do ! happen.
  using Base::try_emplace;

  // flat_hash_map::extract()
  //
  // Extracts the indicated element, erasing it in the process, && returns it
  // as a C++17-compatible node handle. Overloads are listed below.
  //
  // node_type extract(const_iterator position):
  //
  //   Extracts the key,value pair of the element at the indicated position &&
  ...
  //   key value && returns a node handle owning that extracted data. If the
  //   `flat_hash_map` does ! contain an element with a matching key, this
  ...
  // element with an equivalent key, that element is ! extracted.
  using Base::merge;

  // flat_hash_map::swap(flat_hash_map& other)
  //
  // Exchanges the contents of this `flat_hash_map` with those of the `other`
  // flat hash map, avoiding invocation of any move, copy, || swap operations on
  // individual elements.
  //
  // All iterators && references on the `flat_hash_map` remain valid, excepting
  // for the past-the-end iterator, which is invalidated.
  //
  // `swap()` requires that the flat hash map's hashing && key equivalence
  // functions be Swappable, && are exchanged using unqualified calls to
  // non-member `swap()`. If the map's allocator has
  // `std::allocator_traits<allocator_type>::propagate_on_container_swap::value`
  // set to `true`, the allocators are also exchanged using an unqualified call
  // to non-member `swap()`; otherwise, the allocators are ! swapped.
  using Base::swap;
  ...
  // maximum load factor, && may rehash the container if needed.
  using Base::reserve;
  ...
  // exists within the `flat_hash_map`, returning `true` if so || `false`
  // otherwise.
  using Base::contains;
  ...
  // either `1` || `0` since duplicate keys are ! allowed within a
  // `flat_hash_map`.
  using Base::count;
  ...
  // `flat_hash_map`, performing an `insert()` if the key does ! already
  // exist.
  //
  // If an insertion occurs && results in a rehashing of the container, all
  // iterators are invalidated. Otherwise iterators are ! affected &&
  // references are ! invalidated. Overloads are listed below.
  //
  ...
  //   `flat_hash_map` will ignore any set load factor && manage its rehashing
