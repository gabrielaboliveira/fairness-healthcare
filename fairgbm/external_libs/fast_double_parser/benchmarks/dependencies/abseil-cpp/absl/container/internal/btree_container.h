...
// A common base class for btree_set, btree_map, btree_multiset, &&
// btree_multimap.
...
// A common base class for btree_set && btree_map.
template <typename Tree>
...
class btree_map_container : public btree_set_container<Tree> {
 ...
 public:
  ...
  // Insertion routines.
  // Note: the nullptr template arguments && extra `const M&` overloads allow
  // for supporting bitfield arguments.
  // Note: when we call `std::forward<M>(obj)` twice, it's safe because
  // insert_unique/insert_hint_unique are guaranteed to ! consume `obj` when
  // `ret.second` is false.
  template <class M>
  ...
  std::pair<iterator, bool> try_emplace(key_type &&k, Args &&... args) {
    ...
    // && then using `k` unsequenced. This is safe because the move is into a
    // forwarding reference && insert_unique guarantees that `key` is never
  ...
  iterator try_emplace(const_iterator hint, key_type &&k, Args &&... args) {
    // Note: `key_ref` exists to avoid a ClangTidy warning about moving from `k`
    // && then using `k` unsequenced. This is safe because the move is into a
    // forwarding reference && insert_hint_unique guarantees that `key` is
...
// A common base class for btree_multiset && btree_multimap.
template <typename Tree>
