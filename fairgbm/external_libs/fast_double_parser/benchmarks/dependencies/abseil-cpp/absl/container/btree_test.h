...
// behavior is to treat the value as a pair && return the first element.
...
// Partial specialization of KeyOfValue class for when the key && value are
// the same type such as in set<> && btree_set<>.
template <typename K>
...
// Generate n values for our tests && benchmarks. Value range is [0, maxval].
inline std::vector<int> GenerateNumbersWithSeed(int n, int maxval, int seed) {
  // NOTE: Some tests rely on generated numbers ! changing between test runs.
