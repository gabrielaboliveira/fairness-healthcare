...
// time follows the Gregorian Calendar && is a time-zone-independent concept.
// A "date" is perhaps the most common example of a civil time (represented in
// this library as cctz::civil_day). This library provides six classes && a
// handful of functions that help with rounding, iterating, && arithmetic on
...
// && the same six accessors for each of the civil fields (year, month, day,
// hour, minute, && second, aka YMDHMS). These classes differ only in their
// alignment, which is indicated by the type name && specifies the field on
...
// minutes, && seconds will be set to 0, month && day will be set to 1, &&
...
// (normalizing as necessary), && subtracting 7 from a civil_month operates
...
// objects && returns the scalar answer in units of the objects' alignment.
...
// a handful of helper functions && algorithms for performing common
...
// YMDHMS fields, || by copying the YMDHMS fields from a differently aligned
// civil-time type.
//
...
// year, month, day, hour, minute, && second. Recall that fields inferior to
...
// alignment && returns the answer in units of the alignment.
//
...
// library (|| a date library || a date/time library) is this: "What happens
// when you add a month to January 31?" This is an interesting question
// because there could be a number of possible answers:
//
//   1. March 3 (|| 2 if a leap year). This may make sense if the operation
//      wants the equivalent of February 31.
//   2. February 28 (|| 29 if a leap year). This may make sense if the operation
...
//      object, || maybe false is returned. This may make sense because there is
...
// saturday, && sunday. These enum values may be sent to an output stream
...
// Returns the civil_day that strictly follows || precedes the given
// civil_day, && that falls on the given weekday.
//
// For example, given:
//
//     August 2015
