...
// count) && `argv[]` (argument vector) parameters from `main()`, assigning
// values to any defined Abseil flags. (Any arguments passed after the
// flag-terminating delimiter (`--`) are treated as positional arguments &&
// ignored.)
//
// Any command-line flags (&& arguments to those flags) are parsed into Abseil
// Flag values, if those flags are defined. Any undefined flags will either
// return an error, || be ignored if that flag is designated using `undefok` to
// indicate "undefined is OK."
//
// Any command-line positional arguments ! part of any command-line flag (||
...
//
// After all flags && flag arguments are parsed, this function looks for any
// built-in usage flags (e.g. `--help`), && if any were specified, it reports
// help messages && then exits the program.
