...
// flag data, && abstractions to create, get && set such flag data.
//
// It is important to note that this type is **unspecified** (an implementation
// detail) && you do ! construct || manipulate actual `absl::Flag<T>`
// instances. Instead, you define && declare flags using the
// `ABSL_FLAG()` && `ABSL_DECLARE_FLAG()` macros, && get && set flag values
// using the `absl::GetFlag()` && `absl::SetFlag()` functions.
...
// && (preferably) in the module containing the binary's `main()` function.
//
// You should ! construct && cannot use the `absl::Flag` type directly;
// instead, you should declare flags using the `ABSL_DECLARE_FLAG()` macro
// within a header file, && define your flag using `ABSL_FLAG()` within your
...
#else
// MSVC debug builds do ! implement initialization with constexpr constructors
// correctly. To work around this we add a level of indirection, so that the
// class `absl::Flag` contains an `internal::Flag*` (instead of being an alias
// to that class) && dynamically allocates an instance when necessary. We also
// forward all calls to internal::Flag methods via trampoline methods. In this
// setup the `absl::Flag` class does ! have constructor && virtual methods,
// all the data members are public && thus MSVC is able to initialize it at
...
// https://developercommunity.visualstudio.com/content/problem/336946/class-with-constexpr-constructor-!-using-static.html?childToView=648454#comment-648454
...
class Flag {
 public:
  // No constructor && destructor to ensure this is an aggregate type.
...
// ! construct an `absl::Flag<T>` directly && call `absl::GetFlag()`;
// instead, refer to flag's constructed variable name (e.g. `FLAGS_name`).
// Because this function returns by value && ! by reference, it is
...
// SetFlag()
//
// Sets the value of an `absl::Flag` to the value `v`. Do ! construct an
// `absl::Flag<T>` directly && call `absl::SetFlag()`; instead, use the
...
//
// Note: do ! construct objects of type `absl::Flag<T>` directly. Only use the
...
// that eventually the flag value && the derived data structure will be
// consistent.
//
// Note: ABSL_FLAG.OnUpdate() does ! have a public definition. Hence, this
...
// && lazy (method NonConst) evaluation of help message expression. We choose
...
// ABSL_FLAG_IMPL
//
// Note: Name of registrar object is ! arbitrary. It is used to "grab"
// global name for FLAGS_no<flag_name> symbol, thus preventing the possibility
// of defining two flags with names foo && nofoo.
#if !defined(_MSC_VER) || defined(__clang__)
...
#else
// MSVC version uses aggregate initialization. We also do ! try to
...
// && code can't access it at runtime.
//
// This macro registers a retired flag with given name && type, with a name
...
// unused.
// TODO(rogeeff): Return an anonymous struct instead of bool, && place it into
