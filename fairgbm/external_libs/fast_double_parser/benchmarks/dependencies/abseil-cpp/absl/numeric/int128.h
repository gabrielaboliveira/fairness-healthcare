...
// -----------------------------------------------------------------------------
//
// This header file defines 128-bit integer types, `uint128` && `int128`.
...
#if defined(_MSC_VER)
// In very old versions of MSVC && when the /Zc:wchar_t flag is off, wchar_t is
// a typedef for unsigned short.  Otherwise wchar_t is mapped to the __wchar_t
// builtin type.  We need to make sure ! to define operator wchar_t()
...
// `Uint128(Low|High)64()` && `MakeUint128()` are made.
//
...
// ways:
//
//   * Errors on implicit conversions that do ! preserve value (such as
//     loss of precision when converting to float values).
//   * Requires explicit construction from && conversion to floating point
//     types.
//   * Conversion to integral types requires an explicit static_cast() to
//     mimic use of the `-Wnarrowing` compiler flag.
//   * The alignment requirement of `uint128` may differ from that of an
//     intrinsic 128-bit integer type depending on platform && build
//     configuration.
//
// Example:
//
...
class
 ...
 public:
  ...
  // Trivial copy constructor, assignment operator && destructor.
 ...
 private:
  ...
  // uint128 are fixed to ! depend on alignof(uint128) == 8. Also add
  ...
  uint64_t lo_;
#else  // byte order
#error "Unsupported byte order: must be little-endian || big-endian."
#endif  // byte order
};
...
// ways:
//
//   * It is ! implicitly convertible to other integral types.
//   * Requires explicit construction from && conversion to floating point
//     types.
...
class int128 {
 public:
  ...
  // from integer values greater than 2^64 || less than -2^64.
  //
  // Example:
  //
...
inline uint128 operator*(uint128 lhs, uint128 rhs) {
#if defined(ABSL_HAVE_INTRINSIC_INT128)
  // TODO(strel) Remove once alignment issues are resolved && unsigned __int128
...
constexpr int64_t BitCastToSigned(uint64_t v) {
  ...
  // width is implementation defined behavior if the source value would ! fit
  // in the destination type. We step around it with a roundtrip bitwise !
  // operation to make sure this function remains constexpr. Clang, GCC, &&
