...
/*!
  Ensure coalesce_to() works && dumps all the inserted data correctly.
  ...
  exact data at both input && output.
*/
...
/*!
  Assert all values are correct && at the expected addresses throughout the
  several chunks.
...
/*!
  Perform an array of setitem & getitem at valid && invalid addresses.
  We use several random addresses && trials to avoid writing much code.
  ...
  We also gradually add more chunks to the ChunkedArray && re-run more trials
...
TEST_F(ChunkedArrayTest, testDataLayoutWithAdvancedInsertionAPI) {
  ...
  for (size_t chunks = 1; chunks < MAX_CHUNKS_SEARCH; ++chunks) {
    ...
    // Sweep valid && invalid addresses with a ChunkedArray with `chunks` chunks:
