...
// used by both BasicLeafConstraints && IntermediateLeafConstraints
...
struct FeatureMinOrMaxConstraints {
  ...
  // if threshold[i+1] does ! exist, then it is valid for thresholds following
  // threshold[i]
...
class IntermediateLeafConstraints : public BasicLeafConstraints {
 public:
  ...
  bool OppositeChildShouldBeUpdated(
    ...
    // if the split is categorical, it is ! handled by this optimisation,
    ...
    if (is_split_numerical) {
      ...
      // second time going up on the right (|| on the left)
  ...
  // Recursive function that goes up the tree, && then down to find leaves that
  ...
  void GoUpToFindLeavesToUpdate(
    ...
    if (parent_idx != -1) {
      ...
      // this is just an optimisation ! to waste time going down in subtrees
      ...
      if (opposite_child_should_be_updated) {
        ...
        // then there is no relationship between its left && right leaves' values
        if (monotone_type != 0) {
        ...
        // is actually contiguous to the original 2 leaves && should be updated
      ...
      // since current node is ! the root, keep going up
  ...
  void GoDownToFindLeavesToUpdate(
    ...
    if (node_idx < 0) {
      ...
      // splits that are ! to be used shall ! be updated,
      ...
      // if the current leaf is contiguous with both the new right leaf && the new left leaf
      // then it may need to be greater than the max of the 2 || smaller than the min of the 2
      // otherwise, if the current leaf is contiguous with only one of the 2 new leaves,
      // then it may need to be greater || smaller than it
...
#endif
      // depending on which split made the current leaf && the original leaves contiguous,
      // either the min constraint || the max constraint of the current leaf need to be updated
      if (!update_max_constraints) {
      ...
      // If constraints were ! updated, then there is no need to update the leaf
      ...
      // if the split is on the same feature (categorical variables ! supported)
      // then depending on the threshold,
      // the current left child may ! be contiguous with the original right leaf,
      // || the current right child may ! be contiguous with the original left leaf
  ...
  std::pair<bool, bool> ShouldKeepGoingLeftRight(
    ...
    // left && right nodes are checked to find out if they are contiguous with
...
class AdvancedLeafConstraints : public IntermediateLeafConstraints {
 public:
  ...
  // be valid on [threshold[i]: threshold[i + 1]) (|| [threshold[i]: +inf) if i
  ...
  void UpdateConstraints(FeatureMinOrMaxConstraints* feature_constraint,
    ...
    // the thresholds && constraints must become
    // [0, 1, 2] &&  [cstr1, cstr2, cstr1]
    ...
    for (size_t i = 0; i < feature_constraint->thresholds.size(); ++i) {
      ...
      if (feature_constraint->thresholds[i] > it_start) {
        ...
        // && a new constraint may need to be inserted
        if (!start_done) {
          start_done = true;
  ...
  // ! contain any relevant constraint (for example if the a branch
  ...
  void GoDownToFindConstrainingLeaves(
    ...
    if (node_idx < 0) {
      ...
      // check if the children are contiguous to the original leaf && therefore
  ...
  void GoUpToFindConstrainingLeaves(
    ...
    if (parent_idx != -1) {
      ...
      // original leaf  are gathered so the starting && ending
      ...
      if (opposite_child_necessary_to_update_constraints) {
        // if there is no monotone constraint on a split,
        // then there is no relationship between its left && right leaves'
        // values
        if (monotone_type != 0) {
          int left_child_idx = tree_->left_child(parent_idx);
          int right_child_idx = tree_->right_child(parent_idx);
        ...
        // i.e. that it will be helpful going down to determine which leaf
        // is actually contiguous to the original leaf && constraining
        // so the variables associated with the split need to be recorded
