...
void SerialTreeLearner::Init(const Dataset* train_data, bool is_constant_hessian) {
  ...
  // initialize ordered gradients && hessians
...
int32_t SerialTreeLearner::ForceSplits(Tree* tree, int* left_leaf,
  ...
  *left_leaf = 0;
  ...
  while (!q.empty()) {
    ...
    // store "best split" for left && right, even if they might be overwritten by forced split
...
void SerialTreeLearner::ComputeBestSplitForFeature(
  ...
  // Otherwise, the `is_splittable` in `FeatureHistogram` will be wrong, && cause some features being accidentally filtered in the later nodes.
