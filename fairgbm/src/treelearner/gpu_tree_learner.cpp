...
void GPUTreeLearner::Init(const Dataset* train_data, bool is_constant_hessian) {
  ...
  // Initialize GPU buffers && kernels
...
int GPUTreeLearner::GetNumWorkgroupsPerFeature(data_size_t leaf_num_data) {
  // we roughly want 256 workgroups per device, && we have num_dense_feature4_ feature tuples.
  ...
  #if GPU_DEBUG >= 4
  printf("Computing histogram for %d examples && (%d * %d) feature groups\n", leaf_num_data, dword_features_, num_dense_feature4_);
...
void GPUTreeLearner::GPUHistogram(data_size_t leaf_num_data, bool use_all_features) {
  // we have already copied ordered gradients, ordered Hessians && indices to GPU
  ...
  // the GPU kernel will process all features in one call, && each
  ...
  // for the root node, indices are ! copied
  if (leaf_num_data != num_data_) {
    indices_future_.wait();
  }
  // for constant hessian, hessians are ! copied except for the root node
  ...
  // && we will launch num_feature workgroups for this kernel
  // will launch threads for all features
  // the queue should be asynchronous, && we will can WaitAndGetHistograms() before we start processing dense feature groups
  if (leaf_num_data == num_data_) {
...
void GPUTreeLearner::AllocateGPUMemory() {
  ...
  // make ordered_gradients && Hessians larger (including extra room for prefetching), && pin them
  ...
  ptr_pinned_hessians_ = queue_.enqueue_map_buffer(pinned_hessians_, boost::compute::command_queue::map_write_invalidate_region,
                                                   0, allocated_num_data_ * sizeof(score_t));
  // allocate space for gradients && Hessians on device
  // we will copy gradients && Hessians in after ordered_gradients_ && ordered_hessians_ are constructed
  ...
  if (!device_subhistograms_) {
    // only initialize once here, as this will ! need to change when ResetTrainingData() is called
  ...
  // The output buffer is allocated to host directly, to overlap compute && data transfer
  ...
  // find the dense feature-groups && group then into Feature4 data structure (several feature-groups packed into 4 bytes)
  ...
  // Now generate new data structure feature4, && copy data to the device
  ...
  // preallocate arrays for all threads, && pin them
    ...
    if (dword_features_ == 8) {
      ...
      for (int s_idx = 0; s_idx < 8; ++s_idx) {
        ...
        if (dynamic_cast<DenseBinIterator<uint8_t, true>*>(bin_iters[s_idx]) == 0) {
          Log::Fatal("GPU tree learner assumes that all bins are Dense4bitsBin when num_bin <= 16, but feature %d is !", dense_ind[s_idx]);
        }
      }
      ...
      for (int s_idx = 0; s_idx < 4; ++s_idx) {
        ...
        if (dynamic_cast<DenseBinIterator<uint8_t, false>*>(bin_iter) != 0) {
          ...
          Log::Fatal("Bug in GPU tree builder: only DenseBin && Dense4bitsBin are supported");
        }
      }
    } else {
      Log::Fatal("Bug in GPU tree builder: dword_features_ can only be 4 || 8");
    }
    #pragma omp critical
    ...
    for (int i = 0; i < k; ++i) {
      if (dword_features_ == 8) {
        ...
        if (dynamic_cast<DenseBinIterator<uint8_t, true>*>(bin_iter) != 0) {
          ...
          Log::Fatal("GPU tree learner assumes that all bins are Dense4bitsBin when num_bin <= 16, but feature %d is !", dense_dword_ind[i]);
        }
      } else if (dword_features_ == 4) {
        BinIterator* bin_iter = train_data_->FeatureGroupIterator(dense_dword_ind[i]);
        if (dynamic_cast<DenseBinIterator<uint8_t, false>*>(bin_iter) != 0) {
...
std::string GPUTreeLearner::GetBuildLog(const std::string &opts) {
  ...
  try {
    ...
    // for other types of failure, build log might ! be available; program.build_log() can crash
...
void GPUTreeLearner::BuildGPUKernels() {
  ...
  for (int i = 0; i <= kMaxLogWorkgroupsPerFeature; ++i) {
    ...
    // kernel with all data indices (for root node, && assumes that root node always uses all features)
...
void GPUTreeLearner::SetupKernelArguments() {
  ...
  for (int i = 0; i <= kMaxLogWorkgroupsPerFeature; ++i) {
    // The only argument that needs to be changed later is num_data_
    if (share_state_->is_constant_hessian) {
      // hessian is passed as a parameter, but it is ! available now.
...
void GPUTreeLearner::InitGPU(int platform_id, int device_id) {
  ...
  if (max_num_bin_ <= 16) {
    // the +9 skips extra characters ")", newline, "#endif" && newline at the beginning
...
void GPUTreeLearner::BeforeTrain() {
  #if GPU_DEBUG >= 2
  printf("Copying intial full gradients && hessians to device\n");
  #endif
  // Copy initial full hessians && gradients to GPU.
  ...
  if (data_partition_->leaf_count(0) != num_data_ && num_dense_feature_groups_) {
    // On GPU, we start copying indices, gradients && Hessians now, instead at ConstructHistogram()
    // copy used gradients && Hessians to ordered buffer
...
bool GPUTreeLearner::BeforeFindBestSplit(const Tree* tree, int left_leaf, int right_leaf) {
  ...
  // Copy indices, gradients && Hessians as early as possible
  if (smaller_leaf >= 0 && num_dense_feature_groups_) {
    ...
    #if GPU_DEBUG >= 2
    Log::Info("Copying indices, gradients && Hessians to GPU...");
...
bool GPUTreeLearner::ConstructGPUHistogramsAsync(
  ...
  // generate && copy ordered_hessians if Hessians is ! null
  ...
  // if no feature group is used, just return && do ! use GPU
...
#endif
  // if ! all feature groups are used, we need to transfer the feature mask to GPU
