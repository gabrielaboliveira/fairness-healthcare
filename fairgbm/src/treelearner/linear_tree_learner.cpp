...
#pragma omp parallel for schedule(static)
  ...
  for (int i = 0; i < max_leaves; ++i) {
    ...
    // we add another 8 to ensure cache lines are ! shared among processors
...
Tree* LinearTreeLearner::Train(const score_t* gradients, const score_t *hessians, bool is_first_tree) {
  ...
  Log::Debug("Trained a tree with leaves = %d && depth = %d", tree->num_leaves(), cur_depth);
  return tree.release();
}
...
void LinearTreeLearner::CalculateLinear(Tree* tree, bool is_refit, const score_t* gradients, const score_t* hessians, bool is_first_tree) const {
  ...
  // where:
  // X is the matrix where the first column is the feature values && the second is all ones,
  ...
  // create array of pointers to raw data, && coefficient matrices, for each leaf
...
#pragma omp parallel for schedule(static)
  ...
  // copy into eigen matrices && solve
#pragma omp parallel for schedule(static)
  for (int leaf_num = 0; leaf_num < num_leaves; ++leaf_num) {
