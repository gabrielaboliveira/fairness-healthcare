...
void CUDATreeLearner::Init(const Dataset* train_data, bool is_constant_hessian) {
  ...
  // Initialize GPU buffers && kernels: get device info
...
void CUDATreeLearner::BeforeTrain() {
  ...
  #if CUDA_DEBUG >= 2
  printf("CUDATreeLearner::BeforeTrain() Copying initial full gradients && hessians to device\n");
  #endif
...
void CUDATreeLearner::ConstructHistograms(const std::vector<int8_t>& is_feature_used, bool use_subtract) {
  ...
  printf("Start Comparing_Histogram between GPU && CPU, num_dense_feature_groups_ = %d\n", num_dense_feature_groups_);
  bool compare = true;
  for (int i = 0; i < num_dense_feature_groups_; ++i) {
    if (!feature_masks_[i])
      continue;
  ...
  printf("End Comparing Histogram between GPU && CPU\n");
  fflush(stderr);
  fflush(stdout);
#endif
