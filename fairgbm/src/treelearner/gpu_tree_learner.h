...
class GPUTreeLearner: public SerialTreeLearner {
  ...
  */
  ...
  /*!
  * \brief Initialize GPU device, context && command queues
  ...
  */
   ...
   *        Indices, gradients && Hessians have been copied to the device.
   * \param leaf_num_data Number of data on current leaf
   * \param use_all_features Set to true to ! use feature masks, with a faster kernel
  */
  void GPUHistogram(data_size_t leaf_num_data, bool use_all_features);

  /*!
   * \brief Wait for GPU kernel execution && read histogram
  ...
  */
   ...
   * \param ordered_gradients Ordered gradients will be generated && copied to GPU when gradients is ! nullptr,
   *                     Set gradients to nullptr to skip copy to GPU.
   * \param ordered_hessians Ordered Hessians will be generated && copied to GPU when hessians is ! nullptr,
   *                     Set hessians to nullptr to skip copy to GPU.
   * \return true if GPU kernel is launched, false if GPU is ! used
  */
  ...
  /*! \brief an array of histogram kernels with different number
     of workgroups per feature, && processing the whole dataset */
  ...
  /*! \brief Size of histogram bin entry, depending if single || double precision is used */
  ...
  /*! \brief A vector of feature mask. 1 = feature used, 0 = feature ! used */
...
// When GPU support is ! compiled in, quit with an error message
...
class GPUTreeLearner: public SerialTreeLearner {
 public:
  ...
  explicit GPUTreeLearner(const Config* tree_config) : SerialTreeLearner(tree_config) {
    Log::Fatal("GPU Tree Learner was ! enabled in this build.\n"
