...
/*!
 * \brief FeatureHistogram is used to construct && store a histogram for a
 * feature.
 */
...
#undef ARGUMENTS
  ...
  void FindBestThresholdCategoricalInner(double sum_gradient,
    ...
    if (use_onehot) {
      ...
      for (int t = bin_start; t < bin_end; ++t) {
        ...
        // if data ! enough, || sum hessian too small
  ...
  void GatherInfoForThresholdNumericalInner(double sum_gradient, double sum_hessian,
    ...
    const double cnt_factor = num_data / sum_hessian;
    // from right to left, && we don't need data in bin0
    for (; t >= t_end; --t) {
...
class HistogramPool {
 public:
  ...
  void Reset(int cache_size, int total_size) {
    ...
    // at least need 2 bucket to store smaller leaf && larger leaf
...
#pragma omp parallel for schedule(static)
   ...
   * \return True if this index is in the pool, False if this index is ! in
   * the pool
   */
