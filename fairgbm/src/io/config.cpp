...
void Config::KV2Map(std::unordered_map<std::string, std::string>* params, const char* kv) {
  ...
  if (tmp_strs.size() == 2 || tmp_strs.size() == 1) {
    ...
    if (key.size() > 0) {
      ...
      if (value_search == params->end()) {  // ! set
...
void GetMetricType(const std::unordered_map<std::string, std::string>& params, std::vector<std::string>* metric) {
  ...
  // add names of objective function if ! providing metric
...
void Config::CheckParamConflict() {
  // check if objective, metric, && num_class match
  ...
  if (objective_type_multiclass) {
    if (num_class_check <= 1) {
      Log::Fatal("Number of classes should be specified && greater than 1 for multiclass training");
    }
  } else {
  ...
  for (std::string metric_type : metric) {
    ...
    if ((objective_type_multiclass && !metric_type_multiclass)
      ...
      Log::Fatal("Multiclass objective && metrics don't match");
    }
  }
  ...
  if (max_depth > 0) {
    ...
    if (full_num_leaves < num_leaves) {
      // Fits in an int, && is more restrictive than the current num_leaves
  ...
  // linear tree learner must be serial type && run on CPU device
  if (linear_tree) {
  ...
  // calculated split gain can be positive even with zero gradient && hessian.
  ...
  if (is_parallel && (monotone_constraints_method == std::string("intermediate") || monotone_constraints_method == std::string("advanced"))) {
    ...
    Log::Warning("Cannot use \"intermediate\" || \"advanced\" monotone constraints in distributed learning, auto set to \"basic\" method.");
  ...
  if (feature_fraction_bynode != 1.0 && (monotone_constraints_method == std::string("intermediate") || monotone_constraints_method == std::string("advanced"))) {
    ...
    // split initially, then the sampling needs to be recorded || done once again, which is currently ! supported
    Log::Warning("Cannot use \"intermediate\" || \"advanced\" monotone constraints with feature fraction different from 1, auto set monotone constraints to \"basic\" method.");
    monotone_constraints_method = "basic";
  }
  ...
  if (min_data_in_leaf <= 0 && min_sum_hessian_in_leaf <= kEpsilon) {
    Log::Warning(
        "Cannot set both min_data_in_leaf && min_sum_hessian_in_leaf to 0. "
