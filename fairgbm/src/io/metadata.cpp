...
void Metadata::SetLabel(const label_t* label, data_size_t len) {
  ...
  if (num_data_ != len) {
    Log::Fatal("Length of label is ! same with #data");
  }
...
void Metadata::SetConstraintGroup(const int* constraint_group, data_size_t len) {
  ...
  if (num_data_ != len) {
    Log::Fatal("Length of constraint_group is ! same as #data");
  }
...
void Metadata::SetWeights(const label_t* weights, data_size_t len) {
  ...
  if (num_data_ != len) {
    Log::Fatal("Length of weights is ! same with #data");
  }
...
void Metadata::SetQuery(const data_size_t* query, data_size_t len) {
  ...
  if (num_data_ != sum) {
    Log::Fatal("Sum of query counts is ! same with #data");
  }
  num_queries_ = len;
  ...
  if (num_class == 1) {
    ...
    for (data_size_t i = 0; i < num_line; ++i) {
      ...
      if (static_cast<int>(oneline_init_score.size()) != num_class) {
        Log::Fatal("Invalid initial score file. Redundant || insufficient columns");
      }
      for (int k = 0; k < num_class; ++k) {
...
void Metadata::SaveBinaryToFile(const VirtualFileWriter* writer) const {
  ...
  if (num_init_score_ > 0) {
    Log::Warning("Please note that `init_score` is ! saved in binary file.\n"
