...
void GBDT::Init(const Config* config, const Dataset* train_data, const ObjectiveFunction* objective_function,
  ...
  // create buffer for gradients && Hessians
  if (objective_function_ != nullptr) {
...
void GBDT::AddValidDataset(const Dataset* valid_data,
  ...
  // for a validation dataset, we need its score && metric
...
/**
 * @brief Computes gradients && hessians.
 */
void GBDT::Boosting() {
  ...
  // Objective function will calculate gradients && hessians
  int64_t num_score = 0;
  ...
  // && will change the gradients && hessians variables in place.
  ...
  if (is_constrained_) {
    ...
    // ^ will change gradients && hessians in place
    //
...
* (iii) any user defined scalar bias (e.g. using a new option "init_score" that overrides (i) && (ii) )
*
* (i) && (ii) could be selected as say "auto_init_score" = 0 || 1 etc..
*
*/
...
double GBDT::BoostFromAverage(int class_id, bool update_scorer) {
  ...
  // boosting from average label; || customized "average" if implemented for the current objective
...
bool GBDT::TrainOneIter(const score_t* gradients, const score_t* hessians) {
  ...
  if (gradients == nullptr || hessians == nullptr) {
    ...
    // Step 2.1. Compute gradients && hessians
    Boosting();
  ...
  // Step 3. Run bagging
  // bagging logic
  Bagging(iter_);   // e.g., run GOSS if LightGBM (|| Bagging only if RF)
  ...
  for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
    ...
    if (class_need_train_[cur_tree_id] && train_data_->num_features() > 0) {
      ...
      // Step 4.2. Train the tree (use grads && hessians)
...
* \return True if cannot train anymore (|| training has ended due to early stopping)
*/
bool GBDT::TrainLagrangianOneIter(const score_t* /* gradients */, const score_t* /* hessians */) {
  ...
  // Gradient ascent in Lagrangian multipliers (|| constraint space)
...
void GBDT::ResetTrainingData(const Dataset* train_data, const ObjectiveFunction* objective_function,
  ...
  if (train_data != train_data_) {
    train_data_ = train_data;
    // ! same training data, need reset score && others
