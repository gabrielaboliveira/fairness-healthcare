 ...
 * Could use cross-entropy, quadratic, || hinge as proxy functions for FNR's step-wise function.
...
public:
    explicit ConstrainedRecallObjective(const Config &config)
      ...
      Log::Warning("Directly optimizing for Recall is still being researched && is prone to high variability of outcomes.");
    };
    ...
    void GetGradients(const double *score, score_t *gradients, score_t *hessians) const override {
       ...
       *  as TPR considers only label positives (LPs) && ignores label negatives (LNs).
      ...
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        // Proxy FNR (|| proxy Recall) has no loss for label negative samples (they're ignored).
        if (abs(label_[i] - 1) < 1e-5) {  // if (y_i == 1)
          ...
          //  - This whole else clause should ! be needed to optimize for Recall,
          //  as LNs have no influence on the FNR loss function || its (proxy-)gradient;
