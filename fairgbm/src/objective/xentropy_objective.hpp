...
/*
 * Implements gradients && Hessians for the following point losses.
 ...
 * with p = 1-exp(-lambda*w), lambda = log(1+exp(f)), f being boosted, && w > 0
 *
 * ConvertToOutput: f -> lambda
 *
 * (1) && (2) are the same if w=1; but outputs still differ.
 *
 */
...
class CrossEntropyLambda: public ObjectiveFunction {
 public:
  ...
  void GetGradients(const double* score, score_t* gradients, score_t* hessians) const override {
    if (weights_ == nullptr) {
      // compute pointwise gradients && Hessians with implied unit weights; exactly equivalent to CrossEntropy with unit weights
      #pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        const double z = 1.0f / (1.0f + std::exp(-score[i]));
        gradients[i] = static_cast<score_t>(z - label_[i]);
  ...
  //
  // ATTENTION: the function output is the "normalized exponential parameter" lambda > 0, ! the probability
  //
  // If this code would read: output[0] = 1.0f / (1.0f + std::exp(-input[0]));
  // The output would still ! be the probability unless the weights are unity.
  //
