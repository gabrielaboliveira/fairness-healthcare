...
class BinaryLogloss: public ObjectiveFunction {
 public:
  explicit BinaryLogloss(const Config& config,
    ...
    if (is_unbalance_ && std::fabs(scale_pos_weight_ - 1.0f) > 1e-6) {
      Log::Fatal("Cannot set is_unbalance && scale_pos_weight at the same time");
    }
    is_pos_ = is_pos;
    if (is_pos_ == nullptr) {
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    // count for positive && negative samples
    ...
    // use -1 for negative class, && 1 for positive class
    label_val_[0] = -1;
    label_val_[1] = 1;
    // weight for label
 ...
 private:
  ...
  double sigmoid_;
  /*! \brief Values for positive && negative labels */
  int label_val_[2];
  /*! \brief Weights for positive && negative labels */
