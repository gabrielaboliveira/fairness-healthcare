...
class PrejudiceRemover(Transformer):
    ...
    """
# customized to include class_weights
    def __init__(self, eta=1.0, sensitive_attr="", class_attr="", class_weights=None, **kwargs):
        """
        Args:
    ...
    def predict(self, dataset):
        ...
        # print("DEBUG: m ndim:", m.ndim)
        # print("DEBUG: m sample:", m[:50] if m.ndim == 1 else m[:5])
        # print("====================================")
    ...
    #CUSTOM
    def show_logistic_hyperparameters(self):
        """Exibe os hiperparâmetros usados no classificador de regressão logística interna."""
        print("Hyperparameters of Internal Logistic Regression Classifier:")
        # Caminho personalizado para o AIF360
        customaif = os.path.join(os.getcwd(), '..', 'aif360inprocessing\kamfadm-2012ecmlpkdd')
        sys.path.append(customaif)

        state_dict = self.__getstate__()
        file_path = state_dict['model_name']

        # Abra o arquivo temporário e carregue o conteúdo
        with open(file_path, 'rb') as file:
            model = pickle.load(file)

        # Verifique os hiperparâmetros do modelo
