...
class AdversarialDebiasing(Transformer):
    ...
    def __init__(self,
                 ...
                 debias=True,
                 class_weights=None #custom class weights
                 ):
        """
        Args:
    ...
    def fit(self, dataset):
        ...
        with tf.variable_scope(self.scope_name):
            ...
            # Setup placeholders
            self.sample_weights_ph = tf.placeholder(tf.float32, shape=[None, 1])
            ...
            # pred_labels_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.true_labels_ph, logits=pred_logits)) # removed for class weights
            ...
            if self.class_weights is None:
                pred_labels_loss = tf.reduce_mean(ce_loss)
            else:
                weighted_ce = ce_loss * self.sample_weights_ph
                pred_labels_loss = tf.reduce_mean(weighted_ce)
            #============================
            ...
            for epoch in range(self.num_epochs):
                ...
                for i in range(num_train_samples//self.batch_size):
                    ...
                    if self.class_weights is not None:
                        batch_weights = np.array([self.class_weights[int(y)] for y in batch_labels.flatten()], dtype=np.float32).reshape(-1,1)
                        batch_weights = batch_weights.reshape(-1, 1).astype(np.float32)
                    else:
                    ...
                    batch_feed_dict = {self.features_ph: batch_features,
                                       ...
                                       self.sample_weights_ph: batch_weights, # custom class weights
                                       self.keep_prob: 0.8}
                    if self.debias:
