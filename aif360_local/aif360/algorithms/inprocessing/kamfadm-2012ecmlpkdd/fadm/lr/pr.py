...
#        print >> sys.stderr, "loss:", coef[0, :], coef[1, :]
        ...
        # l = np.sum(y * np.log(p) + (1.0 - y) * np.log(1.0 - p)) # removed for custom class weights

         # ======= CUSTOM: class weights =======
        if self.class_weights is not None:
            w = np.array([self.class_weights[int(label)] for label in y])
        else:
            w = np.ones_like(y, dtype=float)
        # =================================
        l = np.sum(w * (y * np.log(p) + (1.0 - y) * np.log(1.0 - p)))
...
#        print >> sys.stderr, "grad_loss:", coef[0, :], coef[1, :]
        ...
        # for si in range(self.n_sfv_): #removed for custom class weights
        ...
        for si in range(self.n_sfv_):
            idx = (s == si)
            l[si, :] = np.sum(
                w[idx][:, np.newaxis] * (y[idx] - p[idx])[:, np.newaxis] * X[idx, :],
                axis=0
            )
