...
def train(X, y, ns, opt):
    ...
    if opt.ltype == 4:
        ...
        clr.class_weights = opt.class_weights
        clr.fit(X, y, ns, itype=opt.itype)
        clr.class_weights = None 
    else:
...
if __name__ == '__main__':
    ...
    type=str,
    default=None,
    help='class weighting dict, passed as JSON string'
    )
    # =================================
    ...
    # ===== CUSTOM CLASS WEIGHTS ===== 
    if opt.class_weights is not None:
        opt.class_weights = json.loads(opt.class_weights)
        opt.class_weights = {int(k): float(v) for k, v in opt.class_weights.items()}
        print("[DEBUG] Parsed class_weights BEFORE training:", opt.class_weights)
    # =================================
